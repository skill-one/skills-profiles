...
def test_every_placeholder_is_a_number_the_page_has(workdir):
    ...
    known = set(numbers(jev.Config())) | SLOTS
...
def test_the_page_opens_the_directory_it_sits_in(workdir):
    ...
    assert "`profiles/<id>/domain.json` is the label written about it" in text
...
def test_the_page_says_it_is_generated(workdir):
    ...
    assert "Written by `just index` -" in page(jev.Config(), "en")
    assert "由 `just index` 从这棵树生成" in page(jev.Config(), "zh")
