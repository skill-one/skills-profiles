...
def test_a_changed_hash_retires_that_skill_only(workdir, capsys):
    ...
    assert not jev.profile_dir(config, ALPHA).exists()
    assert jev.profile_dir(config, BETA).is_dir()
...
def test_a_skill_that_vanished_keeps_its_profiles(workdir, capsys):
    ...
    assert jev.profile_dir(config, ALPHA).is_dir()
    captured = capsys.readouterr()
    assert captured.out == ""
