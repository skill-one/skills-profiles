...
The READMEs beside it are the front page of the published root, and the same tree said as progress:
what the four layers and six angles are, then per angle how many skills have it and what share of the
mirror's installs that covers, next to the snapshot they describe. They are written for a reader —
the catalog is the file to build on — and they are a projection of the tree, so a tree that did not
change rewrites them identically. Both are generated by `just index`; neither is written by hand.
