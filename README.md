...
as little as possible: which two directories are the skill and what was written about it, then per
angle how many skills have it and what share of the mirror's installs that covers, under the snapshot
the numbers describe. They are written for a reader — the catalog is the file to build on — and they
are a projection of the tree, so a tree that did not change rewrites them identically. Both are
generated by `just index`; neither is written by hand.
