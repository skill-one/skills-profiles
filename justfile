...
# about them - one domain.json and one description_zh.json per skill. `upstream/` is the rest of
# the mirror, its own index above all. `skills.jsonl` at the root is the catalog: one row per
...
# translate.py/description_zh.json.
[script]
[private]
build angle:
	#!/usr/bin/env sh
