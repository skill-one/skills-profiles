...
| `just index`                    | 写 `<output_dir>/skills.jsonl`——清单：镜像列出的每个 skill 一行，按镜像顺序，由镜像自己的行（`id`、`installs`、`hash`、`fetchedAt`）拼接从该 skill 的 `SKILL.md` 读出的 `description`、`profiles/<id>/description_zh.json` 里的 `description_zh`，以及 `profiles/<id>/domain.json` 里的 `domain` 和 `confidence`。拼接字段未知时为 `null`。只读树——无网络、无调用——整体重写文件。同一次遍历还在旁边写两个 README：见下文 `readme.py`。 |
