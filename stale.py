...
`SKILL.md` upstream has since changed - both angles on those were built from text that is no
longer there, so the profile is deleted and the next batches rebuild it.

A skill that vanished from the mirror keeps its profile: it was paid for, and nothing is left to
rebuild it from. One hash covers a whole directory, because both angles come from that one source.
"""
...
INDEX = common.INDEX  # the catalog: two copies of it, before and after a sync, are the whole input
...
def retire(config: common.Config, ids: list[str]) -> list[str]:
    """Delete those skills' profiles; returns the ones that had any, in path order."""
    dropped = []
    for sid in ids:
        directory = common.profile_dir(config, sid)
        if directory.is_dir():
