...
| `size` | enum | 否 | `1024_1024` | `1024_1024`（1:1），`1024_1536`（2:3 肖像），`1536_1024`（3:2 横幅） — **仅限这三个** |
...
`size=auto` 在编辑时保留输入的纵横比 — 强烈建议，除非编辑明确改变构图。
...
**明确说明主体 + 背景 + 氛围。** "一个磨砂陶瓷水瓶的特写，放在温暖的亚麻布上，柔和的窗户光，标签上有干净的衬线字体 "AQUA+"，边缘微妙高光，适合电子商务，8K 细节，中性背景" — 三个具体指令 — 比起 "一个漂亮的瓶子的产品照片" 更好。
...
**一次迭代一个属性。** 当优化时，每次迭代改变一个东西（光照或背景或姿势或文本），并保持其余提示不变。当只有一个旋钮移动时，模型在迭代中保持构图稳定。
...
- **使用方向性语言进行空间编辑。** "将标题从右上角移动到底部中间"，而不是 "重新定位标题"。
...
- **仅 3 个固定大小**在文生图上（编辑上相同 3 个 + `auto`）。极端纵横比会自动调整到最近的支持大小。
- **提示长度** ~ 几千个标记。长文本块会降低输出质量。
- **编辑的多图像**支持是 "最多 10 个参考的指导"，而不是 ControlNet 风格的堆栈。第一个图像被视为主要图像；其余提供辅助提示。
...
5. 轮询时按 Ctrl-C 发送 `POST .../requests/<id>/cancel`，这样您就不会为停止的 GPU 被计费。
