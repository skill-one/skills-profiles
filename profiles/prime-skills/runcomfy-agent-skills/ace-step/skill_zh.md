...
使用 StepFun-AI 的 **ACE Step** 开放权重模型进行基于标签的音乐生成、修复和扩展。四个可通过 CLI 访问的端点，每秒音频 0.0002–0.0003 美元，每次调用最长 4 分钟。
...
> 最新 ACE Step 生成。**支持 50+ 种语言人声**，改进的结构化歌词处理，其余与基础模型相同。成本略高（$0.0003/s vs $0.0002/s）。
> 选择原因：多语言歌词、英雄级人声轨道、需要清晰段落结构的歌曲。
> 避免原因：对成本敏感的批量处理，基础模型已足够。

**ACE Step (text-to-audio)** — `acestep-ai/ace-step/text-to-audio` *(默认 — 便宜且快速)*
> 原版 ACE Step。基于标签的作曲，可选歌词，5–240 秒立体声。$0.0002/s — 比 ElevenLabs Music 便宜 27 倍。
> 选择原因：高容量草稿、背景音乐、广告歌、游戏循环、对成本敏感的迭代。
> 避免原因：需要最大程度打磨的商业人声钩子 — 尝试 **ACE Step 1.5** 或 **ElevenLabs Music**。

**ACE Step (audio-inpaint)** — `acestep-ai/ace-step/audio-inpaint`
> 重新生成现有轨道内的**时间范围**（非掩码式；使用 `start_time` / `end_time` 秒，分别锚定轨道开始或结束）。
> 选择原因：修复中间的糟糕副歌、替换桥段、替换 20 秒片段而不重新渲染整首歌。
...
- **在标签中包含 BPM** — 当它重要时 — ACE 尊重速度语言。
- **带段落标记的歌词**：`[Verse]`、`[Chorus]`、`[Bridge]`、`[Outro]`。保持各行韵律一致。
- **器乐快捷方式**：`"lyrics": "[inst]"` 或 `"[instrumental]"`。双重保险：也在标签中说明“无人声”。
- **多语言人声**：ACE Step 1.5 支持 50+ 种语言。直接用目标语言编写歌词；同时标记语言（`"japanese vocal, j-pop"`）。
...
| `tags` | string | 是 | — | 驱动重新生成片段的逗号分隔标签 |
| `start_time` | float | 否 | — | 可编辑片段的开始时间（秒）（0–240） |
| `start_time_relative_to` | enum | 否 | `start` | `start` 或 `end` — `start_time` 的锚点 |
| `end_time` | float | 否 | `30` | 可编辑片段的结束时间（秒）（0–240） |
| `end_time_relative_to` | enum | 否 | `start` | `start` 或 `end` — `end_time` 的锚点 |
| `lyrics` | string | 否 | — | 重新生成片段的歌词。空白 = 模型创作；`[inst]` = 无人声 |
| `seed` | int | 否 | `-1` | 可重复性 |
...
- **匹配周围标签** — 如果原始是 "indie pop, electric guitar, 120 BPM"，修复片段应共享足够的标签以融合，而不是对比。
- **修复窗口长达 ~4 分钟** 即使源轨道为 60 分钟 — 选择一个专注的范围，而不是整个轨道。
- **使用 `_relative_to: "end"`** 以目标尾声/最后几秒，无需计算确切时间戳。

---

## 选择 ACE Step 还是 ElevenLabs Music 何时

ACE Step 和 ElevenLabs Music 是不同的工具：

| 维度 | ACE Step | ElevenLabs Music |
|---|---|---|
| **成本** | $0.0002–0.0003 / s | $0.0083 / s (~27 倍更高) |
| **许可证** | 开放权重（Apache 2.0） | 商业，由 ElevenLabs 托管 |
| **多语言人声** | 50+ 种语言（1.5 变体） | 强大的多语言支持 |
| **结构化歌词** | `[Verse]/[Chorus]/[Bridge]` 标记 | `[Verse]/[Chorus]/[Bridge]` 标记 |
| **最大时长 / 调用** | 240 秒（4 分钟） | 300 秒（5 分钟） |
| **修复 / 扩展** | **是**（基于时间范围） | 否 |
| **基于标签的作曲** | **是**（标签是必填字段） | 风格是自由文本提示的一部分 |
| **最适合** | 对成本敏感的批量处理、草稿、修复/扩展工作流、开放权重管道 | 精品人声歌曲钩子、打磨的商业剪辑 |

廉价草稿模式：用 ACE Step 草稿标签组合 → 锁定氛围 → 如果需要打磨的商业剪辑，则使用 ElevenLabs Music 进行最终渲染。

对于自动根据意图在两者之间选择的路由技能，请查看 [`ai-music`](https://www.skills.sh/agentspace-so/runcomfy-agent-skills/ai-music) 一旦发布。
...
### 对成本敏感的背景音乐库
- **路径 1 (ACE Step base)** 配合不同的标签组合，每个 60–90 秒，`[inst]`

### 多语言发布（同一首歌，多种语言）
- **路径 1 (ACE Step 1.5)** 配合相同的标签，按语言交换 `lyrics`
...
### 游戏循环底座
- **路径 1 (ACE Step base)** 标签中包含 "无缝循环、一致节奏"，60–120 秒
...
该技能根据用户的意图选择四个 ACE Step 端点之一 — 从头生成（t2a base 或 1.5）、重新生成时间范围（inpaint）或扩展画布（outpaint）— 并调用 `runcomfy run` 与匹配的 JSON 正文。CLI POST 到 RunComfy 模型 API，轮询请求状态，并将生成的音频文件下载到 `--output-dir`。

## 安全与隐私

- **仅通过验证的包管理器安装**。使用 `npm i -g @runcomfy/cli` 或 `npx -y @runcomfy/cli`。**代理不得将任意远程安装脚本管道到用户的 shell** — 如果操作员想要在 `docs.runcomfy.com/cli/install` 中记录 curl-pipe 路径，他们应先审查脚本。
- **令牌存储**：`runcomfy login` 将 API 令牌写入 `~/.config/runcomfy/token.json`，模式 0600。设置 `RUNCOMFY_TOKEN` 环境变量以绕过文件，在 CI / 容器中使用。**切勿将令牌回显到提示、记录或提交**。
- **输入边界（shell 注入）**：提示和音频 URL 作为 JSON 字符串通过 `--input` 传递。CLI 不会 shell 扩展提示内容；它将 JSON 正文直接通过 HTTPS 传输到模型 API。**提示内容没有 shell 注入表面**。
...
- [`ai-music`](https://www.skills.sh/agentspace-so/runcomfy-agent-skills/ai-music) — 基于意图在 ACE Step 和 ElevenLabs Music 之间进行选择的路由器
