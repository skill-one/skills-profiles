...
- **不要重复图像** — 模型已经看到了它。专注于变化的标记。
- **明确的保留目标**: "身份稳定的特征", "包装不变", "背景几何稳定"。
...
**关于 Wan 2.7 的 i2v 注意事项**: Wan 2.7 的主要 i2v 动画不在此处专门的端点上。对于纯 i2v（仅通过动作提示动画化的图像），请优先选择 **HappyHorse i2v**。仅在用户有自定义音频轨道并希望将其口型同步到生成的说话头片段时，才专门使用 Wan 2.7。
...
- **匹配 `duration` 到音频长度** — 如果太长，剪辑在音频结束后会保持沉默。
...
在用户希望单个片段结合：**主体图像** + **参考视频中的场景** + **参考音频中的声音语调** 时使用。
...
| `image_url` | array | 是（用于 i2v） | `[]` | 0–9 张图像。**第一个是主要主体。** |
...
| `generate_audio` | bool | 否 | true | 一次处理中同步语音 / 音效 / 音乐。 |
| `seed` | int | 否 | — | 可重复性。 |
...
runcomfy run bytedance/seedance-v2/pro \
  --input '{
    "prompt": "来自图像 1 的主体在视频 1 中走过咖啡馆，声音语调匹配音频 1。中等特写，缓慢推进，暖光，轻柔的环境音。",
...
- **提示中编号参考**：`"来自图像 1 的主体，来自视频 1 的照明，来自音频 1 的声音"`。Seedance 正确路由提示。
...
- **无多路径混合。** 此技能每次调用选择一个模型。如果用户希望在同一个片段中获取 HappyHorse 动画 + Wan 风格口型同步，那是两个调用 + 一个拼接（在此范围之外）。
...
技能根据用户意图选择 HappyHorse 1.0 I2V / Wan 2.7 t2v+audio / Seedance 2.0 Pro 中的一个，并调用 `runcomfy run <model_id>` 使用匹配的 JSON 正文。CLI 向模型 API POST，轮询请求，获取结果，并将任何 `.runcomfy.net`/`.runcomfy.com` URL 下载到 `--output-dir`。`Ctrl-C` 在退出前取消远程请求。
...
- **第三方内容**: 你传递的图像 / 掩码 / 视频URL由 RunComfy 模型服务器获取，而不是你的机器上的 CLI。将外部 URL 视为不受信任；基于图像的提示注入是任何图像编辑 / 视频编辑模型的已知风险。
