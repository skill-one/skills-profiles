...
**图像编辑，意图路由。** 此技能不会将你锁定在一个模型上——它会根据用户实际的需求，在 RunComfy 目录中挑选合适的编辑模型：批量保持身份、多语言文本重写、单次精确编辑或基于掩码的区域替换。
...
| 用户意图 | 模型 | 原因 |
|---|---|---|
| 批量编辑 1-20 张图像并保持一致性（SKU 画廊、A/B 变体） | **Nano Banana Edit** | 每次调用最多 20 张输入图像；系列锁定宽高比/分辨率 |
| 替换背景，保持主体身份 | **Nano Banana Edit** | 在“保持 X 不变”提示下，身份保持效果强烈 |
...
| 单次精确局部编辑（“她现在拿着一把橙色雨伞”） | **Flux Kontext Pro** | 单参考单指令，高保真度保留 |
| 基于掩码的对象移除（电线、水印、干扰物） | **Z-Image Turbo Inpaint** | 需要掩码，强度可调，边缘一致 |
| 基于掩码的区域替换（使用掩码的全背景替换） | **Z-Image Turbo Inpaint** | 高强度+干净掩码=干净替换 |
| 未指定时的默认值 | **Nano Banana Edit** | 最灵活，支持单张和批量 |
...
| 字段 | 类型 | 必填 | 默认 | 备注 |
|---|---|---|---|---|
| `prompt` | string | 是 | — | 首先提出保留目标，最后说明变更。 |
| `image_urls` | array | 是 | — | **1-20** 公开可获取的 HTTPS URL。 |
| `number_of_images` | int | 否 | 1 | 每次调用 1-4 张输出。 |
...
runcomfy run google/nano-banana-2/edit \
  --input '{
    "prompt": "保持主体身份、姿势和服装不变。将背景转换为雨夜的赛博朋克街道。",
...
| 字段 | 类型 | 必填 | 默认 | 备注 |
|---|---|---|---|---|
| `prompt` | string | 是 | — | 编辑指令；优先提出保留。 |
| `images` | string[] | 是 | — | **最多 10** HTTPS URL。第一个是主要；其余为辅助。 |
...
runcomfy run openai/gpt-image-2/edit \
  --input '{
    "prompt": "将图像 1 的主体组合到图像 2 的房间中。匹配图像 2 的光照和调色板。保持图像 1 主体身份不变。",
...
- **精确引用图像内文本**。为非拉丁文命名脚本: `"日语假名"`, `"西里尔字母"`, `"阿拉伯语从右到左"`。
...
仅单张图像 — 无数组。对于多图像流程，使用路由 1 (Nano Banana Edit)。
...
- **一个声明性指令**。 "她现在拿着一把橙色雨伞并微笑" — 疑命令，单次变更。
- **优先保留**。 首先提出 `"保持[不变元素]"`，然后说明变更。
- **小步迭代**。 单次通过上的复合编辑会漂移；拆分为顺序通过。

---

## 路由 4：Z-Image Turbo Inpaint — 基于掩码的精确区域编辑
...
| 字段 | 类型 | 必填 | 备注 |
|---|---|---|---|
| `prompt` | string | 是 | 要填充/替换的内容；未掩码周边的保留约束。 |
| `image` | string | 是 | 源图像 URL。 |
| `mask_image` | string | 是 | **灰度掩码 URL**（白色=填充，黑色=保留）。 |
...
runcomfy run tongyi-mai/z-image/turbo/inpainting \
  --input '{
    "prompt": "移除天花板上的电线；保留屋顶线和天空渐变；使天空薄而干净。",
    "image": "https://.../街道.jpg",
...
- **需要掩码 URL** — 灰度，白色=填充区域，黑色=保留。掩码边缘轻微模糊（1-3px）比锐利的二进制混合效果更好。
- **强度按意图**: `0.3–0.5` 用于精修/清理，`0.6–0.7` 用于对象替换并匹配风格，`0.8–1.0` 用于全区域替换。
- **在提示中命名掩码外保留的内容**: `"保留屋顶线和天空渐变"`, `"匹配砖块图案和灰泥色调"`。
...
- **每个路由继承其模型的限制**。 Nano Banana: 1-20 输入，1-4 输出。 GPT Image 2 Edit: 最多 10 参考项，4 个固定尺寸。 Flux Kontext: 单参考。 Z-Image Inpaint: 需要掩码。
...
该技能根据用户意图选择 Nano Banana Edit / GPT Image 2 Edit / Flux Kontext Pro / Z-Image Turbo Inpaint 中的一个，并调用 `runcomfy run <model_id>` 并附带匹配的 JSON 正文。CLI 向模型 API 发送 POST 请求，轮询请求，获取结果，并将任何 `.runcomfy.net`/`.runcomfy.com` URL 下载到 `--output-dir`。 `Ctrl-C` 在退出前取消远程请求。

## 安全与隐私

- **令牌存储**: `runcomfy login` 将 API 令牌写入 `~/.config/runcomfy/token.json`，权限为 0600（仅所有者可读写）。设置 `RUNCOMFY_TOKEN` 环境变量可完全绕过文件，在 CI / 容器中使用。
- **输入边界**: 用户提示作为 JSON 字符串通过 `--input` 传递给 CLI。CLI 不会对提示进行 shell 扩展；它将 JSON 正文直接通过 HTTPS 传输到模型 API。提示内容没有 shell 注入表面。
