...
Wan-AI 的 **Wan 2.7** — 标杆视频模型，支持多参考条件处理和音频驱动口型同步 — 部署在 **RunComfy 模型 API** 上。
...
| 您需要 | 使用 |
|---|---|
| 将口型同步视频到您提供的音频轨道 | **Wan 2.7** (`audio_url`) |
| 多参考精细运动控制 | **Wan 2.7** |
| 平滑过渡、精确运动物理 | **Wan 2.7** |
| 目前投票第一的盲选视频模型 | HappyHorse 1.0 |
| 多模态电影效果，包含图像+视频+音频参考 + 生成语音 | Seedance 2.0 Pro |
| 现有素材的电影级运动编辑 | Kling Video O1 |
| 超快迭代 | LTX 2 |

如果用户明确提到 "Wan" / "Wan 2.7" / "wan-ai" / "alibaba video"，无论何种情况都路由至此。
...
runcomfy run wan-ai/wan-2-7/text-to-video \
  --input '{
    "prompt": "中景拍摄发言人，温暖的主光，固定三脚架，轻微呼吸动作。",
...
**用普通英语描述相机和运动。** "缓慢向前推进"，"固定三脚架，低角度"，"手持跟拍"，"从上方俯冲移动"。在提示中优先描述拍摄场景。

**每个片段一个主要动作。** 不要堆叠多个竞争动作。选择节奏点： "她转身，然后微笑" 而不是 "她转身 AND 微笑 AND 一辆公交车经过 AND..."。

**使用 `negative_prompt` 指出具体问题。** 好： "无字幕，无水印，无闪烁"。差（模糊）： "无不良光线"。

**默认开启提示扩展。** 短提示会被模型自动重写。对于简洁/精确的提示（如品牌严格广告文案），使用 `enable_prompt_expansion: false` 禁用。

**音频规格很重要。** `audio_url` 必须是 3–30 秒，≤15MB，WAV/MP3。超出范围的文件会被拒绝。匹配音频长度与片段时长。

**迭代种子。** 当您希望同一提示的不同变体保持一致输出时，重复使用相同的种子。改变种子以获得真实多样性。

**反模式：**
- 静态帧描述 → 运动会模糊不清。
- 模糊的负面提示 ("无不良颜色") → 被忽略。
- 音频超出 3–30 秒 / 15MB / WAV-MP3 规格 → 被拒绝。
- 提示超过 5000 字符 / 1500 个 token → 输出质量下降。

## Wan 2.7 的优势

| 用例 | 为什么选择 Wan 2.7 |
|---|---|
| **带自定义配音的口型同步广告** | `audio_url` 接受您的轨道 |
...
```
电影感中景拍摄产品在大理石表面上，柔和的影棚灯光，缓慢的轻微向前推进，浅景深，高端商业风格，清晰的 1080p 细节
```

**口型同步发言人（带 `audio_url`）：**

```
中景拍摄自信的发言人，在柔和灯光的录音棚中，略微朝向镜头，固定三脚架，浅景深，来自镜头左侧的温暖主光。
```

**垂直平台原生：**

```
9:16 垂直短片。咖啡师制作单份浓缩咖啡，蒸汽升入晨光中，丰富的奶油缓慢形成。手持拍摄，浅景深，温暖咖啡馆氛围。
```

## 限制

- **时长上限 15 秒。** 对于更长的叙事，拼接多个调用。
- **无原生 4K** — 1080p 为上限。
- **宽高比** — 仅支持 5 个预定义值。
- **音频规格** — 3–30 秒，≤15MB，仅支持 WAV/MP3。
- **参考媒体上限 5**（图像 + 视频 + 语音组合）。
- **用于生成语音（无单独音频轨道），使用 Seedance 2.0 Pro** — Wan 接受音频而非生成。
...
- **Token 存储**：`runcomfy login` 将 API token 写入 `~/.config/runcomfy/token.json`，权限为 0600（仅所有者可读写）。在 CI / 容器中设置 `RUNCOMFY_TOKEN` 环境变量可绕过文件。
- **输入边界**：用户提示作为 JSON 字符串通过 `--input` 传递给 CLI。CLI 不会对提示进行 shell 扩展；它直接将 JSON 正文通过 HTTPS 传输给模型 API。提示内容没有 shell 注入风险。
- **第三方内容**：您传递的图像 / 掩码 / 视频链接由 RunComfy 模型服务器获取，而非您机器上的 CLI。将外部 URL 视为不可信；基于图像的提示注入是任何图像/视频编辑模型已知的风险。
- **出站端点**：仅 `model-api.runcomfy.net`（请求提交）和 `*.runcomfy.net` / `*.runcomfy.com`（生成输出下载白名单）。无遥测数据，无回调。
- **生成文件大小上限**：CLI 会中止任何大于 2 GiB 的下载，以防止恶意或失控的模型输出导致磁盘填满。
