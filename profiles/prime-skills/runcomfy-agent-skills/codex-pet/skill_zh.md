...
- `spritesheet.webp` — Codex Pet精灵图集，**1536x1872** PNG或WebP，8列x9行，每行192x208个单元格，透明背景。

9行对应Codex播放的9个动画状态。每行使用固定数量的领先帧；尾随单元格保持完全透明。
...
这个技能遵循Codex内置Codex Pet使用的相同模式：**一个标准的姿势，通过ImageMagick微变换在整个单元格中复制**，以实现微妙的动画（1-2 px的移动，眨眼帧，倾斜帧）。这与官方 `hatch-pet` 输出的实际单元格外观相匹配——Codex桌面应用中可见的Codex Pet动画是故意设计得微妙的。

选择这个技能的情况：

- 你想要一个自定义的Codex Pet，但没有Codex Pro / `$imagegen`。
- 你想要通过RunComfy模型API构建自定义Codex Pet。
- 你想要从源图像文件夹中批量生成Codex Pet（每个宠物一次调用）。
- 你正在使用OpenAI Codex Pet竞赛，但视觉效果背后使用的是不同的模型。
- 你明确说了“codex pet”, "/hatch", "make me a codex pet", "spritesheet.webp", "desktop pet for codex"。

## Codex Pet动画行

Codex读取一个固定的图集：8列，9行，192x208个单元格。每个Codex Pet行对应一个特定的动画状态，并具有特定数量的领先帧。
...
- **夸张的chibi比例**：头部占据总高度的大约60%；身体和腿又小又短。整个角色应该能适应接近正方形的边界框。
- 像素艺术相邻的低分辨率吉祥物，粗大的轮廓
- 厚重的深色1-2 px轮廓，可见的阶梯状像素边缘
- 有限的调色板，平面赛璐璐着色，简单的表情，微小的肢体
- 透明背景

避免：运动线，阴影，发光，闪光，漂浮效果，文本标签，场景，白色/黑色背景。

## 前置条件

1. **RunComfy CLI** — `npm i -g @runcomfy/cli`
2. **RunComfy账户** — `runcomfy login`。CI替代方案：`RUNCOMFY_TOKEN=<token>`。
3. **ImageMagick** — `brew install imagemagick`（macOS）或 `apt-get install imagemagick`（Linux）。提供 `magick` 命令以确定性地组装图集。
...
2. **单元格标准化** — 色度键品红色到alpha 0，修剪，按比例调整到192x208，带有透明填充。
...
微变换方法与Codex内置Codex Pet实际执行的操作相匹配——Codex Pet动画是故意设计得微妙的，因此每个单元格的1-2 px平移和眨眼遮罩就足以产生正确的视觉效果，而无需消耗72个GPT Image 2调用。

### 第1步：生成标准的Codex Pet（1次调用）
...
runcomfy run openai/gpt-image-2/edit \
  --input "{
    \"prompt\": \"生成一个基于输入图像的标准Codex数字宠物精灵。夸张的chibi比例：头部占据总高度的大约60%；身体和腿又小又短。整个宠物角色必须能适应接近正方形的边界框（整体宽高比接近1:1）。像素艺术相邻的低分辨率吉祥物，粗大的全身轮廓，厚重的深色1-2 px轮廓，可见的阶梯状像素边缘，有限的调色板，平面赛璐璐着色，简单的表情，微小的肢体。居中显示。没有精致的插图，没有画家风格的渲染，没有动画关键艺术，没有3D渲染，没有光泽的应用图标润色，没有逼真的细节。背景：实心的平面品红色 ${CHROMA} 色度键填充在宠物轮廓之外。宠物本身不能使用色度键颜色或任何接近品红色的亮部。没有渐变，没有阴影，没有光晕，没有场景，没有文本。保留从输入图像中提取的身份特征。\",
    \"images\": [\"${SOURCE_URL}\"],
    \"size\": \"1024*1024\"
  }" \
...
### 第2步：将标准转换为192x208的Codex Pet单元格

色度键品红色到alpha，修剪到宠物精灵的边界框，按比例调整到192x208，带有透明填充。
...
18%的模糊度是为GPT Image 2的抗锯齿品红色边缘调优的。如果Codex Pet有更宽的品红色光晕，请将其调整为25%；如果宠物有接近品红色的亮部，则可能需要将其调整为8-10%。
...
# 9个Codex Pet行，每个行对应一个动画状态及其特定数量的领先帧
...
微变换表是Codex Pet在Codex桌面应用中具有可读但微妙的运动感的原因。根据个人喜好调整每行的数字；这些增量被故意设计得很小（1-2 px），这样Codex Pet感觉生动而不分散注意力。
...
**首先锁定chibi比例。** "夸张的chibi比例，头部约占总高度60%" 是一个细长的高角色（这在192x208单元格中会因 pillarbox 而效果不佳）和一个头部主导的chibi（自然地填充单元格）之间的区别。后者是Codex内置Codex Pet的样子。

**明确在Codex Pet基础提示中要求品红色 `#FF00FF` 色度键。** GPT Image 2只输出RGB（没有alpha），因此唯一获得透明Codex Pet的方法是在后处理中消除已知的背景颜色。

**宠物本身禁止使用色度键颜色。** 添加： "宠物本身不能使用色度键颜色或任何接近品红色的亮部。" 否则，色度键步骤会消除Codex Pet中偶然出现品红色-ish的部分。

**锁定风格。** "像素艺术相邻，粗大的轮廓，1-2 px轮廓，有限的调色板，平面赛璐璐着色"——锁定使Codex Pet符合Codex房屋风格的每个术语。

**禁止错误风格。** "没有精致的插图，没有画家风格的渲染，没有动画关键艺术，没有3D渲染，没有光泽的应用图标润色，没有逼真的细节。" 没有这个，GPT Image 2会倾向于过度渲染的动漫艺术。

**反模式**：
- 通用"透明背景"——GPT Image 2会以接近白色进行绘制。使用色度键。
- 允许模型自由发挥比例——它会绘制一个细长狭窄的chibi，不适合192x208。
- 在一个提示中混合风格——锁定一个风格锚点并坚持它。

## 调整微动画

步骤3中的默认ImageMagick配方生成的Codex Pet动画类似于内置Codex Pet——轻微的上下浮动，偶尔眨眼，跳跃弧线，头部倾斜。要使动画更明显或更不明显，请调整增量：

- **更大的空闲浮动**：将 `shift:0:-1` 改为 `shift:0:-2` 在行0。
- **更快的跑步周期**：增加行1中的水平移动（例如 `shift:3:0` 而不是 `shift:2:-1`）。
- **更高的跳跃**：将行4的顶峰从 `shift:0:-8` 改为 `shift:0:-12`。
- **审查中更强的头部倾斜**：将 `rotate:-2` / `rotate:2` 改为 `rotate:-4` / `rotate:4`。

保持增量小（≤ 4 px 或 ≤ 4°），这样Codex Pet不会变得分散注意力。
...
**为什么使用这个Codex Pet技能而不是 `hatch-pet`？** 官方的 `hatch-pet` 需要Codex内部的 `$imagegen` 系统技能（需要Codex Pro）。这个技能只需要 `RUNCOMFY_TOKEN` 并通过RunComfy CLI运行相同的动画行规范，总共一次GPT Image 2调用。

**Codex Pet生成需要多长时间？** 约2分钟——1次GPT Image 2编辑调用（约90秒）加上几秒钟的ImageMagick图集组装。

**为什么只使用一次API调用？** Codex桌面应用中的Codex Pet动画是故意设计得微妙的（你可以通过检查任何内置Codex Pet的图集——72个几乎相同的姿势，每个姿势只有微小的变化））。一个标准的姿势加上确定的ImageMagick微变换就能产生相同的动画感觉，而无需消耗72个单独的生成调用。

**Codex Pet技能能否使用非人类主题？** 是的——宠物、吉祥物、物体、食物都适用。基础提示会自动将源简化为Codex Pet房屋风格。
...
**我能添加自定义姿势或替换行吗？** 是的——修改步骤3中的 `build_row` 调用。每行图集的槽位数量必须与Codex合同匹配（空闲=6，向右/向左跑=8，挥手=4，跳跃=5，失败=8，等待/跑步/审查=6），以便Codex能正确播放它们。

## 限制

- **每个Codex Pet只有一个标准姿势**——动画是通过ImageMagick变换实现的，而不是多帧模型生成。这匹配了内置Codex Pets的微妙动画，但不会产生剧烈的运动（例如，不同的跑步周期，每帧都有区别）。
- **GPT Image 2不输出alpha**——品红色色度键+后处理是一个解决方案。如果Codex Pet有接近品红色的颜色（对于chibi调色板来说很少见），请将色度键改为不同的纯色（`#00FFFF` 青色或 `#00FF00` 绿色）在提示和后处理中。
...
1. 技能调用 `runcomfy run openai/gpt-image-2/edit` 一次，使用用户的源图像和紧密的chibi比例提示，生成一个1024x1024的标准Codex数字宠物精灵，背景为品红色。
2. ImageMagick将品红色色度键为alpha 0，修剪精灵边界框，按比例调整到192x208单元格。
3. ImageMagick程序化构建9行条带，通过微变换（1-2 px平移，眨眼遮罩，旋转，镜像）对标准单元格应用。
4. 9行条带堆叠成1536x1872的Codex Pet图集；图集转换为WebP。
5. 编写 `pet.json` 清单；两个文件都复制到 `${CODEX_HOME:-$HOME/.codex}/pets/<name>/`，Codex自动识别自定义Codex Pet。
...
不是Codex客户端。当 `$imagegen` 可用时不是 `hatch-pet` 的替代方案——当Codex Pro可用时，官方 `hatch-pet` 更可取。不是自托管GPT Image 2——依赖于一个有效的RunComfy账户。

## 安全与隐私

- **令牌存储**：`runcomfy login` 将API令牌写入 `~/.config/runcomfy/token.json`，权限为0600。设置 `RUNCOMFY_TOKEN` 环境变量以在CI中绕过文件。
- **输入边界**：Codex Pet提示作为JSON通过 `--input` 传递。CLI不会进行shell扩展。没有shell注入表面。
- **第三方内容**：源图像URL由RunComfy服务器获取。将外部URL视为不受信任——基于图像的提示注入是任何图像编辑模型已知的风险。
- **出站端点**：仅 `model-api.runcomfy.net` 和 `*.runcomfy.net` / `*.runcomfy.com`。
- **生成文件大小上限**：CLI中止任何单个Codex Pet标准下载 > 2 GiB。
- **本地安装路径**：最终Codex Pet写入 `${CODEX_HOME:-$HOME/.codex}/pets/<pet-name>/`。没有远程上传。
