...
将静态图像扩展到其原始画布之外——去裁剪、改变宽高比、填充相机未捕捉到的部分。这项技能在 RunComfy 目录中的身份保持编辑端点之间进行路由，为基于文本的扩展、参考式匹配或品牌锁定式延续选择正确的端点。
...
> 选择用于：宽高比改变（方形 → 16:9）、去裁剪肖像、使用匹配环境扩展风景照片。
> 避免用于：像素级精确扩展以匹配纹理接缝——使用 ComfyUI 扩展工作流程。

**GPT Image 2 Edit** — `openai/gpt-image-2/edit`
> 最高支持 10 张参考图像，布局精确指令遵循。当扩展需要匹配参考样式或包含布局重新定位时很有用。
> 选择用于：合成扩展（扩展画布 + 从另一张图像粘贴元素）、画布变化期间的布局重新定位。
> 避免用于：简单的扩展，无需外部参考。

**FLUX Kontext Pro** — `blackforestlabs/flux-1-kontext/pro/edit`
> 单一指令、高保留编辑。使用格式：`"将画布扩展为 16:9 宽高比。添加匹配的天空和建筑，从现有场景延续。保留原始图像中的所有内容。"`
...
runcomfy run google/nano-banana-2/edit \
  --input '{
    "prompt": "将画布扩展为 16:9 宽高比，在图像的左右两侧添加匹配的环境。延续现有的背景风格——相同的照明、相同的相机距离、相同的调色板。保留原始主体、姿势、构图和中心内容，与输入完全一致。",
...
runcomfy run google/nano-banana-2/edit \
  --input '{
    "prompt": "向下扩展画布以显示主体的整个上半身和手臂。延续现有的服装风格、照明和背景。保留面部和当前可见区域，与输入完全一致。",
...
- **首先描述画布变化**: `"将画布扩展为 [宽高]"`，`"向下扩展"`，`"在两侧扩展约 30%"`。
- **描述扩展的内容**: 延续背景风格、匹配照明、匹配相机距离、匹配调色板。
- **以保留结束**: `"保留 [原始可见区域] 与输入完全一致"`。否则 Nano Banana 可能会微妙地重新生成原始部分。
...
3. **ComfyUI 工作流程** — RunComfy 主办了多个扩展节点图：
...
这些是 GUI 工作流程，不是 CLI 端点。CLI 无法访问它们——在 RunComfy ComfyUI 云中打开它们以进行更精细的控制。
...
### 从肖像生成横幅/英雄图像
- **路由 1** 配合 `aspect_ratio: "21:9"` 或 `"16:9"`，提示扩展两侧，匹配环境
...
### 多步扩展（扩展，然后重新扩展）
- 链接：扩展传递 1 → 将结果用作传递 2 的输入。每次扩展约 30–50%，以避免边界处质量下降。

### 此技能无法执行的操作
- **基于掩码的局部编辑**（在现有画布内部填充孔洞）：参见 [`image-inpainting`](https://www.skills.sh/agentspace-so/runcomfy-agent-skills/image-inpainting)。
...
- [ComfyUI 工作流程](https://www.runcomfy.com/comfyui-workflows?utm_source=skills.sh&utm_medium=skill&utm_campaign=image-outpainting) — 搜索 "outpaint" 获取专用的扩展节点图
...
技能分类用户意图——简单的宽高比交换、参考式匹配或品牌锁定式延续——选择匹配的编辑端点，并调用 `runcomfy run` 带有扩展形状的 JSON 正文。CLI 向模型 API 发送 POST 请求，轮询请求状态，并将结果下载到 `--output-dir`。
...
- [`image-inpainting`](https://www.skills.sh/agentspace-so/runcomfy-agent-skills/image-inpainting) — 基于掩码的内部区域编辑（与扩展相反）
