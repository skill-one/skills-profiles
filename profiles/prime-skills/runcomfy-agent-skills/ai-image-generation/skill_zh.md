...
通过 [RunComfy](https://www.runcomfy.com/?utm_source=skills.sh&utm_medium=skill&utm_campaign=ai-image-generation) CLI 生成和编辑图像，支持 11 款以上 AI 模型 — 文本到图像和图像到图像，单认证，单命令。此技能根据用户意图选择合适的模型，并提供文档化的提示模式 + 每个 `runcomfy run` 调用的精确指令。
...
> 避免原因：照片真实肖像 — **Seedream 5** 在肤色和灯光方面更胜一筹。
...
> 最新字节跳动 Seedream 层级。照片真实肤色，自然灯光，强烈的东亚美学。
...
> 每次调用最多 1–20 张输入图像，默认保留身份，尊重空间语言（"右上角"，"左侧对象"）。
...
> 最多 10 张参考图像，多语言图像中文本重写，布局精确的重新定位。
...
> 单参考单指令，最高的保留保真度（"保留除 X 之外的一切"）。
> 选择原因：单张图像精确的局部编辑（"仅将她的雨伞改为橙色"）。
...
| `steps` | int | 否 | 25 (9B) / 4 (4B) | 步骤蒸馏；4–8 足够用于构思，~25 用于光泽，>25 买不到多少 |
| `width` | int | 否 | 1024 | 512–1536 典型，最大 ~2K 总计。纵横比上限 16:9 |
| `height` | int | 否 | 1024 | 与 width 的纵横比意图匹配 |

在同一个端点上最多支持 **4 张参考图像**，用于风格迁移 / 指导构图。字段名在模型页面文档中记录 [模型页面](https://www.runcomfy.com/models/blackforestlabs/flux-2-klein/9b/text-to-image?utm_source=skills.sh&utm_medium=skill&utm_campaign=ai-image-generation)。
...
- **主语优先，场景其次，修饰语最后。** "一只紫色的小猫 … 在苔藓石头上 … 金色黄昏，浅景深。"
- **步骤策略**: 4–8 用于构思，~25 用于光泽。不要超过 28 — 收益递减。
- **9B vs 4B**: 默认 9B；仅在需要亚秒批量构思时降至 4B。
- **多参考**: 1–4 张参考 URL；在提示中描述角色 (`"主体来自 ref 1，调色板来自 ref 2"`).
...
runcomfy run openai/gpt-image-2/text-to-image \
  --input '{
    "prompt": "极简产品海报。居中粗体标题精确读取 \"AURORA — Spring 2026\"，使用干净的白色无衬线字体，背景为深海军蓝。标题下方有一行等宽字体读取 \"runs on water\"。3:2 布局。",
    "size": "1536_1024"
  }' \
  --output-dir ./out
```
...
runcomfy run openai/gpt-image-2/text-to-image \
  --input '{
    "prompt": "日本杂志封面。垂直标题精确读取 \"今日のおすすめ\"，粗体日语假名，右边缘对齐，照片真实女性肖像。",
    "size": "1024_1536"
  }' \
  --output-dir ./out
```
...
- **精确引用图像中文本。** `"标志读取精确为 'CLOSED'"` — 不加引号，模型会改写。
- **为非拉丁文本命名脚本**: `"Japanese kana"`, `"Cyrillic"`, `"Arabic right-to-left"`. 没有这个会回退到罗马化。
- **布局语言受尊重**: `"top-left"`, `"centered"`, `"两行堆叠"`, `"基线对齐"`.
- **仅 3 个尺寸。** 不要传递任意宽度。
...
runcomfy run google/nano-banana-2/text-to-image \
  --input '{
    "prompt": "三个陶瓷咖啡杯照片，放在大理石台面上，温暖的早晨光线，俯视角度，极简风格",
    "num_images": 4,
    "aspect_ratio": "1:1",
    "resolution": "0.5K"
  }' \
...
- **降至 `0.5K` 进行构思，仅在最终稿时跳至 `2K`+ — `4K` ~16× 的成本高于 `0.5K`。
