...
Google **Nano Banana 2** — Gemini 家族中的快速级文本到图像模型 — 在 **RunComfy 模型 API** 上托管。针对创意构思、社交缩略图批处理和快速草稿进行了优化，具有强大的图像内排版式设计。
...
| `num_images` | int | 否 | 1 | 1–4。用于创意构思轮次时使用 4。 |
...
**主体优先的声明性语法。** "一部电影般的特写肖像，一位美国女性站在东京的霓虹灯下，浅景深，反射的湿街道，超精细，逼真的皮肤纹理" — 主要主体，然后是动作、环境、风格、相机。前置主体；后跟指令。
...
|---|---|
| **营销草稿缩略图（4 个批处理）** | 0.5K 时快速迭代，然后将获胜者提升到 2K |
| **社交平台原生** | 支持宽高比包括 9:16, 4:5, 21:9 |
...
```
一部电影般的特写肖像，一位美国女性站在东京的霓虹灯下，浅景深，反射的湿街道，超精细，逼真的皮肤纹理
```

**品牌资产卡片带引用文本：**

```
一个极简 16:9 产品卡片：一个磨砂黑陶瓷马克杯居中在柔和暖灰色纸张背景上，左上角的反光，标题 "Brewed Quietly" 在右上角干净粗体无衬线字体，平衡的负空间下方，电子商务准备，干净工作室灯光
```

**垂直平台原生：**

```
一个 9:16 垂直主角用于一个健康品牌：一个陶瓷茶杯在亚麻跑道上，柔和的晨光侧面照明，顶部手绘衬线大号字体 "Slow Down"，轻柔的蒸汽升起，中性调色板，无杂乱
```
...
- **令牌存储**：`runcomfy login` 将 API 令牌写入 `~/.config/runcomfy/token.json`，模式 0600（仅所有者可读写）。设置 `RUNCOMFY_TOKEN` 环境变量以在 CI / 容器中完全绕过文件。
