...
**视频编辑，意图路由。** 此技能不会将你锁定在一个模型上——它会根据用户实际想要的内容，在RunComfy目录中选择合适的视频编辑模型：一般重制、从参考片段中转移动作，或轻量级的身份稳定服装/背景替换。
...
| 应用电影色调/商业润色 | **Wan 2.7 Edit-Video** | 擅长单方向全局外观变化 |
| **精确转移动作**从参考视频到目标角色 | **Kling 2.6 Pro Motion Control** | 设计用于身份保持的动作映射 |
| 目标角色的唇部动作与源视频的唇部运动同步 | **Kling 2.6 Pro Motion Control** | 建立在紧密的时间一致性之上 |
| **轻量级服装/服装交换**与身份保留 | **Lucy Edit Restyle** | 核心优势是局部身份稳定的编辑 |
| **身份稳定的重制**（“宇航员在沙漠中”，“温暖的黄金时刻光照”） | **Lucy Edit Restyle** | 专门用于重制的时序一致性 |
| 如未指定，则默认 | **Wan 2.7 Edit-Video** | 最通用，最高分辨率 |
...
- **首先强调保留目标**: `"保留[面部 / 姿势 / 动作 / 构图 / 唇部动作]；然后说明更改"`.
- **每次调用一个编辑方向。** 复合编辑在动作上会漂移。
- **`reference_image`仅在合理时使用**（包装替换、目标视觉的服装替换）。不要为一般重制传递参考。
- **`audio_setting: "origin"`** 对于不需要重新生成音轨的访谈视频。
- **源视频限制**: 2–10秒，≤100MB。
...
在用户想要将参考视频的动作**转移到目标角色上**时使用（由图像或另一个视频驱动）。这不是重制——它是带有身份保持的动作映射。
...
- **主体必须在图像参考中占帧的 > 5%** 以获得干净的身份保持。
- **空间限制有帮助**: `"角色在左侧，背景动作在右侧"`.
- **简化**如果结果在迭代之间漂移——删除形容词，保留核心动作描述。
- **`character_orientation: "image"`** 输出限制在10秒；`"video"` 允许30秒。

---

## 路线 3：Lucy Edit Restyle — 轻量级身份稳定的重制 / 服装交换

**模型**: `decart/lucy-edit/restyle`

在编辑是**局部风格修改**时使用——服装交换、场景重光、氛围重制——并且身份保留至关重要。比Wan 2.7 Edit轻量；限制在720p。
...
- **保留身份目标** — `"保留面部和动作"`就足够了；不要过度指定。
- **避免完全替换**（“宇航员在太空中”有效；“交换主体为不同的人”无效）。Lucy是用于局部风格修改的，不是完全角色交换。
- **无宽高比控制** — 输出匹配输入。如果预先不匹配，服务器端会进行裁剪。

---

## 限制

- **每个路线继承其模型的限制。** Wan 2.7 Edit: 2–10秒，1080p上限。Kling: 10秒（图像方向）或30秒（视频方向）。Lucy: 720p上限，无宽高控制。
...
该技能根据用户意图选择Wan 2.7 Edit-Video / Kling 2.6 Pro Motion Control / Lucy Edit Restyle中的一个，并调用`runcomfy run <model_id>`与匹配的JSON正文。CLI向模型API POST，轮询请求，获取结果，并将任何`.runcomfy.net`/`.runcomfy.com` URL下载到`--output-dir`。`Ctrl-C`在退出前取消远程请求。

## 安全与隐私

- **令牌存储**: `runcomfy login`将API令牌写入`~/.config/runcomfy/token.json`，权限为0600（仅所有者可读写）。设置`RUNCOMFY_TOKEN`环境变量以在CI / 容器中完全绕过文件。
- **输入边界**: 用户提示作为JSON字符串通过`--input`传递给CLI。CLI**不会**对提示进行shell扩展；它将JSON正文直接通过HTTPS传输到模型API。提示内容没有shell注入表面。
