...
通过一个CLI命令即可使用RunComfy的全视频模型目录生成视频——文本到视频、图像到视频，以及Veo的视频扩展。这项技能会根据用户意图选择合适的模型，并提供相应的文档提示模式+精确的`runcomfy run`调用命令。
...
# 2. 登录
runcomfy login              # 或者 在CI环境中: export RUNCOMFY_TOKEN=<token>
...
> 目前在Artificial Analysis Video Arena中排名第一。原生同步音频在处理过程中生成（无需单独的Foley步骤）。原生1080p，最长可达~15秒，强大的多镜头角色一致性。
> 选择用于：通用t2v、带音频的广告创意、社交媒体片段、多镜头叙事。
> 避免用于：音频驱动的特定配音MP3唇同步——使用**Wan 2-7**。
...
> Kling的最新版本，4K输出，强大的多镜头角色身份，高级摄像机语言。
> 选择用于：主角镜头、最终交付的4K剪辑、多镜头角色叙事。
> 避免用于：成本敏感的迭代——降级到**Kling 2-6 Pro**或**标准**i2v。

**Seedance v2 Pro** — `bytedance/seedance-v2/pro`
> 字节跳动旗舰产品——多模态（最多9张参考图像、3个参考视频、3个参考音频），处理过程中同步音频，电影级运动细化，镜头语言得到尊重。
> 选择用于：电影级广告帧、多参考组合（主体+场景+音频参考）、21:9变形宽银幕效果。
...
> Seedance v2 Pro的更快版本，具有相同的多模态功能。
> 选择用于：在Seedance v2组合上进行迭代，在锁定Pro版本之前。
> 避免用于：主角交付。

**Wan 2-7** — `wan-ai/wan-2-7/text-to-video`
> 开源权重旗舰产品，`audio_url`字段用于音频驱动的唇同步，原生与Wan图像模型配对。
> 选择用于：唇同步对话场景，必须与特定配音文件同步的嘴部；开源权重管道要求。
> 避免用于：处理过程中音频生成（无MP3输入）——使用**HappyHorse 1.0**。
...
> Kling的上一个级别——仍然具有比3.0 4K更低的成本，但质量仍然很强。
> 选择用于：大规模生产，其中3.0 4K过于昂贵。
...
> Seedance的上一个生成版本，更便宜。
> 选择用于：1-5代之间身份稳定的批量；成本敏感的基线。
...
> Google的旗舰产品——尊重物理的运动，强大的物体持久性（“旋转180度”=180°），与`extend-video`配对以生成更长的片段。
...
> MiniMax Hailuo的最新版本——自然运动，在真实世界主题上表现强劲。
> 选择用于：真实人物/真实产品主题的自然运动。
> 避免用于：风格化角色——使用Kling或Dreamina。
...
> 更旧的Seedance i2v生成版本，更便宜。
> 选择用于：在Seedance上进行成本敏感的批量i2v。
> 避免用于：新工作——Seedance v2 Pro功能更强（t2v + i2v + 多模态）。
