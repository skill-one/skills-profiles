...
- `--mode fast` 用于在~1s延迟预算内进行高质量搜索
- `--mode advanced` 用于更难的查询（多步骤、代理式搜索）。默认`basic`适用于几乎所有情况；只有当基本结果不足时才升级到`advanced`，对于高量的简单查找则降级到`turbo`
...
- 以关键答案/发现开头
- 包括具体事实、名称、数字、日期
- 每个事实都要内联引用为[来源标题](url)——不要留下任何未引用的声明
- 如果有多个主题，按主题组织
