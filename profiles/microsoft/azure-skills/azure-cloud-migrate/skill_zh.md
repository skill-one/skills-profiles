...
> 此技能处理将现有云工作负载迁移到 Azure 的**评估和代码迁移**。
...
5. 使用目标服务最新的支持运行时
6. 破坏性行动需要 `ask_user` — [functions global-rules](references/services/functions/global-rules.md) | [app-service global-rules](references/services/app-service/global-rules.md)
7. **向用户报告进度** — 在长时间运行的操作（部署、镜像推送）期间，提供资源级别的状态更新，以便用户不会在没有反馈的情况下等待 — 见 [workflow-details.md](references/workflow-details.md)
8. **审计应用代码中的服务发现** — Kubernetes DNS 名称（例如，`http://order-service:3001`）在容器应用中无法解析。在评估期间，扫描源代码中 HTTP 客户端中的硬编码主机名/端口，并标记它们以进行基于环境变量的 URL 注入
...
| Kubernetes (GKE/EKS/自托管) | Azure Container Apps | [k8s-to-container-apps.md](references/services/container-apps/k8s-to-container-apps.md) |
...
> 没有匹配的场景？使用 `mcp_azure_mcp_documentation` 和 `mcp_azure_mcp_get_azure_bestpractices` 工具。

## 输出目录

所有输出都位于工作区根目录下的 `<workspace-root-basename>-azure/`，其中 `<workspace-root-basename>` 是顶级工作区目录的名称本身（不是其子目录）。切勿修改源目录。

## 步骤

1. **创建** 工作区根目录下的 `<workspace-root-basename>-azure/`
2. **评估** — 分析源代码，映射服务，使用场景特定的评估指南生成报告 → [functions assessment](references/services/functions/assessment.md) | [app-service assessment](references/services/app-service/assessment.md)
3. **迁移** — 使用场景特定的迁移指南转换代码/配置 → [functions code-migration](references/services/functions/code-migration.md) | [app-service code-migration](references/services/app-service/code-migration.md)
4. **询问用户** — "迁移完成。本地测试还是部署到 Azure？"
5. **交接** 给 azure-prepare 进行基础设施、测试和部署

在 `migration-status.md` 中跟踪进度 — 见 [workflow-details.md](references/workflow-details.md)。
