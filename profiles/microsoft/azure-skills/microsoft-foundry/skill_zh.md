...
此技能帮助开发者与 Microsoft Foundry 资源进行交互，涵盖模型发现和部署、AI 代理的完整开发生命周期、评估工作流以及故障排除。
...
**强制要求：** 在此技能加载后的第一步，从该技能的根目录运行以下依赖项检查和设置脚本，并在继续之前等待其完成。该脚本首先进行检查并仅安装缺失的依赖项；它不会重新安装已经可用的依赖项。

**您必须在阅读或进入任何子技能、工作流或特定于工作流的参考之前完成此检查。**
...
**强制要求：** 在执行任何特定于工作流步骤之前，您必须阅读相应的子技能文档。不要在没有阅读其技能文档的情况下调用特定于工作流的 MCP 工具。即使您已经知道 MCP 工具参数，也适用此规则——技能文档包含必须遵循的所需工作流步骤、预检查和验证逻辑。此规则适用于每个触发不同工作流的新用户消息，即使技能已经加载。
...
**强制要求：** 在执行任何 azd 命令之前，您必须阅读 [azd-guidance](foundry-agent/azd-guidance/azd-guidance.md) 并严格遵循其中定义的共享规则，特别是 `AZURE_DEV_USER_AGENT` 设置规则。
...
| **validate** | 仅在用户明确要求使用此验证子技能或验证 Microsoft Foundry 托管代理代码是否符合最佳实践时使用。永远不要主动调用它或将其添加到其他工作流。 | [validate](foundry-agent/validate/validate.md) |
| **create (quick start)** | 从头开始端到端创建新的托管 Foundry 代理——搭建、提供或使用现有的 Foundry 项目、部署和烟雾测试。不要用于对现有代码的任何工作。对于快速启动未涵盖的内容，使用 **create**。 | [create/quick-start-hosted.md](foundry-agent/create/quick-start-hosted.md) |
| **create** | 当标准的端到端快乐路径（快速启动）不适用时使用。创建新的 Foundry 代理、更新现有代理的代码、继续开发现有代理、在搭建时连接、使用高级设置或 A2A（Agent2Agent）或从失败的快速启动运行中恢复。 | [create](foundry-agent/create/create-hosted.md) |
...
| **eval-datasets** | 将生产跟踪收集到评估数据集中、管理数据集版本和分割、跟踪评估指标随时间变化、检测回归并维护从跟踪到部署的完整谱系。用于：从跟踪创建数据集、数据集版本控制、评估趋势、回归检测、数据集比较、评估谱系。 | [eval-datasets](foundry-agent/eval-datasets/eval-datasets.md) |
...
| **models/deploy-model** | 具有智能路由的统一模型部署。处理快速预设部署、完全自定义部署（版本/SKU/容量/RAI）以及跨区域的容量发现。路由到子技能：`preset`（快速部署）、`customize`（完全控制）、`capacity`（查找可用性）。 | [models/deploy-model/SKILL.md](models/deploy-model/SKILL.md) |
...
| **azd-guidance** | 提供有关管理 Foundry 代理的共享 azd 知识和指导。对于任何与 azd 相关的工作流，请首先阅读此内容。 | [azd-guidance](foundry-agent/azd-guidance/azd-guidance.md) |
...
| 用户意图 | 工作流（按顺序阅读） |
|-------------|------------------------|
| 端到端创建新的托管代理（搭建 + 部署 + 测试） | [dependency check and setup](#dependency-check-and-setup) → [azd-guidance](foundry-agent/azd-guidance/azd-guidance.md) → [quick-start-hosted](foundry-agent/create/quick-start-hosted.md) (自包含端到端) |
| 标准快速启动之外的内容（现有代码、迁移、重新托管、部署自定义、搭建时连接、A2A（Agent2Agent）、恢复） | [dependency check and setup](#dependency-check-and-setup) → [azd-guidance](foundry-agent/azd-guidance/azd-guidance.md) → [create](foundry-agent/create/create-hosted.md) → [deploy](foundry-agent/deploy/deploy.md) → [invoke](foundry-agent/invoke/invoke.md) |
| 优化现有的 Python 托管代理 | [dependency check and setup](#dependency-check-and-setup) → [azd-guidance](foundry-agent/azd-guidance/azd-guidance.md) → [agent-optimizer](foundry-agent/agent-optimizer/agent-optimizer.md) → 搭建/审查 → eval.yaml → 优化 → 应用候选者 → 部署 → invoke |
...
| 代码更改后重新部署/重新部署代理 | [dependency check and setup](#dependency-check-and-setup) → [azd-guidance](foundry-agent/azd-guidance/azd-guidance.md) → 部署（包括 eval-suite 设置）→ invoke → observe (评估/优化) |
...
| 与代理进行调用/测试/聊天 | [dependency check and setup](#dependency-check-and-setup) → [azd-guidance](foundry-agent/azd-guidance/azd-guidance.md) → invoke |
...
- [标准代理设置](references/standard-agent-setup.md) — 高级设置，适用于需要数据驻留控制的生产行业（自带 Cosmos DB / 存储 / AI 搜索通过 Foundry 能力主机）。默认的 `azd ai agent` 流使用 **基本代理设置**，并且**不**提供 `capabilityHosts/agents` — 不要将其不存在标记为错误。对于默认的后期提供状态，请参阅 [foundry-agent/create/create-hosted.md](foundry-agent/create/create-hosted.md) 中的“Expected env-var fingerprint”部分。

## 代理：通用项目上下文解析

代理技能应仅在它们需要配置值而尚未拥有时运行此步骤。如果某个值（例如代理根目录、环境、项目端点或代理名称）已经从用户消息或同一会话中的先前技能中知道，请跳过该值的解析。

### 步骤 1：发现代理根目录和 azd 上下文

首先检查工作区是否具有使用 `host: azure.ai.agent` 的 `azure.yaml`。

- **一个 azd 代理服务** -> 使用该服务的 `project` 文件夹作为代理根目录。
- **多个 azd 代理服务** -> 要求用户选择目标服务/文件夹。
- **没有 azd 代理服务** -> 搜索工作区中包含 `agent-metadata.yaml` 或 `agent-metadata.<env>.yaml` 的 `.foundry/` 文件夹。
  - **一个匹配** -> 使用该代理根目录。
  - **多个匹配** -> 要求用户选择目标代理文件夹。
  - **没有匹配** -> 对于 [insights](foundry-agent/insights/insights.md)，仅请求缺失的远程输入，而不初始化本地文件；对于 create/deploy 工作流，在设置期间种子一个新的 `.foundry/` 文件夹；对于其他工作流，停止并询问用户要初始化哪个代理源文件夹。

选择代理根目录后，仅在该文件夹内保留所有本地 `.foundry` 缓存检查、源检查、评估器建议、数据集建议和提示优化上下文。**不要**扫描兄弟代理文件夹，除非用户明确切换根目录。
...
1. 用户明确命名的环境
2. 从 `azd env get-values` 的 `AZURE_ENV_NAME`
3. 从 `.azure/config.json` 的 azd 默认环境
4. 会话中先前选择的 环境

当项目/部署值尚不知道时，运行 `azd env get-values` 以选择的环境。优先使用 azd 值进行部署上下文：
...
当 azd 提供这些值时，将其用作事实来源，并且在元数据写入时**不要**将它们复制到 `.foundry/agent-metadata*.yaml`。

### 步骤 3：选择元数据覆盖并解析环境

在选定的代理根目录中，按以下顺序选择元数据文件：
...
3. 会话中先前选择的 环境
4. 从元数据中的 `defaultEnvironment`

如果选定的元数据文件仍然包含多个环境并且上述规则未选择任何文件，提示用户选择。在每个工作流摘要中保留选定的代理根目录、元数据文件、环境以及上下文是否来自 azd 或元数据。

如果选定的环境暴露了较旧的 `testSuites[]` 元数据但没有 `evaluationSuites[]`，则将 `testSuites[]` 视为此会话的源，并在内存中规范化每个条目以匹配 `evaluationSuites[]` 形状，然后继续。如果元数据仍然较旧并且仅暴露了遗留的 `testCases[]`，则以相同方式规范化该列表。保留数据集和评估器字段，保留任何现有的 `tags`，并且仅在 `tags.tier` 缺失时将遗留的 `priority` 映射到 `tags.tier`：`P0` -> `smoke`, `P1` -> `regression`, `P2` -> `coverage`。
...
- `evaluators[]` -> Foundry 评估器名称候选；在使用它们作为远程评估器之前，请与 `evaluator_catalog_get` 进行验证。
...
如果 azd 和元数据都提供相同的值并且它们不同，请停止并询问哪个来源具有权威性。如果它们匹配，请使用 azd 值，并且在未来的元数据写入中避免重写重复的值。
...
- `evaluationSuites[]` 带有远程套件/数据集/评估器引用和本地缓存路径
- `lastEval`, 结果文件, 比较摘要或显式的非 azd 覆盖

当 azd 已经提供它们时，不要将 azd 拥有的部署值复制到元数据。如果选定的文件是首选的单环境文件，请仅重写该环境块。如果选定的文件是遗留的多环境文件，请仅重写选定的环境块。永远不要自动复制或合并兄弟元数据文件中的环境。如果选定的环境仍然使用较旧的 `testSuites[]` 或遗留的 `testCases[]`，请将其重写为 `evaluationSuites[]` 并从重写的条目中删除已迁移的 `priority` 字段。

### 步骤 7：收集缺失的值

仅对于从用户消息、会话上下文、元数据或 azd 引导未解析的值，使用 `ask_user` 或 `askQuestions` 工具。常见值技能可能需要的值：
- **代理根目录** — 目标 azd 服务项目文件夹或包含 `.foundry/agent-metadata*.yaml` 的文件夹
- **元数据文件** — 本地/开发用的 `agent-metadata.yaml`，或显式的边车文件，例如 `agent-metadata.prod.yaml`
