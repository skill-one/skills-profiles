...
让 repo 中的每个 package 成为 **深模块**：用小的 interface 隐藏大量的 behaviour。Package 的 public surface 是其 **入口点**（package root 中的 files），所有 subfolders 都被隐藏。这个 skill 会安装 [dependency-cruiser](https://github.com/sverweij/dependency-cruiser)，加入强制只能通过入口点访问的 rules，并证明这些 rules 确实会拦截违规。

Vocabulary（深模块、interface、seam、depth）来自调用 Skill 工具并指定 `codebase-design`；整个过程都使用它的语言。
...
src/packages/
  <name>/
    index.ts        ← 一个入口点（public）。从外部导入这个。
    client.ts       ← 另一个入口点。Packages 可能暴露多个。
    lib/            ← 实现：对外隐藏，可以互相导入。
    tests/          ← 同一位置的 tests + fixtures（一个 subfolder，所以是私有的）。
```

Public surface 是 package 的 **root files**，而不是指定的单个 `index.ts`。按 convention，implementation 放在 `lib/`，tests 放在 `tests/`，使所有 packages 采用相同的 two-folder shape。Rule 本身是通用的：*任何* subfolder 中的*任何*内容都是 private，因此永远无需为了新增 folder 扩展 config。

四条 rules，全部为 `error`：

1. **入口点边界** — package 外的 code（app code 或其他 package）只能 import 该 package 的入口点（root files），不能 import subfolder 中的任何内容。
2. **包内自由** — package 自己的 files 可以自由互相 import。
3. **通过入口点运行 tests** — `<pkg>/tests/` 下的 files 可以 import 任意 package 的入口点和自己的 `tests/` fixtures，但不能 import 任何 package 的 subfolder internals（包括自己的）。允许跨 package integration tests，不允许 deep imports。
4. **无循环依赖** — 不允许 dependency cycles。

**入口点，不是一个桶。** Public surface 是每个 root file，因此 package 可以提供多个小入口点（`index.ts`、`client.ts`、`server.ts`），不必把一切汇入巨大的 `index.ts`。不鼓励 re-export 整个 subtree 的 barrel files；entry points 要小，implementation 隐藏在 subfolders。
...
- **包管理器** — `pnpm-lock.yaml` → pnpm，`yarn.lock` → yarn，`bun.lockb` → bun，否则 npm。后续每条 command 都使用它（`pnpm`/`yarn`/`npm run`/`bunx`）。
- **Packages root** — 存在 `src/` 就用 `src/packages`，否则用 `packages`。如果 repo 已有明显不同的 convention，与用户确认。
- **现有 config** — 检查 `.dependency-cruiser.*` file。若存在，不要覆盖；merge 四条 rules 和 options，并说明添加了什么。

**完成时：** package manager、packages root 和 existing-config status 全部明确。

### 2. 安装 dependency-cruiser

使用检测到的 package manager，把 `dependency-cruiser` 安装为 devDependency。

**完成时：** `dependency-cruiser` 出现在 `devDependencies`。
...
**完成时：** `.dependency-cruiser.cjs` 存在、`PACKAGES_ROOT` 正确，并包含四条 forbidden rules。
...
**完成时：** `lint:boundaries` 存在，且和 typecheck 由同一 command 运行。
...
**完成时：** example package 存在，通过 root entry point 暴露 behaviour，并把 `impl` 隐藏在 subfolder。
...
**完成时：** 已观察到 pass、deep import 时 fail、恢复后再 pass。Step 2 不失败，就先修正 wiring，不能完成任务。
...
**完成时：** `<packages-root>/README.md` 存在且 discourages barrels，repo 的 `CLAUDE.md`/`AGENTS.md` 链接到它。
