# 设置预提交钩子

## 这会设置的内容

- **Husky** 预提交钩子
- **lint-staged** 对所有已暂存文件运行 Prettier
- **Prettier** 配置（如果缺失）
- 预提交钩子中的 **typecheck** 和 **test** 脚本

## 步骤

### 1. 检测包管理器

检查 `package-lock.json` (npm)、`pnpm-lock.yaml` (pnpm)、`yarn.lock` (yarn)、`bun.lockb` (bun)。使用存在的那个。不清楚时默认 npm。

### 2. 安装依赖

作为 devDependencies 安装：

```
husky lint-staged prettier
```

### 3. 初始化 Husky

```bash
npx husky init
```

这会创建 `.husky/` 目录，并向 package.json 添加 `prepare: "husky"`。

### 4. 创建 `.husky/pre-commit`

写入这个文件（Husky v9+ 不需要 shebang）：

```
npx lint-staged
npm run typecheck
npm run test
```

**调整**：把 `npm` 替换为检测到的包管理器。如果 repo 的 package.json 没有 `typecheck` 或 `test` script，就省略对应行并告知用户。

### 5. 创建 `.lintstagedrc`

```json
{
  "*": "prettier --ignore-unknown --write"
}
```

### 6. 创建 `.prettierrc`（如果缺失）

只有没有 Prettier 配置时才创建。使用这些默认值：

```json
{
  "useTabs": false,
  "tabWidth": 2,
  "printWidth": 80,
  "singleQuote": false,
  "trailingComma": "es5",
  "semi": true,
  "arrowParens": "always"
}
```

### 7. 验证

- [ ] `.husky/pre-commit` 存在且可执行
- [ ] `.lintstagedrc` 存在
- [ ] package.json 中的 `prepare` script 是 `"husky"`
- [ ] prettier 配置存在
- [ ] 运行 `npx lint-staged` 验证可用

### 8. 提交

暂存所有已更改/创建的文件，并用 message 提交：`Add pre-commit hooks (husky + lint-staged + prettier)`

这会经过新的预提交钩子，是一个不错的 smoke test。

## 注意事项

- Husky v9+ 不需要钩子文件中的 shebangs
- `prettier --ignore-unknown` 会跳过 Prettier 无法解析的文件（如图片等）
- 预提交先运行 lint-staged（快，只针对已暂存文件），再运行完整的 typecheck 和 tests
