...
1. **失败的测试**，放在能触达 bug 的 seam 上：unit、integration、e2e 都可以。
2. **Curl / HTTP 脚本**，打到运行中的 dev server。
3. **CLI 调用**，使用 fixture input，并把 stdout 与 known-good snapshot diff。
4. **无头浏览器脚本**（Playwright / Puppeteer），驱动 UI，并断言 DOM/console/network。
5. **重放捕获的 trace。** 把真实 network request / payload / event log 保存到磁盘，并在隔离环境中 replay 到代码路径。
6. **一次性 harness。** 启动系统的最小子集（一个 service、mocked deps），用一次 function call 触发 bug code path。
7. **属性 / 混淆循环。** 如果 bug 是 "sometimes wrong output"，运行 1000 个 random inputs 并寻找 failure mode。
8. **二分法 harness。** 如果 bug 出现在两个已知状态之间（commit、dataset、version），自动化 "boot at state X, check, repeat"，这样可以 `git bisect run`。
9. **差异循环。** 用同一 input 跑 old-version vs new-version（或两个 configs），然后 diff outputs。
10. **HITL bash 脚本。** 最后手段。如果必须由人点击，就用 `scripts/hitl-loop.template.sh` 驱动 _人_，让 loop 仍保持结构化。捕获的输出反馈给你。
...
1. **Debugger / REPL 检查**，如果环境支持。一个 breakpoint 胜过十条 logs。
