...
*   **如果执行类型特定行为（代数数据类型）：** 使用Object模式与`sealed`类结合以确保完备性。
*   **如果匹配数值范围或条件：** 在switch臂中使用关系（`>=`，`<=`）和逻辑与（`&&`）模式。
*   **如果多个情况共享逻辑：** 使用逻辑或（`||`）模式共享单个情况体或守卫子句。
*   **如果忽略特定值：** 在集合中使用通配符模式（`_`）或非匹配的Rest元素（`...`）。
...
*   **如果执行语句或副作用：** 使用**switch语句**。
    ...
    *   规则：空情况向下传递到下一个情况。非空情况隐式中断（不需要`break`关键字）。
...
*   **关系：** `==`，`!=`，`<`，`>`，`<=`，`>=`后跟一个常量表达式。
*   **强制类型转换（`as`）：** `pattern as Type`。如果值不匹配类型则抛出异常。用于在解构期间强制断言类型。
...
*   **记录：** `(pattern1, named: pattern2)`。匹配形状精确的记录。使用`:var name`推断getter名称。
*   **对象：** `ClassName(field: pattern)`。匹配`ClassName`的实例。使用`:var field`推断getter名称。
...
### 反馈循环1：完备性检查（静态验证）
当在sealed类或枚举上切换时，确保在编译时处理所有子类型：

1. **运行分析器：** 执行`dart analyze`。
2. **查看错误：** 查找“类型'X'未被switch情况完备匹配”或不可达模式臂警告。
...
2. **快速失败回退：** 确保未匹配或格式错误的映射结构命中显式`_ => throw FormatException(...)`臂，而不是静默失败`if-case`检查。

## 示例

### 多态JSON反序列化（有区分的联合）
使用Map模式与switch表达式验证标记JSON有效负载并构造`sealed`类层次结构。有关可执行实现示例，请参阅
