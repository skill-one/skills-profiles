...
*   **错误：** 绝不显式捕获 `Error` 或其子类型（例如，`TypeError`，`ArgumentError`）。错误表示必须修复的编程错误，而不是捕获。通过启用 `avoid_catching_errors` linter 规则来强制执行此操作。
...
使用此顺序工作流来识别、修复和验证 Dart 项目中的静态分析错误。将检查清单复制到跟踪您的进度。
...
**2. 应用自动修复**
使用 `dart fix` 工具自动解决标准的 linter 和分析问题。
```bash
# 预览更改
dart fix --dry-run
# 应用更改
