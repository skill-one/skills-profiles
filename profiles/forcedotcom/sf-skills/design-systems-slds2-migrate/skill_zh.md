...
使用 SLDS linter 和结构化的指导，将 Lightning Web Components 从 SLDS 1 系统地迁移到 SLDS 2，并修复所有样式钩类别中的违规行为。
...
颜色钩需要最多的判断（上下文相关的选择）。非颜色钩大多是编号刻度，映射关系简单。

## 前置条件

- 安装 Node.js 14.x 或更高版本
- 可以访问组件 CSS 和标记文件（`.html` 用于 LWC，`.cmp` 用于 Aura）
- 可以通过终端/命令行运行 linter
- Git 仓库用于备份（推荐）
...
componentName.css
  ...
  23:5  错误    '--lwc-colorBackground' 设计令牌已弃用。用 SLDS 2 样式钩替换它，并将后备设置为 '--lwc-colorBackground'。
  ...
  31:15  错误   考虑删除 t(fontSizeMedium) 或用 var(--slds-g-font-size-base, var(--lwc-fontSizeMedium, 0.8125rem)) 替换它。
                 将后备设置为 t(fontSizeMedium)。更多信息，请参阅 lightningdesignsystem.com 上的样式钩。               slds/no-deprecated-tokens-slds1
```

四种违规类型，每种类型都有其自己的修复方法（见第 3 步）。

**重要提示：** linter 标记所有硬编码值。修复颜色、间距、尺寸、字体、边框和阴影值 — 但**跳过布局值**（`100%`、`auto`、`0`、`inherit`、`none`）。参见 [rule-no-hardcoded-values.md](references/rule-no-hardcoded-values.md) 以获取完整的修复与跳过决策表。
...
| 违规规则 | 简要概述 | 参考 |
|---|---|---|
| `slds/no-hardcoded-values-slds2` | 替换硬编码值为 SLDS 钩 + 原始值作为后备 | [rule-no-hardcoded-values.md](references/rule-no-hardcoded-values.md)|
| `slds/lwc-token-to-slds-hook` | 用 SLDS 2 钩替换 `--lwc-*` 令牌，保留 LWC 令牌作为后备 | [rule-lwc-token-to-slds-hook.md](references/rule-lwc-token-to-slds-hook.md) |
...
| `slds/no-deprecated-tokens-slds1` | 用 SLDS 2 钩 + LWC 后备替换旧的 `t()`/`token()` 语法 | [rule-no-deprecated-tokens-slds1.md](references/rule-no-deprecated-tokens-slds1.md) |

**始终包含后备值** — `var(--slds-g-hook, originalValue)`，其中 `originalValue` 是源 CSS 中的原始值。

### 类重写快速参考

类重写需要更改 CSS 和标记（`.html` 或 `.cmp`）。这是最常被遗漏的步骤：
...
颜色钩需要上下文相关的选择。**必须：当任何违规行为涉及颜色属性（`color`、`background-color`、`background`、`fill`、`border-color`），您必须在选择钩之前阅读 [color-hooks-decision-guide.md](references/color-hooks-decision-guide.md)。** linter 以任意顺序列出可能的钩 — 不要选择第一个建议。该指南包含基于属性的规则，这些规则决定了正确的钩。

非颜色钩更简单 — 将 CSS 值与编号刻度匹配。参见 **[non-color-hooks-decision-guide.md](references/non-color-hooks-decision-guide.md)** 以获取值到钩查找表，涵盖间距、尺寸、字体、边框、半径和阴影。
...
返回完全迁移的 CSS（以及修复了类重写的更新 HTML 标记）并确保零 SLDS linter 违规。所有样式钩都必须包含后备值以保留原始 CSS 值。
...
使用不透明的 `rgb()` 作为后备（不是 `rgba()`）— `color-mix` 处理透明度。

## calc() 表达式与令牌

当迁移 `t('calc(...)')` 或带已弃用令牌的 `calc()`：
...
- **仅编号刻度** — 命名钩如 `spacing-medium`、`font-weight-bold`、`radius-large` 不存在
...
| 迁移后外观发生变化 | 验证后备值与原始值匹配；检查表面与容器系列 |
...
| 间距/尺寸不匹配 | 检查值到钩映射在 non-color-hooks-decision-guide.md 中；验证间距与尺寸使用 |
| 命名钩不起作用（例如，`spacing-medium`） | 命名钩不存在 — 使用编号刻度：`spacing-4` 用于 16px，`font-weight-7` 用于内联粗体强调（非标题） |
| 组件在紧凑密度中看起来不同 | 使用密度感知钩（`--slds-g-spacing-var-*`）用于适应密度的组件 |
...
- **[规则：无 SLDS 类重写](references/rule-no-slds-class-overrides.md)** — 类重命名和 HTML 更新
- **[迁移示例](references/examples.md)** — 按场景和复杂性的前后示例
- **[常见模式](references/common-patterns.md)** — 永远不要重写的类、已弃用的 SLDS2 类、调色板后备、没有 SLDS2 等效的令牌
