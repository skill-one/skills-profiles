...
- 用户想知道哪些移动原生 API 是可用的，或哪个适合他们的功能。

**不要**使用此技能用于：

- LWC 的移动离线审查（lwc:if、内联 GraphQL、Komaci-priming 违规） — 使用 `mobile-platform-offline-validate`。
- 选择或样式化通用 Lightning Base 组件 / SLDS 蓝图 — 使用 `design-systems-slds-apply`。

## 前置条件

- 知道 LWC 将在受支持的移动容器（Salesforce 移动应用、现场服务移动应用）中运行。这些功能在桌面和移动网页上不可用；在每次调用后使用 `isAvailable()` 进行门控。
...
| 联系人 | [联系人](references/contacts.md) | 读取或创建设备通讯录中的条目。 |
...
每个会话中只读取这两个共享引用一次 — 它们适用于每个功能，并且不在每个功能的文件中重复：
...
3. 在 `isAvailable()` 后面进行门控：
   ...
   ```
4. 调用**非弃用的**入口点。几个服务保留旧方法标记为 `@deprecated`，与推荐方法一起使用 — 始终优先考虑参考中的推荐方法。
...
每个服务定义了自己的失败代码枚举。将代码转换为用户可操作的消息：`USER_DENIED_PERMISSION` 应提示用户授予权限；`USER_DISABLED_PERMISSION` 必须引导他们到操作系统设置；`SERVICE_NOT_ENABLED` 应该是开发者可见的错误，不应向用户显示。

### 第 5 步 — 确保在受支持的表面上运行

移动功能仅在 LWC 在受支持的 Salesforce 移动应用中运行时可用。如果相同的组件在桌面或移动网页上渲染，工厂仍然会返回一个对象，但 `isAvailable()` 将返回 `false`。永远不要假设可用性 — 每次调用都要进行门控。
...
4. 在选项中，将 `barcodeTypes` 设置为所需的符号（默认为所有支持类型）并设置 `enableMultiScan: false` 以进行单次读取。
...
3. 在 `isAvailable()` 上进行门控。
4. 根据参考构建支付请求对象。
5. 在解决时，将交易 ID 展示给调用流程。在拒绝时，分别处理用户取消和支付失败路径。
...
- **`isAvailable()` 在真实设备上返回 `false`** — 设备正在运行不受支持的应用表面（不是 Salesforce 移动应用或现场服务移动应用），或者服务被组织级别的设置门控。修复是组织配置，而不是代码。
- **TypeScript 找不到导入** — 确认 LWC 可以访问 `lightning/mobileCapabilities`。该模块在 Salesforce 移动容器内全局声明；在外部，类型必须单独安装。
