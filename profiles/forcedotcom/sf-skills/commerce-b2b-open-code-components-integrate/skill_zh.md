...
- 在Experience Builder中提供开源代码组件

## 规则

1. **执行前必须解释。** 在运行任何命令之前，你必须告诉用户该命令的作用以及你运行它的原因。不要只显示原始命令并请求权限。用户应该能够阅读你的解释并理解目的，然后再批准。

## 概述

此技能将官方Salesforce存储库（https://github.com/forcedotcom/b2b-commerce-open-source-components）中的所有开源B2B Commerce组件复制到B2B Commerce商店的站点元数据中。集成后，这些组件将出现在Experience Builder组件调色板中。
...
读取`sfdx-project.json`并选择活动包目录。提取`packageDirectories[]`并使用标记为默认的条目；如果没有条目被标记为默认，则使用第一个条目。将此值作为下方所有地方的`<package-dir>`使用。如果`sfdx-project.json`丢失或没有`packageDirectories`，则告诉用户并中止。

### 检查1：开源存储库

验证存储库是否克隆在`.tmp/b2b-commerce-open-source-components`：

1. **如果目录不存在：** 告诉用户：“我正在从GitHub将官方B2B Commerce开源组件存储库克隆到本地`.tmp/`文件夹中。这使我们能够访问所有开源代码组件。”
   然后，运行：`git clone https://github.com/forcedotcom/b2b-commerce-open-source-components .tmp/b2b-commerce-open-source-components`
...
1. 告诉用户：“我正在检查您的项目是否已经具有B2B商店元数据本地。”
   检查`<package-dir>/main/default/digitalExperiences/site/`是否包含任何商店目录。
2. **如果商店元数据存在：** 使用它。如果找到多个商店，请要求用户选择一个。
3. **如果未找到商店元数据：** 在委托之前尝试从连接的org中检索：
   1. 运行`sf org list`（或检查`sf config get target-org`）以找到连接的org。要求用户确认或选择一个，如果有多个。
   ...
   4. **只有当没有连接的org可用，或者没有站点包，或者检索失败时：** 委托给**commerce-b2b-store-create**技能。

**所有检查后的必要状态：**
- **包目录** — 在检查0中解析的值（例如，`force-app`）
...
- **目标：** `<package-dir>/main/default/digitalExperiences/site/<store-name>/sfdc_cms__lwc/`和`sfdc_cms__label/`（在检查0中解析的`<package-dir>`）
...
3. 告诉用户：“我现在正在将所有开源代码LWC组件从克隆的存储库复制到您的商店的站点元数据目录中。”
   将所有组件目录从源复制到目标。
...
**代理：** “我现在正在将所有开源代码LWC组件从克隆的存储库复制到您的商店的站点元数据目录中……”
...
已复制：45个组件和38个标签集到 My_B2B_Store1
...
| 存储库结构无效 | “存储库结构已更改。预期sfdc_cms__lwc和sfdc_cms__label。” | 警告用户，中止 |
...
- [ ] 组件复制到正确目标路径（`sfdc_cms__lwc/`）
- [ ] 标签复制到正确目标路径（`sfdc_cms__label/`）
- [ ] 复制过程中没有文件权限错误
- [ ] 提供部署命令并告知用户测试
