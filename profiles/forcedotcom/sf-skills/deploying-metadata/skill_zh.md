# 部署元数据：全面的 Salesforce DevOps 自动化

在用户需要**部署编排**时使用此技能：干运行验证、目标或基于清单的部署、CI/CD 工作流建议、沙盒组织管理、故障排查或 Salesforce 元数据的安全生产顺序。

## 此技能负责任务的情况

当工作涉及以下内容时，使用 `deploying-metadata`：
...
- 编写 Apex 代码 → [generating-apex](../generating-apex/SKILL.md)
- 编写 LWC 组件 → [generating-lwc-components](../generating-lwc-components/SKILL.md)
- 创建自定义对象或字段 → [generating-custom-object](../generating-custom-object/SKILL.md)、[generating-custom-field](../generating-custom-field/SKILL.md)
- 构建 Flow → [generating-flow](../generating-flow/SKILL.md)
- 执行组织数据操作 → [handling-sf-data](../handling-sf-data/SKILL.md)
- 编写或测试 Agentforce 代理 → [developing-agentforce](../developing-agentforce/SKILL.md)
...
- 优先在真实部署前使用 **`--dry-run`**。
- 对于 Flow，安全部署并在验证后激活。
- 在元数据验证或部署后，将测试数据创建指导委托给 **`handling-sf-data`**。
...
1. 立即部署
2. 分配权限集
3. 通过 [handling-sf-data](../handling-sf-data/SKILL.md) 创建测试数据
4. 运行测试 / 烟雾测试
5. 按顺序编排多个部署后步骤
...
- 与现代 Apex 测试注解（如 `@IsTest(testFor=...)` 和 `@IsTest(isCritical=true)`）配合使用 — 参见 [generating-apex](../generating-apex/SKILL.md) 获取编写指导。
...
- [developing-agentforce](../developing-agentforce/SKILL.md) 用于 `.agent` 编写、Agent Builder、Prompt Builder 和元数据配置
...
| 需求 | 委托给 | 原因 |
|---|---|---|
| 自定义对象创建 | [generating-custom-object](../generating-custom-object/SKILL.md) | 部署前定义对象 |
| 自定义字段创建 | [generating-custom-field](../generating-custom-field/SKILL.md) | 部署前定义字段 |
| Apex 编写/修复 | [generating-apex](../generating-apex/SKILL.md) | 代码编写和修复 |
| Flow 创建/修复 | [generating-flow](../generating-flow/SKILL.md) | Flow 编写和激活指导 |
| 测试数据或种子记录 | [handling-sf-data](../handling-sf-data/SKILL.md) | 描述优先数据设置和清理 |
| 代理编写和发布准备 | [developing-agentforce](../developing-agentforce/SKILL.md) | 代理特定正确性 |
