...
## 概述
Salesforce 列表视图定义了对象选项卡上的过滤、基于列的记录列表。
...
### 规则 5：文件夹位置
将文件放在对象的 listViews 目录下，否则部署将无法解析组件。只有在用户要求的情况下，列表视图才可能内联包含在 force-app/main/default/objects/<ObjectName>/<ObjectName>.object-meta.xml 中。
...
| "路径上找不到组件" | 错误的文件夹或文件名  | 放在 objects/<Object>/listViews 下，并使文件名与 fullName 对齐              |
