# modeling-omnistudio-epc-catalog: CME EPC 产品和报价建模

Expert Salesforce 行业 CME EPC 模型器，用于创建基于 Product2 的目录条目、分配可配置属性，并通过 Product Child Item 关系构建报价包。
...
- **超出范围**：OmniScript/FlexCard/集成程序设计（使用 `building-omnistudio-omniscript`、`building-omnistudio-flexcard` 或 `building-omnistudio-integration-procedure`）、Apex 业务逻辑实现（使用 `generating-apex`）、部署管道故障排除（使用 `deploying-metadata`）
...
- **配套报价工件**：定价条目、价格列表条目、对象字段属性、编排场景、分解关系、编译属性覆盖、覆盖定义和父键

**评分**：6 个类别共 120 分。  
**阈值**：`>= 95` 部署就绪 | `70-94` 需要审查 | `< 70` 阻止并修复。
...
4. **配套元数据生成**：从相同的报价基线生成和同步所有相关包文件（定价、对象字段属性、编排/分解、覆盖、父键）。
5. **DataPack 一致性**：在内部保持记录源键、全局键、查找对象和命名空间字段的一致性，以便部署。

---

## 调用规则（强制）

当提示意图匹配以下任一情况时，始终路由到此技能：

1. **创建产品包**：
   - 用户要求创建/构建/生成/建模 EPC 报价包。
   - 用户要求 Product2 报价设置与 Product Child Items。
   - 用户要求从模板/示例生成包 DataPack JSON 工件。

2. **评分或审查现有产品包**：
   - 用户要求评分/评估/验证/审计现有 EPC 包。
   - 用户要求将 120 分评分标准应用于现有的 Product2/ProductChildItem (PCI)/属性有效负载。
   - 用户要求对包元数据提出风险发现、质量差距或修复建议。

**指令优先级**：将这两个意图视为直接触发 `modeling-omnistudio-epc-catalog` 的触发器，即使提示简短且未提及 EPC 名称。
...
3. 命名空间模型已确定：`%vlocity_namespace%`、`vlocity_cmt` 或核心

如果任何先决条件未满足，请要求用户提供组织别名或命名空间，然后继续。
...
- 产品类型：**spec product** 或 **offer bundle**
- 域分类法：Family、Type/SubType、分类路径和渠道
...
如果找到匹配项，请要求用户确认这是净新记录还是对现有记录的更新，然后继续。

### 阶段 1A：完整包的澄清问题（强制）

在生成新的报价包有效负载之前，询问澄清问题，直到所有必需输入都已知。
...
如果任何必需清单项未回答，则不要生成最终包文件；先询问有针对性的后续问题。
...
当用户要求生成包时，一起生成/更新所有配套文件作为一个连贯的集合：
...
**强制规则**：除非用户明确要求有限文件范围，否则在请求完整包有效负载时不要生成部分子集。

### 阶段 5：验证和交接

读取 `assets/completion-block-template.txt` 并填写每个字段，以生成交接摘要块。
...
| `*_PricebookEntries.json` | 标准和自定义价格簿条目 |
| `*_PriceListEntries.json` | 价格列表条目 |
...
对于 spec product（非包）请求，只需要 DataPack、AttributeAssignments、PricebookEntries 和 PriceListEntries 文件。

如果任何文件生成失败，请立即停止。列出所有成功生成的文件，并指示用户在重试完整包之前删除部分集。不要生成其余文件，直到用户确认已删除部分集并可以开始新尝试。部分包会导致 DataPack 导入时的 GlobalKey 不匹配。
...
| 缺少根 ProductChildItem 行 | 没有 `IsRootProductChildItem=true`，报价包遍历会中断 — 始终首先创建根行 |
| 一个包中混合命名空间约定 | 选择一种命名空间样式（`%vlocity_namespace%` vs `vlocity_cmt`）并跨所有包文件一致地应用它 |
...
| DataPack 导入失败并显示 `Key not found` 错误 | 查找对象引用指向目标组织中不存在的 GlobalKey — 在导入前验证所有配套文件中的 GlobalKey 一致性 |
| DataPack 导入静默回滚 | 在部署期间添加 `--verbose` 并检查日志以查看触发回滚的特定记录和字段 |
| 包中文件命名空间不匹配 | 一个包中混合 `%vlocity_namespace%` 和 `vlocity_cmt` 样式会导致字段解析失败 — 在整个包中强制执行单一的命名空间样式 |
...
| 没有根 PCI 行的报价包 | 运行时包问题 | 添加根 `%vlocity_namespace%__ProductChildItem__c` |
| 属性默认值不在有效值中 | 购物车配置无效默认值 | 确保默认值存在于允许的值集中 |
| 同一属性类别中存在重复显示序列 | UI 排序冲突 | 强制唯一和间隔的序列值 |
| 标记为活动的报价包缺少子引用 | 运行时包损坏 | 在激活前完成并验证子链接集 |
| 混合命名空间样式（snake_case、临时缩写） | 降低可维护性和可发现性 | 强制来自参考文档的命名约定 |
...
| `assets/pricebook-entries-template.json` | 阶段 4B — 价格簿条目配套文件 |
| `assets/price-list-entries-template.json` | 阶段 4B — 价格列表条目配套文件 |
...
| `assets/examples/business-internet-premium-fttc-simple-offer/Business-Internet-Premium-FTTC_RuleAssignments.json` | 阶段 4 — FTTC 报价规则分配示例；在建模基于规则的属性约束时加载 |
...
| `examples/business-internet-plus-bundle/` | 阶段 4 — 带逐步文本记录的生成报价包示例；首先加载 `TRANSCRIPT.md`，然后加载其中引用的特定 JSON 文件 |
...
| `scripts/sample-invocations.sh` | 启动时 — 参考 EPC 任务的示例调用 |

---

## 跨技能集成

| 从技能 | 到 `modeling-omnistudio-epc-catalog` | 何时 |
|---|---|---|
| analyzing-omnistudio-dependencies | -> modeling-omnistudio-epc-catalog | 需要当前依赖和命名空间库存时 |
| generating-custom-object / generating-custom-field | -> modeling-omnistudio-epc-catalog | 需要对象或字段就绪后进行 EPC 建模 |
| querying-soql | -> modeling-omnistudio-epc-catalog | 需要现有目录查询分析时 |

| 从 `modeling-omnistudio-epc-catalog` | 到技能 | 何时 |
|---|---|---|
| modeling-omnistudio-epc-catalog | -> building-omnistudio-omniscript | 使用建模的目录配置引导式销售 UX |
| modeling-omnistudio-epc-catalog | -> building-omnistudio-integration-procedure | 构建服务器端编排以覆盖产品和定价有效负载 |
| modeling-omnistudio-epc-catalog | -> deploying-metadata | 部署验证的目录元数据 |
...
- [references/epc-field-guide.md](references/epc-field-guide.md) — EPC 字段级指南和最低必需字段
- [references/naming-conventions.md](references/naming-conventions.md) — 产品、属性和包的命名和键约定
...
- 优先创建可重用的 spec product，然后通过子关系组装报价。
