# data360-connect：数据云连接阶段

当用户需要**源连接工作**时使用此技能：连接器发现、连接元数据、连接测试、源对象浏览、连接器模式检查，或为外部源准备连接器特定设置的有效负载。

## 此技能负责任务的情况

当工作涉及以下内容时，使用 `data360-connect`：
...
- 创建数据流或 DLO → [data360-prepare](../data360-prepare/SKILL.md)
- 创建 DMO、映射、IR 规则集或数据图 → [data360-harmonize](../data360-harmonize/SKILL.md)
- 编写 Data Cloud SQL 或搜索索引工作流 → [data360-query](../data360-query/SKILL.md)
...
- 首先验证插件运行时；参见 [../data360-orchestrate/references/plugin-setup.md](../data360-orchestrate/references/plugin-setup.md)。
- 在修改连接之前运行共享就绪分类器：`node ../data360-orchestrate/scripts/diagnose-org.mjs -o <org> --phase connect --json`。
- 在连接创建之前优先进行只读发现。
- 使用 `2>/dev/null` 抑制链接插件警告噪音，用于标准用法。
- 请记住 `connection list` 需要 `--connector-type`。
- 对于通过名称解析的非 Salesforce 连接的 `connection test`，请传递 `--connector-type`。
- 当组织不熟悉时，首先从流中发现现有连接器类型。
- 在发明连接器特定凭据或参数之前，使用精选示例有效负载。
- 对于精选示例之外的连接器类型，在构建 JSON 之前，通过 REST 检查一个已知的良好 UI 创建的连接。
- 不要因为连接创建成功就承诺为每种连接器类型基于 API 创建流。
