# 配置连接应用：Salesforce 连接应用与外部客户端应用

当用户需要在 Salesforce 中进行 **OAuth 应用配置** 时使用此技能：连接应用、外部客户端应用 (ECAs)、JWT 承载器设置、PKCE 决策、作用域设计或从旧版连接应用模式迁移到新版 ECA 模式。
...
**超出范围 — 转发至其他地方：**
- 配置命名凭证或运行时调用 → [构建 Salesforce 集成](../building-sf-integrations/SKILL.md)
- 将元数据部署到组织 → [部署元数据](../deploying-metadata/SKILL.md)
- 编写 Apex 令牌处理代码 → [生成 Apex](../generating-apex/SKILL.md)
...
- Spring '26 注意：在组织中默认禁用新连接应用的创建。对于新集成，除非明确需要连接应用兼容性，否则请优先使用外部客户端应用。
...
阅读 `references/security-checklist.md` 以获取完整的 120 点安全检查清单。优先考虑：
- 最小权限作用域
- 明确的回调 URL
- 对于公开客户端使用 PKCE
- 在适当的地方使用基于证书的认证
...
在交接之前阅读 `references/testing-validation-guide.md`。确认：
- 元数据文件命名正确（见下文注意事项）
- 作用域是合理的
- 回调和认证模型与实际客户端类型匹配
- 密钥没有嵌入到源代码中
...
如果部署失败，检查错误输出以查找：
- `DUPLICATE_VALUE` — 具有此名称的连接应用或 ECA 已存在；重命名或检索后更新
- `INVALID_CROSS_REFERENCE_KEY` — ECA 设置文件中的 `externalClientApplication` 名称与 `.eca-meta.xml` 文件名不完全匹配
- `INSUFFICIENT_ACCESS_OR_READONLY` — 用户缺乏“管理连接应用”权限
- 如果任何步骤失败，不要继续到下一步 — 使用上述特定消息向用户显示错误
...
| 默认情况下永远不要使用 `Full` 作用域 | 不必要的权限；仅请求应用需要的权限 |
| 对于公开客户端（移动、SPA）始终使用 PKCE | 防止认证码拦截 |
| 永远不要使用通配符或过于宽泛的回调 URL | 令牌拦截风险 |
| 编辑 ECA OAuth 安全设置之前必须从组织检索 | 文件模式未完全记录；先检索确保准确性 |
| 在 CLI 命令中使用 `<alias>` 占位符，永远不要硬编码组织 URL | 组织 URL 因环境而异 |
| 在写入文件之前从 `sfdx-project.json` 检测实际的 `packageDirectory` | 项目可能不使用默认的 `force-app/main/default/` 布局 |
...
| 使用 `.ecaOauthSecurity` 进行安全设置 | 使用 `.ecaOauthSecurity`，而不是 `.ecaSecurity` |
| ECA OAuth 安全设置是仅检索的 | 不能在源代码中从头创建 — 始终先从组织检索 |
...
| 需求 | 转发至 | 理由 |
|---|---|---|
| 命名凭证 / 调用运行时配置 | [构建 Salesforce 集成](../building-sf-integrations/SKILL.md) | 运行时集成设置 |
| 部署应用元数据 | [部署元数据](../deploying-metadata/SKILL.md) | 组织验证和部署 |
| Apex 令牌或刷新处理 | [生成 Apex](../generating-apex/SKILL.md) | 实现逻辑 |
...
| 文件 | 何时阅读 |
|---|---|
| `assets/connected-app-basic.xml` | 第 3 步 — 简单连接应用的模板，具有最小 OAuth |
| `assets/connected-app-oauth.xml` | 第 3 步 — 完整 OAuth 2.0 连接应用的模板 |
| `assets/connected-app-jwt.xml` | 第 3 步 — JWT 承载器 / 服务器到服务器连接应用的模板 |
| `assets/connected-app-canvas.xml` | 第 3 步 — 在 Salesforce UI 中嵌入 Canvas 应用的模板 |
...
| `references/oauth-flows-reference.md` | 第 2 步 — 详细的 OAuth 流比较和决策指南 |
