...
用于声明式、分步引导式数字体验的专家级 OmniScript 构建器。OmniScript 是 OmniStudio 中 Screen Flow 的类似物：多步骤、交互式流程，用于收集输入、编排服务器端逻辑（集成程序、DataRaptor），并向用户呈现结果——所有这些都不需要代码。

## 快速参考

**评分**：跨越 6 个类别的 120 分。**阈值**：✅ 90+（部署） | ⚠️ 67-89（审核） | ❌ <67（阻止 - 需要修复）
...
- **超出范围**：构建 FlexCard（使用 `building-omnistudio-flexcard`）、直接创建集成程序（使用 `building-omnistudio-integration-procedure`）、映射完整依赖树（使用 `analyzing-omnistudio-dependencies`）、将元数据部署到组织（使用 `deploying-metadata`）
...
**analyzing-omnistudio-dependencies → building-omnistudio-datamapper → building-omnistudio-integration-procedure → building-omnistudio-omniscript → building-omnistudio-flexcard**（你当前的位置：building-omnistudio-omniscript）

OmniScript 消耗集成程序和 DataRaptor。首先构建它们。FlexCard 可能会启动 OmniScript——在构建 FlexCard 之后构建。使用 analyzing-omnistudio-dependencies 在开始之前映射完整依赖树。
...
| 洞察 | 详细信息 |
|---------|---------|
| **类型/子类型/语言三元组** | 唯一标识一个 OmniScript。所有三个值都是必需的，并形成复合键。示例：类型=`ServiceRequest`，子类型=`NewCase`，语言=`English` |
...
| **元素层次结构** | 元素使用 Level/Order 字段来构建树结构。Level 0 = 步骤，Level 1+ = 步骤内的元素。Order 决定同一级别内的顺序 |
| **版本管理** | 可以存在多个版本；每个类型/子类型/语言三元组只有一个可以激活。通过 `IsActive` 字段激活 |
| **数据 JSON** | OmniScript 通过所有步骤传递单个 JSON 数据结构。元素通过合并字段语法从该共享 JSON 读取和写入 |
...
**构建前，评估替代方案**：OmniScript 适用于复杂、多步骤的引导流程。对于简单的单屏数据输入，请考虑 Screen Flow。对于无交互的数据显示，请考虑 FlexCard。
...
**然后**：检查现有的 OmniScript 以避免重复，识别可重用的集成程序或 DataRaptor，并映射依赖链。
...
| **循环块** | 遍历数据列表并渲染每个项目的元素 | `loopData`（JSON 路径到数组） |
...
| **数字** | 带有可选格式的数字输入 | `label`，`min`，`max`，`step`，`format` |
...
| **下拉列表** | 下拉选择 | `label`，`options`，`optionSource`（静态/数据） |
...
| **货币** | 带有区域设置格式的货币输入 | `label`，`currencyCode`，`min`，`max` |
| **电子邮件** | 带有格式验证的电子邮件输入 | `label`，`placeholder` |
| **电话** | 带有掩码的电话号码输入 | `label`，`mask`，`placeholder` |
| **URL** | 带有格式验证的 URL 输入 | `label`，`placeholder` |
...
| **披露** | 可展开/可折叠的内容 | `label`，`defaultExpanded` |
...
| 元素类型 | 目的 | 关键配置 |
|-------------|---------|------------|
| **DataRaptor 提取动作** | 从 Salesforce 拉取数据 | `bundle`，`inputMap`，`outputMap` |
| **DataRaptor 加载动作** | 将数据推送到 Salesforce | `bundle`，`inputMap` |
| **集成程序动作** | 调用服务器端集成程序 | `ipMethod`（Type_SubType），`inputMap`，`outputMap`，`remoteOptions` |
| **远程动作** | 调用 Apex @RemoteAction 或 REST | `remoteClass`，`remoteMethod`，`inputMap` |
| **导航动作** | 页面导航或重定向 | `targetType`，`targetId`，`URL` |
| **DocuSign 信封动作** | 触发 DocuSign 信封 | `templateId`，`recipientMap` |
| **电子邮件动作** | 发送电子邮件 | `emailTemplateId`，`recipientMap` |
...
| **提交动作** | 收集数据的最终提交 | `postMessage`，`preTransformBundle`，`postTransformBundle` |
...
- **数据预填充**：验证 DataRaptor 提取动作是否正确填充元素
- **稍后保存**：测试恢复功能（如果启用）
- **导航**：测试跨所有步骤的返回/前进/取消行为
- **错误场景**：模拟 IP/DataRaptor 失败，验证错误处理
- **嵌入式 OmniScript**：测试父 OmniScript 和子 OmniScript 之间的数据传递
- **批量数据**：使用 Loop Block 和 Type Ahead 元素测试大数据集
...
| 无限 DataRaptor 提取 | **性能下降** | 添加过滤条件；限制返回的记录 |
| 缺少输入验证 | **不良数据输入** | 添加验证元素或 `pattern`/`required` 在输入上 |
| 硬编码 Salesforce ID | **跨组织部署失败** | 使用合并字段或 Custom Settings/Metadata |
| 集成程序 (IP) 动作没有错误处理 | **静默失败** | 在 PropertySetConfig 中配置 `showError`，`errorMessage` |
| Text Block 中的大图像 | **页面加载缓慢** | 使用 Image 元素和优化的 URL |
| 每个步骤过多元素 | **用户体验差** | 限制每个步骤到 7-10 个输入元素 |
...
| 集成程序引用有效 | 5 | 所有 IP 动作引用激活的 IP |
| 输入/输出映射正确 | 5 | 数据在元素和动作之间正确流动 |
| 数据预填充配置 | 5 | 在用户交互之前加载初始数据 |

### 错误处理（20 分）

| 检查 | 分数 | 标准 |
|-------|--------|----------|
| 动作元素具有错误处理 | 5 | 所有 IP/DR 动作配置了 `showError` |
...
| 检查 | 分数 | 标准 |
|-------|--------|----------|
| 无无界数据获取 | 5 | 所有 DataRaptor Extracts 都有过滤器/限制 |
| 配置了惰性加载 | 5 | 动作元素在步骤进入时触发，而不是 OmniScript 加载时 |
| 每个步骤的元素数量合理 | 5 | 没有步骤包含 >15 个元素 |
| 使用条件渲染 | 5 | 当元素不适用时隐藏元素（而不仅仅是不可见） |
...
| 导航控件 | 5 | 适当配置返回、下一个、取消、稍后保存 |
| 响应式布局 | 5 | 元素配置为移动和桌面断点 |

### 安全（15 分）

| 检查 | 分数 | 标准 |
|-------|--------|----------|
| 客户端 JSON 中没有敏感数据 | 5 | 密码、SSN、令牌保存在服务器端 |
| IP 动作使用服务器端处理 | 5 | 敏感逻辑在集成程序中，而不是客户端 OmniScript |
| 尊重字段级访问 | 5 | 数据访问匹配用户配置文件/权限集 |
...
| 从技能 | 到 building-omnistudio-omniscript | 当... |
|------------|------------------|------|
| analyzing-omnistudio-dependencies | -> building-omnistudio-omniscript | "在构建 OmniScript 之前分析依赖关系" |
| building-omnistudio-datamapper | -> building-omnistudio-omniscript | "DataRaptor 准备就绪，构建使用它的 OmniScript" |
| building-omnistudio-integration-procedure | -> building-omnistudio-omniscript | "IP 准备就绪，将其连接到 OmniScript 动作" |

| 从 building-omnistudio-omniscript | 到技能 | 当... |
|--------------------|----------|------|
| building-omnistudio-omniscript | -> building-omnistudio-flexcard | "构建启动此 OmniScript 的 FlexCard" |
| building-omnistudio-omniscript | -> deploying-metadata | "将 OmniScript 部署到目标组织" |
| building-omnistudio-omniscript | -> analyzing-omnistudio-dependencies | "在部署前映射完整依赖树" |
| building-omnistudio-omniscript | -> building-omnistudio-integration-procedure | "此 OmniScript 动作需要新的 IP" |
| building-omnistudio-omniscript | -> building-omnistudio-datamapper | "为数据预填充需要 DataRaptor" |
...
| 大型 Loop Block 数据集 | 分页或限制 DataRaptor 结果；考虑在集成程序 (IP) 中进行服务器端过滤 |
...
| 版本冲突 | 激活旧版本之前停用新版本；同一类型/子类型/语言三元组永远不会有两个激活版本 |
| OmniScript 中的自定义 LWC | 将 LWC 注册为 OmniScript 兼容；遵循 `omniscript-lwc` 命名空间惯例 |
| 命名空间组织 | 如果部署到受管理的 OmniStudio 包组织，请将捆绑名称和 API 名称以相应的命名空间为前缀（例如，`omnistudio__`） |
| `OmniProcessType` 在创建时不能设置 | `OmniProcessType` 是根据 `IsIntegrationProcedure`（对于 OmniScript 为 false）计算的；不要直接设置它 |

对于常见的运行时故障排除（元素未渲染、数据未预填充、IP 动作静默失败），请参阅 `references/best-practices.md` 第 8 节。
...
**必需的上游技能**：`building-omnistudio-datamapper`，`building-omnistudio-integration-procedure`

**可选技能**：`deploying-metadata`，`building-omnistudio-flexcard`，`analyzing-omnistudio-dependencies`

**以编程方式创建 OmniScript**：使用 REST API (`sf api request rest --method POST --body @file.json`)。必需字段：`Name`，`Type`，`SubType`，`Language`，`VersionNumber`。OmniScripts 默认为 `IsIntegrationProcedure=false` — 不要直接设置 `OmniProcessType`（它会被计算）。`sf data create record --values` 标志无法处理像 `PropertySetConfig` 这样的 JSON 文本区域字段。通过 REST API 创建子 `OmniProcessElement` 记录以每个步骤和元素。
