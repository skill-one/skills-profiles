...
- **超出范围**：编写或修改 OmniScripts（使用 `omnistudio-omniscript-generate`）、构建 FlexCards（使用 `omnistudio-flexcard-generate`）、创建集成程序（使用 `omnistudio-integration-procedure-generate`）、配置数据映射器（使用 `omnistudio-datamapper-generate`）
...
**目的**：在查询任何组件元数据之前确定组织使用哪个 OmniStudio 命名空间。
...
1. **核心（行业命名空间）**：
   ...
   ```
   如果这成功，则该组织使用核心命名空间（API 234.0+ / Spring '22+）。
...
**评估结果**：成功的查询（退出代码 0 且 JSON 中有 `totalSize`）确认命名空间。查询失败（`INVALID_TYPE` 或 `sObject type not found`）意味着该命名空间不存在。
...
FlexCards 将其数据源配置存储在 `DataSourceConfig` JSON 字段中（不是 `Definition` — 该字段在 `OmniUiCard` 上不存在）：

```text
解析 DataSourceConfig JSON:
  1. 访问 dataSource 对象（单数，不是数组）
  2. 对于每个 dataSource 其中 type === 'IntegrationProcedures'（注意：复数）：
     - 提取 dataSource.value.ipMethod（IP 类型_子类型）
     - 添加边：FlexCard → 集成程序
  3. 对于每个 dataSource 其中 type === 'ApexRemote':
...
影响分析（最依赖的组件）：
  1. DM:AccountExtract       → 5 依赖者
  2. IP:fetchAccountData     → 3 依赖者
  3. DM:OrderTransform       → 2 依赖者
```
...
- `IsIntegrationProcedure` 布尔值和 `DataSourceConfig`（不是 `Definition`）字段名称是核心独有
...
| omnistudio-integration-procedure-generate | 提供命名空间和 IP 依赖关系图 | IP 编写使用依赖关系图以避免循环引用 |
| omnistudio-omniscript-generate | 提供命名空间和元素依赖数据 | OmniScript 编写使用正确的字段名称 |
| omnistudio-flexcard-generate | 提供命名空间和数据源依赖关系图 | FlexCard 编写使用检测到的 IP 引用进行验证 |
| external-diagram-mermaid-generate | 消费依赖关系图进行可视化 | 此技能生成与 external-diagram-mermaid-generate 风格兼容的 Mermaid 输出 |
| platform-custom-object-generate / platform-custom-field-generate | 提供sObject元数据用于数据映射器分析 | 依赖关系提取期间的对象字段验证 |
| platform-metadata-deploy | 部署使用命名空间正确的元数据类型 | 此技能提供每个命名空间的正确元数据类型名称 |

---

## 注意事项

- **依赖关系**：需要 `sf` CLI 与组织认证。可选：external-diagram-mermaid-generate 用于风格化可视化。
- **必须首先检测命名空间**：所有下游查询都依赖于知道正确的对象和字段 API 名称。
- **PropertySetConfig 是关键**：几乎所有依赖关系信息都存储在 OmniProcessElement 记录的此 JSON 字段中。
- **FlexCards 的数据源**：数据源在 `DataSourceConfig` 中，不在 `Definition` 字段中（`OmniUiCard` 上不存在）。卡片布局/状态在 `PropertySetConfig` 中。
- **数据映射器项包含对象引用**：OmniDataTransformItem 记录上的 InputObjectName 和 OutputObjectName 揭示数据映射器读取的源对象和写入的目标对象。到父记录的外键是 `OmniDataTransformationId`（全词“Transformation”）。
- **IsIntegrationProcedure 是区分符**：`OmniProcess` 使用布尔值 `IsIntegrationProcedure` 字段，而不是 `TypeCategory` 字段（该字段不存在）。`OmniProcessType` 选择列表根据此布尔值计算得出，可用于过滤读取但无法直接在创建时设置。
- **sf data create record 限制**：`--values` 标志无法处理 textarea 字段中的 JSON 字符串（例如，PropertySetConfig）。改用 `sf api request rest --method POST --body @file.json` 来创建具有 JSON 配置的记录。
...
| 文件 | 何时阅读 |
|------|-------------|
| `references/namespace-guide.md` | 阶段 1 — 所有三个命名空间（核心、vlocity_cmt、vlocity_ins）之间完整的对象/字段映射，部署的元数据类型名称，混合命名空间迁移场景 |
| `references/dependency-patterns.md` | 阶段 3 — 每种元素类型的完整依赖关系提取规则，FlexCard 数据源解析，数据映射器项解析，循环引用检测算法，影响分析模式 |
