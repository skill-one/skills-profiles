...
当用户希望在 React UI 包中实现文件上传功能时，请遵循此工作流。此功能仅提供 **API** — 您必须使用提供的 API 自行构建 UI 组件。
...
- 构建自己的自定义 UI（文件输入、拖放区域、进度条等）
- 通过 `onProgress` 回调跟踪上传进度

**不要：**

- 期待预构建的组件，如 `<FileUpload />` — 它们没有被导出
- 尝试导入 React 钩子，如 `useFileUpload` — 它们没有被导出
- 寻找拖放组件 — 它们没有被导出

源代码包含用于演示的参考组件，但它们是 **不可导入** 的。将它们用作构建自己的 UI 的示例。
...
interface FileUploadResult {
  ...
  contentVersionId?: string; // 如果提供了 recordId，则可用
}
```
...
**这些不可导出**，但可作为示例查看。阅读源文件以了解构建自己的 UI 的模式。
