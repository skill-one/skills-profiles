...
使用此技能生成生产级 Apex：新的类、选择器、服务、异步作业、可调用方法，以及触发器；并用于现有 `.cls` 或 `.trigger` 的基于证据的审查。
...
- 访问权限：`public`（仅在受管理的包或 `@RestResource` 需要时使用 `global`）
- API 版本：`66.0`（最低版本）
- ApexDoc 注释：是
...
所有步骤都是按顺序执行的。不要跳过、合并或重新排序。如果遇到障碍，请停止并请求缺失的上下文。如果不适用，请在报告中用一句话说明 `N/A`。

### 第一阶段 — 编写

1. **发现项目约定**
   - 服务-选择器-域分层结构、日志实用程序
   - 现有类/触发器以及当前的触发器框架或处理模式
   - 是否已使用触发器操作框架（TAF）

2. **选择最小的正确模式**（见类型特定指导）

3. **审查模板和资源**
   - 在编写之前从 `assets/` 中阅读匹配的模板（见类型特定指导的文件映射）
   - 当类型存在 `references/` 示例时，将其作为具体的样式指南阅读
   - 对于任何测试类工作，始终阅读并使用 `generating-apex-test` 技能

4. **带约束条件的编写** -- 应用以下规则部分中的每条规则
   - 使用 ApexDoc 生成 `{ClassName}.cls`
   - 生成 `{ClassName}.cls-meta.xml`   

5. **生成测试类** -- 加载技能 `generating-apex-test` 以创建 `{ClassName}Test.cls` 和 `{ClassName}Test.cls-meta.xml`。Apex 测试始终需要生成才能部署。没有测试文件创建或编辑可以不加载 `generating-apex-test` 技能来生成测试。

### 第二阶段 — 验证（报告之前必须执行）

编写文件是中途点，不是终点。步骤 6 和 7 每个都需要工具调用，并产生必须在步骤 8 报告中出现的输出。在两个步骤都运行并捕获其输出之前，不要总结或显示报告。
...
7. **执行 Apex 测试**
   ...
   - 将所有测试生成/修复/覆盖率工作委托给 `generating-apex-test`；迭代直到测试通过。
...
| 约束 | 理由 |
|---|---|
| 将所有 SOQL 放在循环外 | 避免 100 查询的查询治理限制 |
| 将所有 DML 放在循环外 | 避免 150 个语句的 DML 治理限制 |
| 在每个类上声明分享关键字 | 防止意外的 `without sharing` 默认值和数据暴露 |
| 使用自定义元数据/标签/描述调用，而不是硬编码的 ID | 确保跨组织可移植性 |
| 始终处理异常（记录、重新抛出或恢复） | 防止静默失败 |
| 使用绑定变量进行所有带有用户输入的动态 SOQL | 防止 SOQL 注入 |
| 使用 Apex 原生集合（`List`、`Map`、`Set`），而不是 Java 类型 | 防止编译错误 |
| 在使用之前验证 Apex 中是否存在方法 | 防止依赖不存在的 API |
| 在主代码路径中避免使用 `System.debug()` | 调试语句即使在日志未激活时也会评估并消耗 CPU。如果主代码路径需要，请使用日志框架 |
| 永远不要使用 `@future` 方法 | 使用 `System.Finalizer` 与队列式；`@future` 不能链接，不能从批处理调用，不能接受非原始类型 |

### 批量化和治理限制

- 所有公共 API 接受并处理集合；单记录重载委托给批量方法
- 在批处理/批量流中，优先选择部分成功 DML（`Database.update(records, false)`）并处理 `SaveResult` 以获取错误
- 使用 `Map<Id, SObject>` 构造函数进行高效的 ID 基于查找
- 使用 `Map<Id, List<SObject>>` 将子记录按父记录分组；在处理之前在单个循环中构建该映射
- 使用 `Set<Id>` 进行去重和成员检查；优先使用 `Set.contains()` 覆盖 `List.contains()`
- 使用关系子查询在单个 SOQL 中获取父记录+子记录，当两者都需要时
- 使用 `AggregateResult` 与 `GROUP BY` 进行汇总计算，而不是在 Apex 中查询和计数
- 仅 DML 记录实际已更改 — 在将其添加到更新列表之前，与 `Trigger.oldMap` 或先前状态进行比较
- 使用 `Limits.getQueries()`、`Limits.getDmlStatements()`、`Limits.getCpuTime()` 监控复杂事务中的消耗

### SOQL 优化

- 使用选择性查询并具有适当的 `WHERE` 子句；在可能的情况下，在过滤器中使用索引字段（`Id`、`Name`、`OwnerId`、查找/主从字段、`ExternalId` 字段、自定义索引）
- SOQL 中不存在 `SELECT *` —— 始终指定所需的精确字段
- 对绑定结果集应用 `LIMIT` 子句；使用 `ORDER BY` 获取确定性结果
- 查询自定义元数据类型（以 `__mdt` 结尾的对象）时，**不要**使用 SOQL — 使用内置方法（`{CustomMdt__mdt}.getAll().values()`、`getInstance()` 等）

### 缓存

- 使用平台缓存（`Cache.Org` / `Cache.Session`）用于频繁访问、很少更改的数据；设置 TTL 并始终处理缓存未命中 — 缓存可以随时被清除
- 使用 `private static Map` 字段作为事务范围缓存，以防止在同一执行上下文中重复查询；第一次访问时懒加载

### 安全

- 默认为 `with sharing`；记录 `without sharing` 或 `inherited sharing` 的理由
- SOQL 中的 `WITH USER_MODE` 和 `Database` DML 中的 `AccessLevel.USER_MODE` 用于 CRUD/FLS 执行
- 通过允许列表或 `Schema.describe` 验证动态字段/运算符名称
- 所有外部凭证/API 密钥的命名凭证
- `AuraHandledException` 用于 `@AuraEnabled` 用户界面错误（不显示内部详细信息）
- `without sharing` 需要自定义权限检查
- 将 `without sharing` 逻辑隔离在专用辅助类中；从 `with sharing` 入口点调用，以限制提升访问范围
- 通过平台加密在静态时加密 PII/敏感数据；永远不会在调试语句、错误消息或 API 响应中暴露 PII

### 安全验证

在最终确定之前验证：CRUD/FLS 执行（SOQL + DML） · 每个类都有显式的分享关键字 · 没有硬编码的密钥或记录 ID · PII 从日志和错误消息中排除 · 错误消息对最终用户进行了清理。

### 错误处理

- 在通用 `Exception` 之前捕获特定异常；在消息中包含上下文
- 仅在可能引发代码周围使用 `try/catch`（DML、调用外部系统、JSON 解析、类型转换）；避免防御性地包装简单的赋值/集合操作/算术
- 保留异常原因链：`new CustomException('message', cause)`（不要用连接的消息替换堆栈跟踪）
- 在每个服务域中提供自定义异常类，当有意义时
- 在 `@AuraEnabled` 方法中，捕获异常并重新抛出为 `AuraHandledException`
...
- 返回空集合，而不是 `null`
- 使用安全导航（`?.`）进行链式属性访问
- 除非保证存在，否则永远不要直接解除引用 `map.get(key)`；首先使用 `containsKey`、赋值+空检查或安全导航
- 使用空合并（`??`）为默认值
- 优先使用枚举而不是字符串常量；枚举值遵循 `UPPER_SNAKE_CASE`
...
| 队列式 | `{Descriptive}Queueable` | `ExternalSyncQueueable` |
| 可计划 | `{Descriptive}Schedulable` | `DailyCleanupSchedulable` |
| DTO | `{Descriptive}DTO` | `AccountMergeRequestDTO` |
| 包装器 | `{Descriptive}Wrapper` | `OpportunityLineWrapper` |
| 实用程序 | `{Descriptive}Util` | `StringUtil` |
| 接口 | `I{Descriptive}` | `INotificationService` |
| 抽象 | `Abstract{Descriptive}` | `AbstractIntegrationService` |
...
| 触发器操作 | `TA_{SObject}_{Action}` | `TA_Account_SetDefaults` |

附加命名规则：
- 类：`PascalCase`
- 方法：`camelCase`，以动词开头（`get`、`create`、`process`、`validate`、`is`、`has`、`can`）
- 变量：`camelCase`，描述性名词；列表作为复数名词（例如，`accounts`、`relatedContacts`）；映射作为 `{value}By{key}`（例如，`accountsById`）；集合作为 `{noun}Ids`
- 常量：`UPPER_SNAKE_CASE`
- 使用完整的描述性名称，而不是缩写（`acc`、`tks`、`rec`）

### ApexDoc

- 类头和每个 `public`/`global` 方法的必需
...
|---|---|---|
| 触发器 | 仅事件路由 | 业务逻辑、编排 |
| 处理程序/服务 | 流控制、协调 | 内联 SOQL/DML/HTTP/解析 |
...
| 场景 | 默认 | 关键特征 |
|---|---|---|
| 标准异步工作 | **队列式** | 作业 ID、链接、非原始类型、可配置延迟（通过 `AsyncOptions` 最多 10 分钟）、去重签名 |
| 非常大的数据集 | **批处理 Apex** | 分块处理，最多 5 个并发；使用 `QueryLocator` 用于大范围 |
| 现代批处理替代方案 | **CursorStep** (`Database.Cursor`) | 2000 记录块，更高的吞吐量，没有 5 个作业限制 |
| 定期计划 | **计划流程**（首选）或 **可计划** | 可计划有 100 个作业限制；仅在链接到批处理或需要复杂 Apex 逻辑时使用 |
| 作业后清理 | **Finalizer** (`System.Finalizer`) | 无论队列式成功/失败都会运行 |
| 长时间调用外部系统 | **Continuation** | 每个事务最多 3 个，3 个并行 |
| 延迟超过 10 分钟 | `System.scheduleBatch()` | 在特定未来时间安排批处理作业 |
| 遗留的“开火并忘记” | `@future` | **新代码中不要使用** — 见硬停止约束；用队列式 + Finalizer 替换 |
...
- `with sharing`；无状态 — 没有 `public` 字段或可变实例状态；保持公共 API 聚焦，合理使用 `static`
...
- 返回 `List<SObject>` 或 `Map<Id, SObject>`；使用共享的基本字段列表常量（不要内联重复）
...
- `with sharing`；实现 `Database.Batchable<SObject>`（跨块跟踪时添加 `Database.Stateful`）
- `start()` = 查询定义；`execute()` = 业务逻辑；`finish()` = 日志/通知
- 使用 `QueryLocator` 用于大数据集；通过 `Database.SaveResult` 处理部分失败
- 通过构造函数接受过滤参数以提高可重用性

### 队列式
- 模板：`assets/queueable.cls`
- `with sharing`；实现 `Queueable` 和在需要 HTTP 调用外部系统时可选实现 `Database.AllowsCallouts`
- 通过构造函数接受数据
- 添加链深度保护以防止无限链
- 可选地实现 `Finalizer` 以进行恢复/清理
- 使用 `AsyncOptions` 进行可配置延迟（通过 `AsyncOptions` 最多 10 分钟）和去重签名

### 可计划
- 模板：`assets/schedulable.cls`
- `with sharing`；`execute()` 委托给队列式或批处理
- 提供cron常量和方便的 `scheduleDaily()` 辅助方法

### DTO / 包装器
- 模板：`assets/dto.cls`
- 无需分享关键字（纯数据容器）
- 简单的公共属性；无参数构造函数和参数化构造函数；`Comparable` 当排序很重要时
- 对私有/受保护的内部 DTO 使用 `@JsonAccess`，这些 DTO 被序列化/反序列化
...
- 包含所有相关的 DML 上下文；如果 TAF：`new MetadataTriggerHandler().run();`
...
- `@InvocableVariable` 支持：原始类型、`Id`、`SObject`、`List<T>` 仅（没有 `Map`/`Set`/`Blob`）；使用 `List<Id>` 或 `List<SObject>` 字段用于 Flow 集合 I/O
- 始终在响应中包含 `isSuccess`、`errorMessage` 和 `errorType` (`e.getTypeName()`)
- 在响应中返回错误（推荐）；抛出异常触发 Flow 错误路径 — 仅用于不可恢复的失败

### REST 资源 (`@RestResource`)
- 模板：`assets/rest-resource.cls`
- `global with sharing`；类和所有方法都必须是 `global`
...
- 捕获异常并重新抛出为 `AuraHandledException`，使用用户友好的消息
...
- `{ClassName}Test.cls`（通过 `generating-apex-test` 技能生成）
- `{ClassName}Test.cls-meta.xml`（通过 `generating-apex-test` 技能生成）
...
风险： <安全、批量化、异步、依赖项说明>
分析器： <必需 -- 粘贴实际 run_code_analyzer 输出或状态 "run_code_analyzer=unavailable: <reason>" >
测试： <必需 -- 粘贴实际测试执行结果（通过/失败、覆盖率）或状态 "test_execution=unavailable: <reason>" >
部署： <干运行或下一步>
```
...
| 需要 | 委托给 |
|---|---|
| Apex 测试 / 修复失败 | `generating-apex-test` 技能 |
| 描述对象/字段 | 元数据技能（如果可用） |
| 部署到组织 | 部署技能（如果可用） |
...
此技能仅处理生产 `.cls`/`.trigger`/`.apex` 问题：编译/解析失败、部署依赖项错误、运行时治理限制失败。对于测试执行、断言、覆盖率或 `sf apex run test` 失败，委托给 `generating-apex-test`.
