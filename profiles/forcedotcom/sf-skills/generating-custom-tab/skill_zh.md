...
## 📋 概述
用于在Salesforce应用程序中导航到对象、网页内容或Visualforce页面的自定义标签页。
...
- **motif**：标签页图标样式 — 选择与对象用途语义匹配的motif。**不要**将相同的motif用于每个标签页。
- **label**：显示名称（仅非对象标签页必需；对象标签页从对象继承标签）
- **url**：网页URL（用于网页标签页）
- **page**：Visualforce页面名称（用于Visualforce标签页）

### 🚨 严格元素允许列表 — 首先阅读此内容

**根元素必须始终为`<CustomTab>`（**不是`<Tab>`**）。XML命名空间必须为`xmlns="http://soap.sforce.com/2006/04/metadata"`。

仅允许以下列出的元素。**任何不在列表中的元素都**会导致部署错误。

| 标签页类型 | 仅允许这些元素（其他任何内容都不允许） |
|---|---|
...
| **Visualforce标签页** | `<customObject>`（必需，设置为`false`），`<label>`（必需），`<motif>`（必需），`<page>`（必需），`<description>`（可选） |

### ⚠️ 禁止元素（这些元素中的任何一项都会导致部署错误）
...
此外禁止：
- 对象标签页上的`<label>`（对象标签页从自定义对象继承标签）
- 网页标签页上的`<page>`（仅用于Visualforce标签页）
...
- **正确示例**（用于Space_Station__c.tab-meta.xml）：
```xml
...
```
- **正确示例**（用于Supply__c.tab-meta.xml — 注意不同的motif）：
```xml
<?xml version="1.0" encoding="UTF-8"?>
<CustomTab xmlns="http://soap.sforce.com/2006/04/metadata">
    <customObject>true</customObject>
...
```
- **❌ 错误** — 不要添加`<sobjectName>`，`<name>`，`<fullName>`或`<label>`：
```xml
<CustomTab xmlns="http://soap.sforce.com/2006/04/metadata">
...
- **复制此精确模板** — 仅替换占位符值。**不要**添加、删除或重命名任何XML元素：
```xml
<?xml version="1.0" encoding="UTF-8"?>
<CustomTab xmlns="http://soap.sforce.com/2006/04/metadata">
...
```
- **上述7个元素**是网页标签页文件中**唯一**允许的元素。**不要**添加任何其他元素。
- `<description>`元素是可选的 — 如果不需要，可以删除它，但不要添加任何其他内容。

### Visualforce标签页
- **目的**：访问自定义Visualforce页面
...
## 📱 支持的应用程序
- **标准应用程序**：可在标准Salesforce应用程序中使用
- **自定义应用程序**：可以包含在自定义应用程序中
- **社区应用程序**：可在社区应用程序中使用
...
- 使用清晰、描述性的标签页标签
- 为功能选择适当的标签页类型
- **为每个标签页选择独特且与上下文相关的motif** — 不要将所有标签页默认设置为相同的图标
- 考虑用户体验和导航流程
- 在不同应用程序中测试标签页功能
- 确保适当的权限和可见性设置
- 遵循一致的命名规范
- 对象标签页文件**必须**仅包含`<customObject>true</customObject>`和`<motif>` — 除此之外不能有其他内容
- 网页标签页文件**必须**仅包含：`<customObject>false</customObject>`，`<label>`，`<motif>`，`<url>`，`<urlEncodingKey>`，以及可选的`<description>`，`<frameHeight>` — 除此之外不能有其他内容
- **不要**包含`<isHidden>`，`<tabVisibility>`，`<type>`，`<mobileReady>`或空元素
