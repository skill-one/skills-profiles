...
与代码分析器交互的所有操作都必须通过 `<skill_dir>/scripts/` 中的捆绑脚本执行。没有例外。
...
`<skill_dir>` 是包含此 SKILL.md 的目录的绝对路径。**绝对不要**使用 `./scripts/` — 那会针对用户的当前工作目录解析，而不是技能目录。

任何聚合、过滤或排序问题（“哪个文件有最多的违规项？”、“PMD 问题有多少？”、“按数量排序的顶级规则”、“按严重性分解”）都由 `query-results.js` 回答 — 其输出已包含 `topRules`、`topFiles` 和 `severityCounts`。

---

## 概述

此技能将自然语言请求（“扫描安全问题”、“检查我的更改”）转换为正确的 `sf code-analyzer run` 命令，执行跨任何组合的引擎/目标/严重性的扫描，并呈现可操作的扫描结果。当引擎提供的修复可用时，它会发现它们，请求用户确认，安全地应用它们，并提供验证。用于静态分析、安全审核、AppExchange 认证、代码质量检查以及在 Salesforce 项目中查找重复项/漏洞。

**范围内：** 运行扫描、解析/过滤/排序结果、应用引擎自动修复、基于差异的扫描、所有输出格式（JSON/HTML/SARIF/CSV/XML）、描述/列出规则、扫描故障排除。

**超出范围：** 安装/配置 `sf` 或插件（→ `configuring-code-analyzer`）、编写自定义规则/引擎、AI 生成的修复（超出引擎提供的范围）、深度重构、CI/CD 设置（→ `configuring-code-analyzer`）。
...
3. **始终**使用带时间戳的名称传递 `--output-file`（例如，`./code-analyzer-results-20260512-143022.json`） — 不要依赖标准输出。
4. **仅前台**（不使用 `run_in_background`）；大型扫描的超时时间为 1200000ms。
5. **无效的 v3 标志**（会导致错误）：`--format`、`--engine`、`--category`、`--json`。使用 `--rule-selector` + `--output-file` 代替。
...
预飞行检查：运行 `sf code-analyzer --help 2>&1 | head -1`。如果失败，或者扫描报告引擎启动错误（例如，“PMD 启动失败”、“java: 命令未找到”、“SFGE 失败”）：

1. **停止** — 不要尝试自己安装/诊断先决条件。
2. **委托给 `configuring-code-analyzer`** — 它处理所有设置。
3. 完成后，返回这里并重新运行扫描。
...
匹配以下请求；如果匹配，则跳转到步骤 3（构建命令）。否则，请按步骤 1 操作。

| 用户说 | 规则选择器 | 备注 |
|--------|------------|------|
| "扫描我的代码" / "运行代码分析器" | `Recommended` | 精选集，所有文件类型 |
| "检查安全问题" / "安全审核" | `all:Security:(1,2)` | 所有引擎，关键+高 |
| "扫描我的更改" / "检查差异" | (见步骤 1.5) | 通过 `git diff` 获取文件，过滤为可扫描类型，通过 `--target` 传递 |
| "运行 PMD" / "检查我的 Apex" | `pmd` | Apex 类和触发器 |
| "LWC 代码风格检查" / "检查我的 JavaScript" | `eslint` | JavaScript/TypeScript/LWC |
| "查找重复项" / "检查复制粘贴" | `cpd` | 代码克隆 |
...
| "AppExchange 安全审核" | `all:Security:(1,2)` | 见 `<skill_dir>/references/special-behaviors.md` → AppExchange |
...
### 1.2 类别
安全/OWASP → `Security` · 性能 → `Performance` · 最佳实践 → `BestPractices` · 代码风格/格式 → `CodeStyle` · 设计/复杂度 → `Design` · 错误易感性 → `ErrorProne` · 文档 → `Documentation`。

### 1.3 严重性
1=关键 · 2=高 · 3=中 · 4=低 · 5=信息。 "仅关键" → `1` · "关键+高" → `(1,2)` · "中及以上" → `(1,2,3)`。

### 1.4 具体规则
如果用户指定了规则（例如，"ApexCRUDViolation", "no-unused-vars"）：`--rule-selector <engine>:<ruleName>`，或者如果引擎不明确，则只指定 `<ruleName>`。

⚠️ **部分名称：** `--rule-selector` 需要**确切的完整**规则名称（例如，`@salesforce-ux/slds/no-hardcoded-values-slds2`，而不是 `no-hardcoded-values`）。不能使用通配符。如果您不确定 100%，请先查找它 — **不要猜测**：
```bash
...
### 1.5 目标
特定路径 → `--target <path>` · 通配符（“所有 Apex”）→ `--target **/*.cls,**/*.trigger` · "我的更改"/"差异" → `git diff --name-only [base]...HEAD`，过滤为可扫描类型，通过 `--target` 传递 · "LWC" → `--target **/lwc/**` · "流程" → `--target **/*.flow-meta.xml` · 未指定 → 省略（整个工作区）。
...
### 1.7 比较 / 差异
"自 main 以来" → `git diff --name-only main...HEAD` → 扫描那些文件 · "自上次提交以来" → `HEAD~1` · "与 develop 对比" → `develop...HEAD`。
...
仅使用 Bash 工具 — 永远不要使用 `run_code_analyzer` MCP 工具。
...
2. 告知用户：
   ...
   对于大型代码库，可能需要几分钟。
   ```
3. 使用嵌入的**字面**时间戳运行（不要 `$TIMESTAMP`），前台，超时 1200000ms，使用 `tee` 到 `.log`：
   ```bash
   ...
   ```
4. 退出 0 = 成功。出错时，请阅读日志文件和 `<skill_dir>/references/error-handling.md`。
...
**1–10** → 所有在一个表格中；
**11–50** → 严重性统计 + 顶级 10；
**50–5000** → 统计 + 顶级 10 违规项 + 顶级 10 规则 + 顶级 5 文件；
**5000+** → 相同，加上建议缩小范围（严重性/类别/文件夹）。始终以输出路径结束，并提供下一步操作：过滤/解释规则/应用修复。
...
引擎提供的修复是**确定性**的（不是 AI 生成的）。流程：供应商过滤（如果需要）→ 发现 → 呈现 → **等待用户确认** → 应用 → 总结。

### 6.1 供应商文件过滤（当需要时）

如果用户说“修复我的代码” / “项目源”，或者如果顶级违规文件是供应商库（jQuery、Bootstrap、`*.min.js`），请运行：
...
报告：“排除 X 个供应商文件（Y 个违规项）— jQuery、Bootstrap 等。仅对 Z 个项目文件应用修复。” 使用过滤后的文件进行 6.2+。

检测逻辑：`<skill_dir>/references/vendor-file-handling.md`。
...
这些都是由引擎生成的安全、确定性修复，不是 AI 生成的。

您希望我应用这些修复吗？(是 / 否 / 选择特定规则)
```

⚠️ **停止并等待用户的回复，即使他们最初说“扫描并修复所有内容”。** 仅在下一轮中“是” / “应用” / “继续”时才应用。
...
我需要重新运行扫描以验证修复是否解决了违规项吗？
```

### 6.6 — 处理用户的选择
- **拒绝 / “否”:** 跳过应用，跳过总结。不要重新扫描。
- **“选择规则”:** 将发现列表过滤为那些规则，并将过滤后的文件传递给 `apply-fixes.js`。
- **“全部” / “是”:** 对完整（或供应商过滤的）结果文件（不变）运行 `apply-fixes.js`。

### 6.7 — 可选重新扫描以验证
如果用户在 6.5 中接受提议，使用新的时间戳重新运行相同的扫描（不要覆盖原始文件）。比较扫描前后的违规项数量，并显示差异 — 清晰解决的修复将消失；剩余的违规项需要手动修复或与无关。

---

## 步骤 7：查询和过滤现有结果

在步骤 5 后，用户可能想要**深入特定子集**而无需重新运行整个扫描。此步骤处理所有结果探索请求。

### 触发时机
在用户请求切片、过滤、排序或探索现有结果时激活：
- "只显示安全问题"
- "AccountService.cls 中有什么？"
- "只显示 PMD 问题" / "过滤为关键和高"
- "哪些 ESLint 规则被触发？" / "显示 LWC 文件夹中的违规项"
- "最严重的 20 个" / "哪个文件有最多的违规项？"
- "最常见的规则是什么？" / "每个引擎的违规项数量是多少？" / "按严重性分解"

**重要：** 任何关于现有扫描结果的问题 — 过滤、排序、计数、聚合 — 必须使用 `query-results.js`。**绝对不要**使用内联 Python、`jq` 或临时脚本解析结果 JSON。查询脚本已经在其输出中提供了 `topFiles`/`topRules`/`severityCounts`。

---

## 步骤 8：描述一个规则

当用户问“这个规则是什么意思？”或“如何修复这个？”时，使用此步骤查找和解释特定规则。
...
知道引擎时（从扫描上下文中）传递 `--engine`；否则进行更广泛的搜索。返回 `success` / `multiple_matches` / `not_found` 之一。状态处理和模板：`<skill_dir>/references/post-scan-workflows.md`。

---

## 步骤 9：列出可用规则

触发："有哪些安全规则可用？" / "列出所有 PMD 规则" / "JavaScript 规则" / "推荐规则" / "有多少 ESLint 规则？" / "Apex 规则"
...
| "推荐规则" | `Recommended` | |
| "高严重性规则" | `(1,2)` | |
| "只给我数量" | `Recommended` | `--count-only` |
| "按数量排序的安全规则" | `Security` | `--top 10` |

过滤器：`--engine`, `--severity`, `--top`（默认 100），`--count-only`。脚本在调用 CLI 之前预先验证选择器标记（捕获拼写错误，如 `secruity`）。呈现：`<skill_dir>/references/post-scan-workflows.md`。
...
| 使用绝对 `<skill_dir>` 路径运行脚本 | `./scripts/` 解析针对用户的当前工作目录，而不是技能目录 |
| 不要在未经确认的情况下应用修复 | 用户必须批准代码修改 |
| 修复前进行供应商文件检查 | 如果 50%+ 供应商（jQuery/Bootstrap/`*.min.js`），请先过滤 |
| 修复脚本顺序：过滤（如果需要）→ 发现 → 应用 → 总结 | 跳过总结会使用户没有结果报告 |
| SFGE 需要显式 `--workspace` | 否则模板文件会导致编译错误 |
| 首先查找部分规则名称 | 猜测会返回 0 结果；使用 `sf code-analyzer rules` |
| 仅使用 Bash 工具，绝不使用 MCP | `run_code_analyzer` 和其他 MCP 工具绕过了脚本工作流 |
| 不要为修复调用其他技能 | 此技能拥有完整的端到端工作流 |
| 查询现有结果，不要重新扫描 | 步骤 7 即时过滤现有 JSON |
| 扫描返回 0 结果 | 无效的规则选择器 — 使用 `sf code-analyzer rules --rule-selector <selector>` 验证 |
| `jq` 解析失败 | 外壳引号 — 使用 `parse-results.js` / `query-results.js` 而不是 |
| 由 LLM 编写的内联脚本 | 不要编写脚本 — 使用 `<skill_dir>/scripts/` 中的现有脚本 |
| 排序/聚合回答为 ad-hoc Python | 始终使用 `query-results.js`；输出已经包含 `topFiles`/`topRules`/`severityCounts` |

---

## 参考文件索引

**脚本**（始终使用绝对 `<skill_dir>/` 前缀通过 `node` 执行，永远不要使用 Read）：
...
| `<skill_dir>/scripts/discover-fixes.js` | 步骤 6.2 — 识别可修复的违规项 |
| `<skill_dir>/scripts/apply-fixes.js` | 步骤 6.4 — 用户确认后安全地应用引擎修复 |
| `<skill_dir>/scripts/summarize-fixes.js` | 步骤 6.5 — 总结已应用的更改 |
| `<skill_dir>/scripts/query-results.js` | 步骤 7 — 不重新扫描即可过滤/深入现有结果的脚本 |
...
| `references/command-examples.md` | 不常见的命令场景 |
