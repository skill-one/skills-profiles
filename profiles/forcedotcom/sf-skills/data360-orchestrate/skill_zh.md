...
当工作涉及以下内容时，请使用`data360-orchestrate`：
- 多阶段Data Cloud设置或修复
...
当用户专注于一个特定领域时，请委派给特定阶段的技能：

| 阶段 | 使用此技能 | 典型范围 |
|---|---|---|
| Connect | [data360-connect](../data360-connect/SKILL.md) | 连接、连接器、源发现 |
| Prepare | [data360-prepare](../data360-prepare/SKILL.md) | 数据流、DLOs、转换、DocAI |
| Harmonize | [data360-harmonize](../data360-harmonize/SKILL.md) | DMOs、映射、身份解析、数据图 |
| Segment | [data360-segment](../data360-segment/SKILL.md) | 分段、计算洞察 |
| Act | [data360-activate](../data360-activate/SKILL.md) | 激活、激活目标、数据操作 |
| Retrieve | [data360-query](../data360-query/SKILL.md) | SQL、搜索索引、向量搜索、异步查询 |

当用户处于以下情况时，请委派到家族外部：
- 提取会话跟踪/STDM遥测 → [agentforce-observe](../agentforce-observe/SKILL.md)
- 仅编写CRM SOQL → [platform-soql-query](../platform-soql-query/SKILL.md)
- 加载CRM源数据 → [platform-data-manage](../platform-data-manage/SKILL.md)
- 创建缺失的CRM架构 → [platform-custom-object-generate](../platform-custom-object-generate/SKILL.md) 或 [platform-custom-field-generate](../platform-custom-field-generate/SKILL.md)
- 实现下游Apex或Flow逻辑 → [platform-apex-generate](../platform-apex-generate/SKILL.md)、[automation-flow-generate](../automation-flow-generate/SKILL.md)
...
- 对于`sf data360`命令，使用`2>/dev/null`抑制链接插件警告噪音，除非需要stderr输出来进行调试。
...
- 优先使用通用、可重用的JSON定义文件，而不是特定组织的研讨会有效负载。
...
将`sf data360 doctor`视为广泛的健康信号，而不是唯一的门。在部分配置的组织中，即使只读命令集（如连接器、DMO或分段）仍然可以工作，它也可能失败。
...
### 5. 在可能的情况下选择确定性工件
优先使用JSON定义文件和可重复的脚本，而不是一次性手动步骤。通用模板位于：
...
- `dmo list --all`在你需要完整目录时很有用，但第一页`dmo list`通常足以进行准备状态检查，并且更快。
- 分段创建可能需要`--api-version 64.0`。
- `segment members`返回不透明的ID；使用SQL连接以获取人类可读的详细信息。
...
| 需要 | 委派给 | 原因 |
|---|---|---|
| 加载或清理CRM源数据 | [platform-data-manage](../platform-data-manage/SKILL.md) | 在摄取之前种子或修复源记录 |
| 创建缺失的CRM架构 | [platform-custom-object-generate](../platform-custom-object-generate/SKILL.md)、[platform-custom-field-generate](../platform-custom-field-generate/SKILL.md) | Data Cloud期望现有的对象/字段 |
| 部署权限或包 | [platform-metadata-deploy](../platform-metadata-deploy/SKILL.md) | 环境准备 |
| 针对Data Cloud输出的Apex编写 | [platform-apex-generate](../platform-apex-generate/SKILL.md) | 代码实现 |
| 分段/激活后的Flow自动化 | [automation-flow-generate](../automation-flow-generate/SKILL.md) | 声明性编排 |
| 会话跟踪/STDM/Parquet分析 | [agentforce-observe](../agentforce-observe/SKILL.md) | 不同的Data Cloud用例 |
