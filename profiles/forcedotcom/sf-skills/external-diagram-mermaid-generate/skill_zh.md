# external-diagram-mermaid-generate: Salesforce 图表生成
...
### 在范围内
当用户需要时，使用`external-diagram-mermaid-generate`：
- Mermaid输出
- ASCII后备图表
...
- ERD之前的对象发现 → [platform-custom-object-generate](../platform-custom-object-generate/SKILL.md) 或 [platform-custom-field-generate](../platform-custom-field-generate/SKILL.md)

---

## 支持的图表系列

| 类型 | 推荐的Mermaid格式 | 典型用途 |
|---|---|---|
...
### 2. 收集数据
对于ERD和基于基础的图表：
- 当需要实际模式发现时，使用 [platform-custom-object-generate](../platform-custom-object-generate/SKILL.md) 或 [platform-custom-field-generate](../platform-custom-field-generate/SKILL.md)
...
| 需求 | 委托给 | 原因 |
|---|---|---|
| 实际对象/字段定义 | [platform-custom-object-generate](../platform-custom-object-generate/SKILL.md) / [platform-custom-field-generate](../platform-custom-field-generate/SKILL.md) | 基于基础的ERD生成 |
| 连接应用认证设置上下文 | [integration-connectivity-connected-app-configure](../integration-connectivity-connected-app-configure/SKILL.md) | 准确的OAuth流程 |
| Agentforce逻辑可视化 | [agentforce-generate](../agentforce-generate/SKILL.md) | 源头行为细节 |
| 流程行为图表 | [automation-flow-generate](../automation-flow-generate/SKILL.md) | 实际Flow逻辑基础 |
...
| OAuth流程参与者因授权类型不同而不同 | 在生成前先阅读相关资产模板，以避免参与者不匹配 |
...
- [references/color-palette.md](references/color-palette.md) — 色盲友好调色板，包含十六进制值
...
- [scripts/query-org-metadata.py](scripts/query-org-metadata.py) — 查询组织模式以作为ERD生成基础
...
- [assets/oauth/client-credentials.md](assets/oauth/client-credentials.md) — 客户端凭证服务账户
...
- **Mermaid代码块** — 可粘贴到GitHub、Confluence或任何Mermaid支持渲染器的` ```mermaid `块
- **ASCII后备方案**（当请求或Mermaid渲染器不可用时） — 使用框/箭头字符的纯文本图表
- **简要解释** — 2-5个要点指出关键关系、流程方向和图表中的任何假设或限制
- 对于ERD：带有字段标签和关系类型注释的对象卡片
- 对于序列图表：带编号步骤（`autonumber`）和清晰演员标签
