# Salesforce 数据操作专家（处理-sf数据）

当用户需要**Salesforce 数据工作**时使用此技能：记录 CRUD、批量导入/导出、测试数据生成、清理脚本或用于验证 Apex、Flow 或集成行为的数据库模式。

## 此技能负责任务的情况

当工作涉及以下内容时，使用 `handling-sf-data`：
- `sf data` CLI 命令
...
- 仅编写 SOQL → [querying-soql](../querying-soql/SKILL.md)
- 运行或修复 Apex 测试 → [running-apex-tests](../running-apex-tests/SKILL.md)
- 首先部署元数据 → [deploying-metadata](../deploying-metadata/SKILL.md)
- 创建或修改自定义对象/字段 → [generating-custom-object](../generating-custom-object/SKILL.md) 或 [generating-custom-field](../generating-custom-field/SKILL.md)
...
| 远程执行 | 他们希望立即在真实组织中创建/修改记录 |

如果用户可能仅希望生成脚本，则**不要**假设远程执行。
...
- `handling-sf-data` 作用于**远程组织数据**，除非用户明确要求本地脚本生成。
- 在创建数据之前，对象和字段必须已经存在。
- 对于自动化测试，当批量行为很重要时，请优先使用**251+ 记录**。
- 在创建大型或嘈杂的数据集之前计划清理——未跟踪的记录会随着运行次数的增加而累积并污染组织状态。
- 在测试记录中使用合成数据，而不是标识符——真实的 PII 会产生合规风险，并且在批量导入后无法安全删除。
- 对于简单的 CRUD，优先使用**CLI 首选**；当操作真正需要服务器端编排时，使用匿名 Apex。

如果元数据缺失，停止并转交给：
- [generating-custom-object](../generating-custom-object/SKILL.md) 或 [generating-custom-field](../generating-custom-field/SKILL.md) 以创建缺失的架构，然后 [deploying-metadata](../deploying-metadata/SKILL.md) 部署它，然后重试数据操作
...
### 2. 运行 describe-first 预飞验证，当架构不确定时
...
参见 [references/sf-cli-data-commands.md](references/sf-cli-data-commands.md) 了解 `sf sobject describe` 命令和用于检查字段、挑单值和可创建约束的 jq 过滤模式。
...
| 需求 | 委托给 | 原因 |
|---|---|---|
| 创建缺失的自定义对象 | [generating-custom-object](../generating-custom-object/SKILL.md) | 架构必须在数据操作之前存在 |
| 创建缺失的自定义字段 | [generating-custom-field](../generating-custom-field/SKILL.md) | 字段级别的架构必须在数据创建之前存在 |
| 运行批量敏感的 Apex 验证 | [running-apex-tests](../running-apex-tests/SKILL.md) | 测试执行和覆盖 |
| 首先部署缺失的架构 | [deploying-metadata](../deploying-metadata/SKILL.md) | 元数据就绪 |
| 实现消费数据的 Apex 生产逻辑 | [generating-apex](../generating-apex/SKILL.md) | Apex 类/触发器编写 |
| 实现消费数据的 Flow 逻辑 | [generating-flow](../generating-flow/SKILL.md) | Flow 编写和自动化 |
...
- `assets/bulk/` — 批量 API 2.0 Apex 模板（插入 200、500、10000 记录；通过外部 ID upsert）
- `assets/cleanup/` — 清理和回滚脚本（按名称、日期、模式删除；事务回滚）
- `assets/soql/` — SOQL 查询模板（聚合、子查询、父子关系、子到父、多态）
