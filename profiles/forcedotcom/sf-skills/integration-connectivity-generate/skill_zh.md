# integration-connectivity-generate：Salesforce 集成模式专家

当用户需要**集成架构和运行时管道**时，使用此技能：命名凭证、外部凭证、外部服务、REST/SOAP 调用模式、平台事件、CDC（变更数据捕获）和事件驱动集成设计。

## 此技能负责任务的情况

当工作涉及以下内容时，使用 `integration-connectivity-generate`：
...
- 配置 OAuth 应用本身 → [integration-connectivity-connected-app-configure](../integration-connectivity-connected-app-configure/SKILL.md)
- 仅编写 Apex 业务逻辑 → `platform-apex-generate` 技能
- 部署元数据 → `platform-metadata-deploy` 技能
- 导入/导出数据 → `platform-data-manage` 技能
...
### 5. 接管部署或实现细节
使用：
- `platform-metadata-deploy` 技能进行部署
- `platform-apex-generate` 技能进行更深入的服务 / 重试代码
- `automation-flow-generate` 技能进行声明式 HTTP 调用编排
...
| 需求 | 委托给 | 原因 |
|---|---|---|
| OAuth 应用设置 | [integration-connectivity-connected-app-configure](../integration-connectivity-connected-app-configure/SKILL.md) | 消费者密钥 / 证书 / 应用配置 |
| 高级调用服务代码 | `platform-apex-generate` 技能 | Apex 实现 |
| 声明式 HTTP 调用 / Flow 包装器 | `automation-flow-generate` 技能 | Flow 编排 |
| 部署集成元数据 | `platform-metadata-deploy` 技能 | 验证和发布 |
| 从 Agentforce 使用集成 | `agentforce-generate` 技能 | 代理动作组合 |
...
### 自动化钩子
- `scripts/suggest_credential_setup.py` — 检测集成文件时自动建议凭证配置步骤
- `scripts/validate_integration.py` — 在代理响应前验证集成模式
...
1. **凭证元数据** — 一个或多个在 `assets/named-credentials/` 或 `assets/external-credentials/` 中的文件，填充了组织特定的值
2. **调用 Apex 类** — 一个 `.cls` 文件，使用命名凭证模式，根据上下文选择异步/同步模式
3. **事件/CDC 资产** — 平台事件 `.object-meta.xml`、订阅者触发器或 CDC 配置（当选择事件驱动模式时）
4. **端点安全元数据** — 远程站点设置和/或 CSP 受信任站点 XML 文件
5. **评分报告** — 跨 6 个类别的 120 分评分（安全、错误处理、批量化、架构、最佳实践、文档）
6. **下一步** — 为生成的资产提供的部署或测试说明
