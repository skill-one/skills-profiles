...
- 编写或重构生产 Apex → `generating-apex` 技能
- 测试 Agentforce 代理 → `testing-agentforce` 技能
- 使用 Jest 测试 LWC → [generating-lwc-components](../generating-lwc-components/SKILL.md)
...
### 4. 运行严格的修复循环
当问题在于代码或测试质量时：
- 需要时将代码修复委托给 `generating-apex` 技能
- 添加或改进测试
- 在更广泛的回归之前重新运行聚焦的测试
...
| 不要在测试中隐藏易碎的 org 依赖 | 防止与 org 状态相关的间歇性失败 |
...
| 问题 | 解决方案 |
|-------|------------|
| 测试在本地通过但在 CI org 失败 | 检查 `SeeAllData=true` 或对 org 特定记录的未声明依赖 |
| 重构后覆盖率意外下降 | 首先运行聚焦的类级测试，然后扩大范围到 `RunLocalTests` 以确认 |
| 调用测试中出现的 "未提交的工作待处理" 错误 | DML 和 HTTP 调用不能在同一测试上下文中混合，除非用 `Test.startTest()` 包装 |
| 模拟在测试中不起作用 | 确保 `Test.setMock()` 在进行调用之前被调用 |
| 测试中 `@TestSetup` 数据缺失 | `@TestSetup` 数据按测试方法提交 — 重新查询它；不要存储在静态变量中 |
...
| 需要 | 委托给 | 理由 |
|------|-------------|--------|
| 修复生产代码或编写测试类 | `generating-apex` 技能 | 代码生成和修复 |
| 创建批量/边缘案例测试数据 | [handling-sf-data](../handling-sf-data/SKILL.md) | 真实的测试数据集 |
| 将更新后的测试部署到 org | [deploying-metadata](../deploying-metadata/SKILL.md) | 部署工作流 |
| 检查详细的运行时日志 | [debugging-apex-logs](../debugging-apex-logs/SKILL.md) | 更深入的失败分析 |
...
| `assets/bulk-test.cls` | 模板：包含 251+ 记录的批量测试，跨越了 200 记录的触发器批量边界 |
| `assets/mock-callout-test.cls` | 模板：使用 `HttpCalloutMock` 的 HTTP 调用模拟 |
| `assets/test-data-factory.cls` | 模板：可重用的 `TestDataFactory`，带有创建和插入辅助函数 |
| `assets/dml-mock.cls` | 模板：`IDML` 接口 + `DMLMock` 实现用于无数据库单元测试 |
| `assets/stub-provider-example.cls` | 模板：基于 `StubProvider` 的依赖注入模拟 |
| `hooks/scripts/parse-test-results.py` | 后工具钩子 — 解析 `sf apex run test` JSON 输出并格式化失败以供自动修复循环 |
