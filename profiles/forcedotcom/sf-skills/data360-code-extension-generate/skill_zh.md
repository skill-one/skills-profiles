# data360-code-extension-generate 技能
...
使用转换逻辑编辑 `payload/entrypoint.py`。
...
**使用 `data360-schema-get` 技能来验证 DLO 是否存在并检查字段名称。**
...
| `DLO not found` | 使用 data360-schema-get 技能验证 DLO 是否存在，检查拼写和 `__dll` 后缀 |
...
### 安全
1. 无硬编码凭证 — 仅使用 SF CLI 认证
2. 验证输入数据 — 检查空值和数据类型
3. 限制写入权限 — 仅授予必要的 DLO/DMO 访问权限

## 与其他技能的集成

**与 data360-schema-get 技能一起使用（验证必需）:**

`data360-schema-get` 技能对于在测试代码扩展之前验证 DLO 是**必需**的。
...
- Docker 仅用于部署，不用于本地测试
- 仅使用 SF CLI 认证（无单独的凭证文件）
- 扫描命令自动检测代码中的权限
