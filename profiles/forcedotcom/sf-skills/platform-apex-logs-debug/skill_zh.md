# platform-apex-logs-debug: Salesforce调试日志分析与故障排除

当用户需要从调试日志中进行**根本原因分析**时使用此技能：治理器限制诊断、堆栈跟踪解释、慢查询调查、堆/ CPU 压力分析，或基于日志证据的基于日志证据的修复循环。

## 此技能拥有任务的时机

当工作涉及以下内容时，使用 `platform-apex-logs-debug`：
- Salesforce 的 `.log` 文件
- 堆栈跟踪和异常分析
- 治理器限制
- SOQL / DML / CPU / 堆故障排除
...
- 生成或实施代码修复 → [platform-apex-generate](../platform-apex-generate/SKILL.md)
- 调试 Agentforce 会话跟踪 / parquet 远程遥测 → [agentforce-observe](../agentforce-observe/SKILL.md)
...
| 为每个发现的问题报告所有六个输出字段 | 确保每个问题都有可操作的、完整的发现 |
| 将每个发现分类为严重、警告或信息 | 帮助用户优先处理哪些问题 |
| 将代码生成委托给 `platform-apex-generate` | 此技能进行诊断；它不会重写 Apex 代码 |
| 将测试执行委托给 `platform-apex-test-run` | 此技能不运行或修复测试类 |
| 在未读取 `LIMIT_USAGE` 事件之前，永远不要假设限制是安全的 | 限制可能被早期操作消耗，在故障点不可见 |
...
| 陷阱 | 解决方案 |
|---------|------------|
| 日志在 2 MB 处截断 | 降低调试级别（例如，`ApexCode: INFO`，`ApexProfiling: FINE`）并重新捕获 |
| 相同的问题同时表现为 SOQL 和 CPU 问题 | 首先修复循环中的 SOQL — 它通常作为次要效应驱动 CPU 峰值 |
| 设置跟踪标志后没有日志出现 | 验证跟踪标志 `ExpirationDate` 是否在未来，并且正确地跟踪了用户 |
| 异步上下文更改限制值 | CPU 限制是 60,000 毫秒异步 vs 10,000 毫秒同步 — 在标记限制之前检查事务类型 |
| 堆栈跟踪指向框架行，而不是用户代码 | 沿调用栈向上走，越过触发处理程序，找到原始用户代码 |
...
| 需要 | 委托给 | 理由 |
|---|---|---|
| 实现 Apex 修复 | [platform-apex-generate](../platform-apex-generate/SKILL.md) | 代码更改生成 / 审查 |
| 通过测试重现 | [platform-apex-test-run](../platform-apex-test-run/SKILL.md) | 测试执行和覆盖循环 |
| 部署修复 | [platform-metadata-deploy](../platform-metadata-deploy/SKILL.md) | 部署编排 |
| 创建调试数据 | [platform-data-manage](../platform-data-manage/SKILL.md) | 针对性种子 / 重现数据 |
...
| `references/cli-commands.md` | SF CLI 命令用于检索、流式传输和管理调试日志 |
