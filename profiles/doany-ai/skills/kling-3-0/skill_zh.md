...
[Kling 3.0](https://www.runcomfy.com/models/kling/kling-3.0) 是快手科技出品的第三代电影级视频模型。该技能涵盖了 RunComfy 上 Kling 3.0 的全部六个渲染端点：两种模式（文本到视频和图像到视频）中的三个质量等级（标准、专业、4K）。

## Kling 3.0 是什么

Kling 3.0 是 Kling 视频模型的 V3 代。它生成具有同步原生音频的多镜头电影级视频，在镜头间保持一致的字符身份，并具有物理感知的动画效果。与 Kling 2.x 相比，Kling 3.0 支持更长的片段（最长 15 秒），在 4K 等级上支持原生 4K 输出，以及一个统一的多元提示分段系统，允许一个 Kling 3.0 生成包含多个不同场景并具有可控过渡。
...
- **4K** - 原生 3840x2160 输出。使用 Kling V3.0 4K，用于高分辨率品牌电影、大银幕电影级序列和原生分辨率完成母带。
...
4K 等级无论是否启用音频，价格都相同。标准和专业等级在启用音频时，每秒费用约增加 50%。
...
- **英雄 1080p 片段、广告创意、高运动保真度的访谈片段**：Kling V3.0 专业。比标准贵约 33%，在相同分辨率下运动更紧密，身份保留更明显。
- **4K 品牌电影、大银幕电影级、完成母带**：Kling V3.0 4K。原生 3840x2160（无放大步骤）。固定 $0.42/秒 使预算更可预测。仅在输出确实需要 4K 时使用——它比标准贵约 5 倍。

根据是否有源图像选择模式：

- **文本到视频 (t2v)**：仅提示，Kling 3.0 从头开始生成外观。使用 Kling 3.0 t2v，用于全新场景、全新构图、没有现有参考的环境。
- **图像到视频 (i2v)**：提示 + 源图像，Kling 3.0 动画化图像。使用 Kling 3.0 i2v，当你有一个必须保留到输出中的精确参考（面部、产品、场景）时。
...
**在一个 Kling 3.0 生成中实现多镜头。** 单个 Kling 3.0 提示可以描述一系列镜头。编号它们："镜头 1：黄昏时咖啡馆的广角。镜头 2：中景双人镜头，举杯。镜头 3：特写女人微笑，浅景深，暖填充光。" Kling 3.0 将在镜头间保留身份（面部、服装、道具）。

**i2v 的身份锚点。** 使用 Kling 3.0 i2v 时，重申应保持稳定的内容："保留主体的面部、姿势和服装；只有摄像机移动，背景变化。"

**`tail_image_url` 用于控制结尾。** 在 Kling 3.0 i2v 上，提供一个尾图像以锁定最后一帧。Kling 3.0 将从源图像插值到尾图像的运动。

**`generate_audio: true` 用于单次通过对话。** 描述 Kling 3.0 应在音频中生成的内容："温暖友好的语调，英语旁白" 或 "城市环境音，远处车流，无对话。" 音频在标准/专业上增加成本；4K 上为固定费率。

**`cfg_scale` 调整。** 默认 0.5 适用于大多数 Kling 3.0 提示。提高到 0.7-0.9，用于风格化输出的严格提示遵循。降低到 0.3-0.4，用于提示松散时的自然运动。
...
- 不支持的支持集之外的宽高比 -> 拒绝。
- 对于 Kling V3.0 4K，要求激进的多元镜头故事加上 15 秒加上对话加上 6 个剪辑 -> Kling 3.0 将会生成，但成本上升到每个生成约 $6.30。先使用标准进行验证。
...
| 原生 4K 英雄电影和电影级大银幕 | Kling V3.0 4K (t2v 或 i2v) |
...
| 精品 4K 完成母带，原生音频 | Kling V3.0 4K 与 `generate_audio: true` (固定费率) |
...
```
电影级多镜头：一对美国年轻人在烛光屋顶餐厅庆祝周年纪念日。镜头 1：黄金时刻的城市天际线广角。镜头 2：中景双人镜头，举杯。镜头 3：特写女人微笑，浅景深，暖填充光。微妙的环境弦乐，轻柔的风声，远处车流声。
```

**Kling 3.0 i2v（动画肖像，4K 等级）：**

```
从源图像缓慢推近镜头。轻微的呼吸运动，身份稳定的特征，柔和的自然光，浅景深。背景：暖金色时刻的光芒，有缓慢漂浮的尘埃微粒。无对话，只有房间环境音。
```

**Kling 3.0 垂直短片（标准等级，9:16）：**

```
9:16 垂直。穿着黑色围裙的咖啡师拉取单份浓缩咖啡，蒸汽升入晨光，浓郁的奶油慢慢形成。手持拍摄，浅景深，温暖的咖啡馆环境音和蒸汽喷嘴的嘶嘶声。
```

## Kling 3.0 常见问题解答

**Kling 3.0 片段的最大持续时间是多少？** 每个等级的每个生成最多 15 秒。对于更长的叙事，将脚本分段成多个 Kling 3.0 调用并拼接。
...
**Kling 3.0 可以生成音频吗？** 是的。设置 `generate_audio: true`。Kling 3.0 在同一生成通过中生成同步对话、环境音效和音乐。在 4K 上，费率保持在 $0.42/秒；在标准/专业上，启用音频时速率约增加 50%。

**Kling 3.0 支持哪些宽高比？** 16:9、9:16、1:1、4:3、3:4、21:9。4K 等级将 21:9 渲染为宽银幕电影裁剪，原生 3840x2160。

**Kling 3.0 i2v 支持尾图像吗？** 是的。`tail_image_url` 锁定最后一帧；Kling 3.0 从源图像到尾图像插值运动。

**Kling 3.0 与 Kling 2.x 有什么区别？** Kling 3.0 具有更强的多镜头身份保留，最大持续时间更长（15 秒，2.x 旗舰为 10 秒），4K 等级上原生 4K，以及所有等级的统一多元提示分段输入。

## 限制

- **每个调用持续时间上限 15 秒** 在每个 Kling 3.0 等级上。
- **Kling 3.0 4K 生成中最多 6 个连续镜头**。
- **i2v 需要一个公开可获取的 HTTPS 图像 URL。** 不支持本地文件。
- **宽高比是固定的** 到文档中列出的六个。其他比例会被裁剪或拒绝。
- **4K 输出文件很大。** 在批量 Kling V3.0 4K 运行之前，计划磁盘和带宽。
...
3. CLI 向 RunComfy 模型 API 提交 POST 请求，附带用户的令牌。
...
- **第三方内容**：你传递的图像 URL 由 RunComfy 服务器获取，而不是你的机器上的 CLI。将外部 URL 视为不受信任；基于图像的提示注入是任何接受图像输入的视频模型的已知风险。
