...
runcomfy run wan-ai/wan-2-7/edit-video \
  --input '{
    ...
    "prompt": "移除所有帧右下角的水印。保留所有其他内容完全不变。在水印位置匹配背景。"
  }' \
  --output-dir ./out
```
...
上述端点是提示驱动的 — 它们从空间语言解析目标区域。对于 SAM2 分割跟踪 + 时间感知修复填充的像素级掩码传播，RunComfy 提供了专门的 ComfyUI 工作流：
...
- **视频扩展**（画布扩展）：请参阅 [`video-outpainting`](https://www.skills.sh/agentspace-so/runcomfy-agent-skills/video-outpainting)。
- **完整视频重绘 / 运动迁移**：请参阅 [`video-edit`](https://www.skills.sh/agentspace-so/runcomfy-agent-skills/video-edit)。
...
该技能根据用户是否需要身份锁定重绘或帧堆栈处理，选择 Wan 2-7 Edit-Video（默认为提示驱动的区域编辑）或其中一个替代方案。CLI 向模型 API 发送 POST 请求，轮询请求状态，并将结果下载到 `--output-dir`。
