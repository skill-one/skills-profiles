...
Flux 2 Klein 的独特优势是 **以延迟优先的创意迭代**：亚秒级的反馈支持实时艺术指导会话和快速产品可视化，而批量式模型无法持续支持。选择它时，**迭代速度比极限分辨率更重要**。

| 您想要 | 使用 |
|---|---|
| 实时 / 实时艺术指导会话 | **Flux 2 Klein 4B** |
| 快速迭代，最终具有强细节 | **Flux 2 Klein 9B** |
...
如果用户明确提到 "Flux 2 Klein" / "BFL Klein" / "flux klein"，则无论他们是否提到 "Flux 2" 都应路由到此处。如果他们泛指 "Flux 2"，则应在默认之前询问他们是否想要 **Klein**（快速）或 **Pro**（最高质量）。
...
| `height` | int | 否 | 1024 | 与 `width` 的宽高意图匹配。 |

### `blackforestlabs/flux-2-klein/4b/text-to-image`

延迟优先变体。亚秒级 4 步推理。用于实时迭代 / 构思。

与 9B 相同的字段集。默认 `steps` 实际上是 4——该变体是为该步数构建的。

### 参考图像（两个变体）

最多支持 **4 个同时的参考图像** 在同一端点上用于风格迁移 / 指导构图。JSON 主体中该字段的准确名称在 [模型的 API 标签](https://www.runcomfy.com/models/blackforestlabs/flux-2-klein/9b/text-to-image?utm_source=skills.sh&utm_medium=skill&utm_campaign=flux-2-klein) 中有记录——通过 CLI 原样传递。参考图像的使用支持无需单独 `/edit` 端点的编辑式工作流。
...
**主语优先的声明性语法**。Flux 2 Klein 训练的结构是 *"主语 + 动作 + 场景 + 风格 + 光照 + 相机 + 质量"*。前置主语；以指令结尾。示例：`"一只充满活力的蜂鸟在飞行中吸食鲜艳的粉色木槿花蜜，翠绿色的蓝宝石羽毛在晨光中闪烁，柔和的焦外模糊的热带花园背景，微距摄影，锐利细节，电影级光照"`。
...
**多参考图像对齐**。当传递参考图像时，**保持它们的审美对齐**。在同一调用中混合水彩 + 照片真实 + 3D 渲染会使编辑器困惑。在所有参考中保持一致的视觉注册。

**条件编辑**：先说明什么保持不变，再说明什么改变。*"与参考图像相同的构图和光照，但将背景从海滩改为山地工作室。"* 此模式保持构图稳定。

**对于文本渲染**（Klein 拥有 8B Qwen3 嵌入器，不错但不如 GPT Image 2）：添加 `"清晰的排版，高对比度标签"` 并将步数增加到 ~25，如果文本出来是模糊的。对于图像中的文本或多语言渲染，请路由到 GPT Image 2。

**反模式**：

- 不要冲突形容词。"极简 + 奢华" 会抵消。
- 不要超过 ~512 个 token。模型会降低质量，不会优雅地截断。
- 不要要求 4K——模型的分辨率极限是 ~2K。
- 不要要求超宽 (>16:9)——模型会裁剪。
...
| **多参考品牌风格** | 参考之间强风格一致性，用于统一资源包 |
| **快速构思 → 润色工作流** | 4B 用于探索，9B 用于最终处理——整个过程中使用相同的提示语法 |
| **消费者 GPU 友好的推理** | 4B 变体可在普通硬件上运行；对于自托管比较相关，但 RunComfy 托管也可以 |
...
```
一只充满活力的蜂鸟在飞行中吸食鲜艳的粉色木槿花蜜
花，翠绿色的蓝宝石羽毛在晨光中闪烁，
柔和的焦外模糊的热带花园背景，微距摄影，锐利
细节，电影级光照
```
...
```
一个磨砂陶瓷马克杯放在回收木材桌上，左侧柔和的北窗光照，
浅景深，50mm 定焦镜头，f/2.0，中性背景，电商准备，4K 产品摄影
```

**品牌一致对（多参考）：**

```
与参考图像相同的构图和光照，但瓶标现在是蓝色，带有白色无衬线字体，
排印 "AURA"；保持瓶身轮廓、桌面和阴影与参考完全一致
```

## 限制

- **分辨率极限 ~2K**——对于更高原生分辨率，请路由到 Seedream 5。
...
1. 技能调用 `runcomfy run blackforestlabs/flux-2-klein/<变体>/text-to-image` 并使用符合模式的 JSON 正文。
...
4. 在终端状态下，CLI 获取 `GET .../requests/<id>/result` 并将任何主机以 `.runcomfy.net` 或 `.runcomfy.com` 结尾的 URL 下载到 `--output-dir`。其他 URL 被列出但不会获取。
5. `Ctrl-C` 时轮询会发送 `POST .../requests/<id>/cancel`，因此您不会为停止的 GPU 付费。

## 此技能不是什么

不是自托管的 Flux 运行器。不是能力授予——取决于可用的 RunComfy 账户。不是多租户。

## 安全与隐私

- **令牌存储**：`runcomfy login` 将 API 令牌写入 `~/.config/runcomfy/token.json`，权限为 0600（仅所有者可读写）。设置 `RUNCOMFY_TOKEN` 环境变量以在 CI / 容器中完全绕过文件。
- **输入边界**：用户提示作为 JSON 字符串通过 `--input` 传递给 CLI。CLI **不会** 扩展提示；它将 JSON 正文直接通过 HTTPS 传输到模型 API。提示内容没有 shell 注入表面。
