...
| 用户意图 | 模型 | 原因 |
|---|---|---|
| 批量编辑 1-20 张图像保持一致性（SKU 画廊，A/B 变体） | **Nano Banana Edit** | 每次调用最多 20 张输入图像；锁定系列宽高比/分辨率 |
...
| 基于掩码的区域替换（完整背景替换） | **Z-Image Turbo Inpaint** | 高强度+干净掩码=干净替换 |
| 未指定时的默认选项 | **Nano Banana Edit** | 最灵活，支持单次和批量 |

代理读取此表格，对用户意图进行分类，并选择匹配的下方子部分。
...
runcomfy run google/nano-banana-2/edit \
  --input '{
    "prompt": "将右下角的商标替换为干净的白色无衬线字体“AURA”。保持输入中的其他内容完全不变。",
...
runcomfy run openai/gpt-image-2/edit \
  --input '{
    "prompt": "保持照片、布局和品牌标记与输入完全一致。仅替换图像中的标题。新的标题以粗体日语假名显示“今日のおすすめ”，位置和字体粗细相同。",
...
- **精确引用图像内文本**。为非拉丁文本命名：`"Japanese kana"`，`"Cyrillic"`，`"Arabic right-to-left"`。
- **编号多参考**：`"主体来自图像 1，光照来自图像 2"`。
- **方向性布局语言**：`"将标题从右上角移动到底部中心"`，`"替换右下角的商标"`。
- **`size: "auto"`** 保持输入比例 — 建议除非编辑改变构图。
...
仅限单张图像 — 没有数组。对于多图像流程，使用路由 1（Nano Banana Edit）。
...
runcomfy run blackforestlabs/flux-1-kontext/pro/edit \
  --input '{
    "prompt": "保持人物的面部、姿势和服装不变。在她的左手添加一个橙色雨伞，并露出轻微的笑容。",
...
- **优先保持**。 领头 `"保持[不变元素]"` 然后说明变更。
- **小步迭代**。 单次复合编辑会在单次传递中漂移；拆分为顺序传递。
...
| 字段 | 类型 | 必填 | 备注 |
|---|---|---|---|
| `prompt` | string | 是 | 要填充/替换的内容；未掩码周围保持目标的约束。 |
| `image` | string | 是 | 源图像 URL。 |
| `mask_image` | string | 是 | **灰度掩码 URL**（白色=填充，黑色=保持）。 |
| `strength` | float | 否 | 0.3–0.6 精修，0.7–1.0 完全替换。 |
...
runcomfy run tongyi-mai/z-image/turbo/inpainting \
  --input '{
    "prompt": "移除天花板电线；保持屋顶线和天空渐变；使天空薄而干净。",
...
- **需要掩码 URL** — 灰度，白色=填充区域，黑色=保持。掩码边缘轻微模糊（1-3px）比锐利的二进制混合效果更好。
- **强度按意图**：`0.3–0.5` 用于精修/清理，`0.6–0.7` 用于对象替换并匹配风格，`0.8–1.0` 用于完全区域替换。
...
- **Token 存储**：`runcomfy login` 将 API token 写入 `~/.config/runcomfy/token.json`，权限为 0600（仅所有者可读写）。设置 `RUNCOMFY_TOKEN` 环境变量可完全绕过文件，在 CI / 容器中使用。
- **输入边界**：用户提示作为 JSON 字符串通过 `--input` 传递给 CLI。CLI 不会对提示进行 shell 扩展；它将 JSON 正文直接通过 HTTPS 传输到模型 API。提示内容没有 shell 注入表面。
- **第三方内容**：您传递的图像/掩码/视频 URL 由 RunComfy 模型服务器获取，而不是您的机器上的 CLI。将外部 URL 视为不可信；基于图像的提示注入是任何图像编辑/视频编辑模型的已知风险。
