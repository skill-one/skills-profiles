...
| 单次精确本地编辑，优先考虑源保真度 | Flux Kontext |
...
runcomfy run openai/gpt-image-2/edit \
  --input '{
    "prompt": "将图像 1 中的主体组合到图像 2 中的房间中。匹配图像 2 的光照和调色板。保留图像 1 主体身份（面部、姿势、服装）不变。",
...
**使用 `size: "auto"` 保留输入比例。** 仅在编辑明确改变构图时（例如将 16:9 裁剪为 1:1）时覆盖。
...
| **品牌安全标题 / 行动号召交换** | 布局精确度 + 保留语言保持其余部分稳定 |
...
```
将背景变为明亮的极简白到柔和灰色的工作室扫掠，带有轻柔的地板阴影；在图像中添加一个大型标题，显示“OPEN STUDIO”，使用粗体清洁的无衬线字体，高对比度，居中；保留主要人物或产品、姿势和面部身份不变
```
...
- **长复合提示漂移** — 需要时拆分为多个步骤。
- **跨许多 SKU 图像的批量一致性，Nano Banana 编辑（最多 20）更好。**
