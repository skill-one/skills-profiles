...
| 品牌安全、电子商务 / 广告 / UI 模拟图像 | **GPT 图像 2** |
| 迭代优化时保持构图稳定 | **GPT 图像 2** |
| 重度风格化、绘画风格 | Flux 2 |
| 超写实肖像 | Nano Banana Pro |
| 电影感 / 美学优先的英雄镜头 | Seedream 5 |

如果用户明确要求 GPT 图像 2 / ChatGPT 图像 2 / 图像 2，无论模型选择如何，都应路由到此处 — 不要质疑模型选择。
...
| `size` | enum | 否 | `1024_1024` | `1024_1024`（1:1），`1024_1536`（2:3 肖像），`1536_1024`（3:2 横幅） — **仅这三个** |
...
| `images` | string[] | 是 | — | **最多 10** 个参考图像 URL（公开可获取的 HTTPS） |
| `size` | enum | 否 | `auto` | `auto`（保留输入比例），或上述三个固定大小之一 |
...
**明确说明主体 + 背景 + 氛围。** "一个磨砂陶瓷水瓶的特写，放在温暖的亚麻布上，柔和的窗户光，标签上有干净的衬线字体 "AQUA+"，边缘微妙高光，适合电子商务，8K 细节，中性背景" — 三个具体指令 — 比起 "一个瓶子的漂亮产品照片" 更好。

**精确引用嵌入文本。保持简短。** GPT 图像 2 是此类中最强的文本渲染模型，但只有在您将**字面字符放在引号中**时才如此。长文本块会降低质量。对于多语言文本，命名脚本： "日语假名"，"西里尔文"，"阿拉伯语从右到左"。

**直接使用构图提示。** "三分法"，"特写"，"空中视角"，"居中主体"，"浅景深" — 这些对模型有学习意义。

**一次迭代一个属性。** 当优化时，每次迭代只改变一个东西（光照或背景或姿势或文本），并保持提示的其余部分不变。当只有一个旋钮移动时，模型在迭代中保持构图稳定。
...
- **声明保留目标。** "**保持**人物姿势和面部身份不变"， "**保持**包装上的品牌标志和排版"， "**保持**整体构图"。模型需要知道不要改变什么。
...
| **指示牌、海报、包装模拟** | 多尺度文本渲染精度 |
| **UI 模拟、科学插图** | 布局精确性和标签可读性 |
...
1. 技能调用 `runcomfy run openai/gpt-image-2/<端点>`，与上述模式匹配的 JSON 正文。
2. CLI POST 到 `https://model-api.runcomfy.net/v1/models/openai/gpt-image-2/<端点>`，使用用户的 bearer 令牌。
3. 模型 API 返回一个 `request_id`；CLI 每 2 秒轮询一次 `GET .../requests/<id>/status`。
4. 终端状态时，CLI 获取 `GET .../requests/<id>/result` 并下载任何主机以 `.runcomfy.net` 或 `.runcomfy.com` 结尾的 URL 到 `--output-dir`。其他 URL 会列出但不会下载。
5. 轮询时按 `Ctrl-C` 发送 `POST .../requests/<id>/cancel`，以免您为停止的 GPU 被计费。

## 此技能不是什么

不是 OpenAI API 客户端。不是能力授权 — 依赖于可用的 RunComfy 账户。不是多租户。
...
- **第三方内容**：您传递的图像 / 掩码 / 视频URL由 RunComfy 模型服务器获取，不是在您的机器上由 CLI 获取。将外部 URL 视为不可信；基于图像的提示注入是任何图像编辑 / 视频编辑模型的已知风险。
