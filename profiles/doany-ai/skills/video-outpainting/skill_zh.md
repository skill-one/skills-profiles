...
扩展视频的空间画布——垂直或水平方向裁剪，改变长宽比同时保留中心动作。这项技能通过 Wan 2-7 编辑视频功能实现空间扩展，以提示形状的画布变化，并在需要顶级缝合质量时，将代理指向专门的 ComfyUI 扩展工作流。
...
> 基于提示的视频编辑；接受空间扩展语言（“将画布扩展到 16:9，在左右两侧添加匹配的环境”）。质量足够用于社交和大多数内部用途。
> 选择原因：长宽比交换（垂直 ↔ 水平）、社交剪辑、缝合质量可接受的裁剪。
...
- **预期缝合处的质量差异**。Wan 2-7 并非专门为扩展训练；对于英雄级交付，请使用 ComfyUI 工作流。

---

## 当你需要英雄级缝合质量时

上述端点在大多数情况下处理长宽比交换效果良好。对于需要严格时间一致性、缝合处理和运动感知填充的空间帧扩展，RunComfy 提供了专门的 ComfyUI 工作流：

| 工作流 | 描述 |
|---|---|
| [ComfyUI 中的 LTX 2-3 扩展](https://www.runcomfy.com/comfyui-workflows/ltx-2-3-outpainting-in-comfyui-spatial-frame-expansion-workflow?utm_source=skills.sh&utm_medium=skill&utm_campaign=video-outpainting) | 使用 LTX 2-3 的专用视频扩展工作流 |
| 浏览 [comfyui-workflows](https://www.runcomfy.com/comfyui-workflows?utm_source=skills.sh&utm_medium=skill&utm_campaign=video-outpainting) 以查找 "outpaint" | 社区提供的其他视频扩展图 |
...
- **路径 1（Wan 2-7 编辑视频）** 使用 16:9 长宽比提示。非英雄级内容的快速路径。
- **ComfyUI LTX 2-3 扩展** 用于英雄级广告投放。
...
### 此技能不执行的操作
- **图像扩展**（单张静态图）：请参考 [`image-outpainting`](https://www.skills.sh/agentspace-so/runcomfy-agent-skills/image-outpainting)。
- **视频扩展**（时间轴上增加更多帧）：请参考 [`video-extend`](https://www.skills.sh/agentspace-so/runcomfy-agent-skills/video-extend)。
- **视频修复**（基于掩码的内部编辑）：请参考 [`video-inpainting`](https://www.skills.sh/agentspace-so/runcomfy-agent-skills/video-inpainting)。
...
- **生成文件大小限制**：CLI 会中止任何单个下载大于 2 GiB 的请求。
- **bash 使用范围**：`Bash(runcomfy *)` 仅用于运行 `runcomfy` 命令。
...
- [`video-inpainting`](https://www.skills.sh/agentspace-so/runcomfy-agent-skills/video-inpainting) — 基于掩码的内部区域编辑
- [`image-outpainting`](https://www.skills.sh/agentspace-so/runcomfy-agent-skills/image-outpainting) — 静态图像扩展
