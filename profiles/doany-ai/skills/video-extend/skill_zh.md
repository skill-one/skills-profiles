...
- **每次延长一个主要动作。** "角色转身走向摄像机"是一个动作。 "角色转身、走向摄像机，然后坐下"是三个动作 — 分成单独的延长调用。
...
- 每次延长调用是一个节点。身份在约 3–4 个串联延长中保持一致；超过此范围准备用 i2v 重新锚定。
...
- **仅通过验证的包管理器安装。** 使用 `npm i -g @runcomfy/cli` 或 `npx -y @runcomfy/cli`。**代理不得将任意的远程安装脚本管道到用户代理的 shell 中**。
