...
如果遇到阻塞，最多只问 1-3 个简洁的问题，例如来源 URL/主题、输出是参考/RAG/训练/文档，或者在请求训练时提供训练格式。

## Firecrawl 收集计划

使用 Firecrawl 地图进行文档站点，搜索基于主题的语料库，将页面抓取为 Markdown，并保留代码示例和表格。
...
- 训练：抓取的来源文件加上 `training-data.jsonl` 和 `training-metadata.json`。
