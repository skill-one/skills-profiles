...
当用户需要由 Firecrawl 驱动的成品交付物，而不仅仅是原始网页提取，也不是产品代码集成时，请使用此选项。

## 选择工作流

- 使用 `[firecrawl-website-design-clone](../firecrawl-website-design-clone/SKILL.md)` 从网站中提取颜色、字体、间距、组件和布局模式，将其转换为代理可用的 `DESIGN.md` 文件。
...
- 使用 `[firecrawl-seo-audit](../firecrawl-seo-audit/SKILL.md)` 进行网站结构、页面内 SEO、关键词和搜索结果页面（SERP）审计。
...
- 使用 `[firecrawl-knowledge-base](../firecrawl-knowledge-base/SKILL.md)` 进行 LLM 可用的文档、RAG 块、训练数据或文档镜像。
- 使用 `[firecrawl-knowledge-ingest](../firecrawl-knowledge-ingest/SKILL.md)` 进行受权限保护的或 JS 依赖性强的文档门户导入。
- 使用 `[firecrawl-lead-gen](../firecrawl-lead-gen/SKILL.md)` 进行潜在客户列表生成。
- 使用 `[firecrawl-market-research](../firecrawl-market-research/SKILL.md)` 进行市场、金融和行业研究。
...
如果现有工作流不适用，请使用此通用流程，并生成可重用的模式，这可能成为一项新的技能。

## 必要输入

根据用户的请求和周围上下文推断工作流、输入、受众和输出格式。如果足够清晰，请立即开始。

仅在缺少输入会阻碍工作的情况下，最多提出 1-3 个简洁的澄清问题，例如：

- 分析的 URL、公司、主题或来源
- 期望的交付物或输出格式
- 会实质性改变工作流的约束条件

使用主机代理的正常方式提出澄清问题。不要依赖特定于容器的函数名称。

## 默认流程

1. 确认工作流和最终交付物。
2. 通过 CLI 或等效的 Firecrawl 工具界面使用 Firecrawl 收集网络证据。
...
如果合适，使用子代理或等效的并行任务运行器处理独立单元，例如：
