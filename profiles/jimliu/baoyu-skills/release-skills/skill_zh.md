...
3. **批量处理**：如果工具支持每调用一次处理多个问题，则将所有适用问题合并为单个调用；如果仅支持单个问题，则按优先顺序逐个询问。
...
| `--minor` | 强制次版本号递增 |
| `--patch` | 强制补丁版本号递增 |
| `--backfill-releases` | 为变更日志部分中存在的现有标签创建缺失的 GitHub Releases |
...
如果 `.releaserc.yml` 定义了 `release.hooks`，请保持发布工作流通用，并将特定于项目的打包/发布委托给这些钩子。
...
- 提交正文/脚注包含 `BREAKING CHANGE:`
- 移除公共 API、重命名导出、更改接口

如果检测到破坏性变更，则警告用户： "检测到破坏性变更。考虑主版本号递增（`--major` 标志）。"
...
2. **检测第三方贡献者**：
   ...
   - 通过 `gh pr view <number> --json author --jq '.author.login'` 查询每个合并的 PR 的 PR 作者
   - 与仓库所有者（`gh repo view --json owner --jq '.owner.login'`）进行比较
...
3. **创建发布笔记文件**：
   ...
   - 仅提取确切的 `## {VERSION} - {YYYY-MM-DD}` 部分到下一个 `##` 之前
   - 当需要时，匹配纯文本版本和带标签的标题，例如 `1.2.3` 和 `v1.2.3`
   - 保持破坏性变更靠近顶部；如果需要，在其余章节之前添加简短的高亮
   - 将笔记写入 UTF-8 临时文件，并重用它用于注释标签消息、GitHub Releases 和 `publish_artifact`
   - 在正常模式下，如果找不到笔记，则停止而不是创建空的标签或 GitHub Release
...
3. **创建注释标签**：
   ...
   ```
   如果 `.releaserc.yml` 设置 `tag.sign: true`，则使用 `git tag -s` 并使用相同的笔记文件。
...
项目工件发布和 GitHub Releases 是单独的输出：
...
当用户要求回填历史发布或传递 `--backfill-releases` 时使用此模式。

1. 不要递增版本、编辑变更日志或创建发布提交。
2. 列出按版本顺序存在的现有标签并检测缺失的发布：
   ```bash
...
3. 对于没有 GitHub Release 的每个标签：
   ...
   - 从 `CHANGELOG.md` 中提取匹配的部分；如果没有匹配的变更日志文件，则回退
...
/release-skills --patch      # 强制补丁版本号递增
...
**重要**：如果用户说 "直接推" 或 "直接 push" 并有未提交的更改，仍然需要首先执行上述所有步骤。
