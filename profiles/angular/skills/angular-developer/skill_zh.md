...
- **基础概念**：组件结构、元数据、核心概念、自闭合标签和模板控制流（@if、@for、@switch）。阅读 [components.md](references/components.md)
...
- **命名规范**：现代 Angular v20+ 的命名风格（“意图优先于角色”）用于文件、组件、服务、指令、管道和模型。阅读 [naming-conventions.md](references/naming-conventions.md)

如果您需要更深入的文档，请参考上述参考内容之外的官方文档，阅读 `https://angular.dev/guide/components` 上的文档。

## 反应性和数据管理

在管理状态和数据反应性时，使用 Angular Signals 并参考以下内容：

- **Signal 概述**：核心 Signal 概念（`signal`、`computed`）、反应性上下文和 `untracked`。阅读 [signals-overview.md](references/signals-overview.md)
...
- 如果应用程序使用 v22 或更高版本且这是新表单，**优先使用 Signal Forms**。
- 对于旧版本应用程序或在处理现有表单时，使用与应用程序当前表单策略匹配的适当表单类型。
...
- **基础概念**：依赖注入概述、服务和 `inject()` 函数。阅读 [di-fundamentals.md](references/di-fundamentals.md)
...
在为以下任何模式构建可访问的自定义组件时：Accordion、Listbox、Combobox、Menu、Tabs、Toolbar、Tree、Grid，请参考以下内容：

- **Angular Aria 组件**：构建无头、可访问的组件（Accordion、Listbox、Combobox、Menu、Tabs、Toolbar、Tree、Grid）以及样式 ARIA 属性。阅读 [angular-aria.md](references/angular-aria.md)
...
- **路由加载策略**：立即加载与延迟加载，以及上下文感知加载。阅读 [loading-strategies.md](references/loading-strategies.md)
...
- **渲染策略**：CSR、SSG（预渲染）和带预渲染的 SSR。阅读 [rendering-strategies.md](references/rendering-strategies.md)
...
- **基础概念**：单元测试的最佳实践（Vitest）、异步模式和 `TestBed`。阅读 [testing-fundamentals.md](references/testing-fundamentals.md)
