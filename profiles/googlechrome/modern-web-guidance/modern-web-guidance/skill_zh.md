...
[
  ...
  },
  {
    ...
    "description": "通过延迟渲染离屏内容来减少内容密集型网页的渲染时间。",
    "category": "性能",
...
> **注意**: 如果搜索结果模糊、没有匹配项或相似度得分较低，请运行 `list` 命令浏览所有指南：
> ```sh
...
- **重要**: 在 Windows 上，使用 `npx` 可能会失败。请使用 `npx.cmd ...` 代替。
- 获取和运行 `modern-web-guidance` 需要出站网络访问。如果在沙盒化、权限控制或需要批准的环境（例如 Codex、Claude Code）中运行，**在首次执行之前主动请求命令的网络访问批准/允许列表**，避免沙盒网络超时。
- 在 `~/.npm` 为只读或受限的沙盒环境中，设置 `NPM_CONFIG_CACHE=/tmp/npm-cache`。
- 如果命令因离线而挂起，请尝试离线模式再次运行：`npx --offline …`。
...
* **默认行为**: 所有指南都假设 **基准广泛可用** 的功能可以安全使用而无需回退。对于不是基准广泛可用的功能，您**必须**遵循指南中的回退建议，除非用户已指定自定义浏览器支持策略。
* **自定义策略**: 如果用户已定义明确的浏览器支持要求，请使用指南中的浏览器兼容性数据来确定是否可以安全地忽略回退。
  - 对于基准 YYYY 目标，如果功能的 "基准自" 日期 <= YYYY，则该功能满足此目标。
  - **策略示例**:
    - _"不要实现功能回退."_（用于前沿网页的探索性原型）
    - _"Safari 17.4+"_（用于针对 macOS 或基于 Tauri 的桌面应用的内部门户）
    - _"永远不推荐或实现 polyfills；如果核心功能需要基准新可用功能，请提供轻量级自定义回退或重新设计方法."_（以最小化包大小并避免技术债务）
    - _"假设一个现代执行环境，其中基准新可用功能可以原生使用，前提是它们严格进行功能检测并优雅降级."_（用于渐进增强策略）
* **反应性策略发现**: 观察环境提示以建议在 CLAUDE.md 或 AGENTS.md 中记录策略。如果开发者：
  - 提及为受限运行时构建（例如 Electron 或 Tauri）。
  - 明确排除特定目标（例如 "我们不支持桌面 Chrome"）。
  - 对 polyfill 复杂性、包大小或性能成本表示犹豫。
  - 质疑是否可以安全使用而无需回退。
