...
一旦定义，这些规则将显著减少重复提示，并确保所有 Figma 实现任务的一致输出。
...
- `clientLanguages`：项目中使用的语言列表，用逗号分隔（例如，"typescript,javascript"，"python"，"javascript"）
...
2. 如果响应太大或被截断，运行 get_metadata 获取高级节点映射，然后重新获取所需的节点 get_design_context
3. 运行 get_screenshot 以获取节点变体的视觉参考
4. 只有在您同时拥有 get_design_context 和 get_screenshot 后，才下载所需的资源并开始实现
5. 将输出（通常是 React + Tailwind）转换为此项目的约定、样式和框架
6. 在标记完成之前，与 Figma 验证 1:1 的外观和行为
...
- Figma MCP 服务器提供了一个资产端点，可以提供图像和 SVG 资产
- IMPORTANT: 如果 Figma MCP 服务器为图像或 SVG 返回本地源，请直接使用该源
- IMPORTANT: 不要导入/添加新的图标包——所有资产都应在 Figma 负载中
- IMPORTANT: 如果提供了本地源，不要使用或创建占位符
- 将下载的资产存储在 `[ASSET_DIRECTORY]`
```
...
| Codex CLI | 项目根目录下的 `AGENTS.md` | Markdown 格式。如果文件已存在，则作为新部分追加。32 KiB 合并大小限制。 |
...
```markdown
---
description: 使用 Figma MCP 服务器实现 Figma 设计的规则。涵盖组件组织、样式约定、设计令牌、资产处理和 Figma 到代码的工作流程。
...
```markdown
- 颜色定义为 CSS 变量在 `src/styles/tokens.css`
- 不要硬编码十六进制颜色——使用 `var(--color-*)` 令牌
- 间距使用 4px 基础尺度：`--space-1`（4px），`--space-2`（8px），等等
```
...
```markdown
- 所有组件必须接受 `className` 属性用于组合
- 变体属性应使用联合类型：`variant: 'primary' | 'secondary' | 'ghost'`
- 图标组件应接受 `size` 和 `color` 属性
```
...
```markdown
- 为导出的组件添加 JSDoc 注释
- 为所有属性包含 PropTypes 或 TypeScript 类型
- 将魔法数字提取到命名常量
```
...
- 颜色定义在 `tailwind.config.js` theme.colors
- 间距使用 Tailwind 的默认尺度
- 自定义组件放在 `src/components/ui/`

## 资产规则

- IMPORTANT: 直接使用 Figma MCP 服务器返回的本地源
- 静态资产存储在 `public/assets/`
- 不要安装新的图标库
- 如果提供了本地源，不要使用或创建占位符
```
...
- 组件位于 `src/components/`
- 组合式位于 `src/composables/`
- Vue SFC 结构：`<script setup>`，`<template>`，`<style scoped>`

## 设计令牌

- IMPORTANT: 所有颜色定义在 `src/styles/tokens.css` 作为 CSS 变量
- 使用 `var(--color-primary)`，`var(--color-secondary)`，等等
...
**结果：** 代理现在正确地将 Figma 设计转换为 Vue 组件，使用项目的设计令牌系统。
...
- IMPORTANT: 不要硬编码值——从令牌包导入

## 文档要求

- 为每个组件添加 Storybook 故事
- 添加带有 @example 的 JSDoc
- 描述所有属性
- 添加可访问性说明
...
**结果：** 整个团队在将 Figma 设计添加到设计系统时遵循一致的模式。
...
写：始终使用来自 `src/components/ui/Button.tsx` 的 Button 组件，变体属性为 ('primary' | 'secondary' | 'ghost')

### 使规则可执行

每条规则都应告诉代理确切要做什么，而不仅仅是避免什么。

好：颜色定义在 `src/theme/colors.ts`——导入并使用这些常量
坏：不要硬编码颜色

### 使用 IMPORTANT 标记关键规则

用 "IMPORTANT:" 前缀标记必须永远遵守的规则，以确保代理优先考虑它们。
...
设计系统规则改变了 AI 编码代理如何与您的 Figma 设计工作：
...
创建良好规则所花费的时间在每次 Figma 实现任务中都会成倍地回报。
