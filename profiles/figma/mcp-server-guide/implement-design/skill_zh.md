...
- Figma MCP 服务器必须已连接并可访问
  - 在继续之前，通过检查 Figma MCP 工具（例如，`get_design_context`）是否可用来验证 Figma MCP 服务器是否已连接。
  - 如果工具不可用，则 Figma MCP 服务器可能未启用。指导用户启用随插件提供的 Figma MCP 服务器。他们可能需要重新启动 MCP 客户端。
...
当用户提供一个 Figma URL 时，提取文件密钥和节点 ID 以作为参数传递给 MCP 工具。
...
- 将 Figma MCP 输出（通常为 React + Tailwind）视为设计和行为的表示，而不是最终的代码样式
...
- 优先考虑 Figma 精度以完全匹配设计
- 避免硬编码值 - 在 Figma 提供时使用设计令牌
...
1. 解析 URL 以提取文件密钥=`kL9xQn2VwM8pYrTb4ZcHjF` 和 节点 ID=`42-15`
...
4. 从资源端点下载任何按钮图标
5. 检查项目是否有现有的按钮组件
6. 如果有，使用新变体扩展它；如果没有，使用项目规范创建新组件
...
1. 解析 URL 以提取文件密钥=`pR8mNv5KqXzGwY2JtCfL4D` 和 节点 ID=`10-5`
2. 运行 `get_metadata(fileKey="pR8mNv5KqXzGwY2JtCfL4D", nodeId="10-5")` 了解页面结构
3. 从元数据中识别主要部分（页眉、侧边栏、内容区域、卡片）及其子节点 ID
4. 对每个主要部分运行 `get_design_context(fileKey="pR8mNv5KqXzGwY2JtCfL4D", nodeId=":childNodeId")`
...
在实现过程中频繁验证，而不仅仅是在最后验证。这可以尽早发现问题。
...
**原因：** 设计过于复杂或具有过多的嵌套层，无法在单个响应中返回。
**解决方案：** 使用 `get_metadata` 获取节点结构，然后使用 `get_design_context` 分别获取特定节点。

### 问题：实现后设计不匹配

**原因：** 实现的代码与原始 Figma 设计之间存在视觉差异。
**解决方案：** 与第 3 步的屏幕截图进行并排比较。检查设计上下文数据中的间距、颜色和字体排印值。
