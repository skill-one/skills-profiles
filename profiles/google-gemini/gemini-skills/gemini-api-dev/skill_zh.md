...
- `gemini-3.8-flash`：1M tokens，快速，适用于代理和多模态任务的均衡性能
- `gemini-3.5-flash-lite`：1M tokens，最快，最高性价比的 3.5 模型，用于高吞吐量执行
- `gemini-3.1-pro-preview`：1M tokens，复杂推理、编码、研究
- `gemini-3.1-flash-lite`：成本高效，最快性能，适用于高频、轻量级任务
- `gemini-3.5-transcribe`：快速语音转文本，具有智能和逐字模式
- `gemini-3-pro-image`（Nano Banana Pro）：65k / 32k tokens，高质量图像生成和编辑
- `gemini-3.1-flash-image`（Nano Banana 2）：65k / 32k tokens，快速、高效的图像生成和编辑
- `gemini-3.1-flash-lite-image`（Nano Banana 2 Lite）：65k / 32k tokens，超快速图像生成和编辑
- `gemini-3.1-flash-tts-preview`：具有导演座椅提示的富有表现力的文本转语音
- `gemini-omni-1.1-flash`：视频生成，第一帧到视频，第一帧和最后一帧过渡，视频扩展（最长40秒），视频编辑和参考引导生成
...
> **如果用户要求已弃用的模型，请使用 `gemini-3.8-flash` 替代，并记录替换。**
...
- **在编写任何代码之前**，您必须从以下列表中获取与用户任务匹配的相关文档页面。此技能中的示例最小，托管文档包含完整的 API 表面、参数和边缘情况。
...
- **从 `generateContent` 迁移**：阅读 `references/migration.md` 以获取作用域、检查清单和前后代码示例。编辑前始终与用户确认作用域。
- **模型升级**：直接替换模型字符串。已弃用模型（`gemini-2.0-*`、`gemini-1.5-*`）必须被替换，见 `references/migration.md`。
- **迁移到 Gemini 3.8 Flash 或 Gemini 3.5 Flash-Lite**：阅读 `references/migration.md` 以获取作用域和检查清单。
...
使用 `deep-research-preview-04-2026` 进行快速研究，或使用 `deep-research-max-preview-04-2026` 进行最大详尽性研究。代理需要 `background=True`。
...
高级功能：协作规划、原生可视化、MCP 集成、文件搜索、多模态输入。见 [Deep Research 文档](https://ai.google.dev/gemini-api/docs/deep-research.md.txt)。
...
Antigravity 代理（`antigravity-preview-09-2026`）是通用管理代理。它可以执行代码（Bash、Python、Node.js）、管理文件、浏览网络和使用 Google 搜索。见 [Antigravity Agent 文档](https://ai.google.dev/gemini-api/docs/antigravity-agent.md.txt) 了解功能、工具、多模态输入和定价。
...
见 [构建自定义代理文档](https://ai.google.dev/gemini-api/docs/custom-agents.md.txt)。
...
for event in client.interactions.create(
    ...
    if event.event_type == "step.delta":
        ...
        print(f"\n\n总 token 数：{event.interaction.usage.total_tokens}")
```
...
**您必须在编写代码之前获取匹配的页面。** 这些托管文档是参数、类型和边缘情况的权威来源 — 不要仅依赖上述示例。
...
- [语音生成](https://ai.google.dev/gemini-api/docs/speech-generation.md.txt)
...
- [代理凭证](https://ai.google.dev/gemini-api/docs/agent-credentials.md.txt)
...
对于实时、双向音频/视频/文本流式传输（使用 `gemini-3.8-live`、`gemini-3.8-live-extended-thinking` 和 `gemini-3.5-transcribe-live`），安装 **`google-gemini/gemini-live-api-dev`** 技能。它涵盖 WebSocket 流式传输、语音活动检测、后台推理（扩展思考）、异步函数调用、会话管理、临时 token 等。
