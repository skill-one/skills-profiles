...
此技能提供了一个结构化的工作流程，用于将 Figma 设计转换为具有像素级精度的生产就绪代码。它确保与 Figma MCP 服务器的持续集成、设计令牌的正确使用以及与设计的 1:1 视觉一致性。
...
- **或者** 当使用 `figma-desktop` MCP 时：用户可以直接在 Figma 桌面应用程序中选择节点（无需 URL）
...
**注意：** 基于选择的提示仅适用于 `figma-desktop` MCP 服务器。远程服务器需要链接到框架或图层以提取上下文。用户必须打开 Figma 桌面应用程序并选择一个节点。
...
力求与 Figma 设计实现像素级完美的视觉一致性。

**指南：**

- 优先考虑 Figma 的保真度以完全匹配设计
- 避免硬编码值 - 在可用的情况下使用 Figma 的设计令牌
...
**原因：** 设计过于复杂或具有过多的嵌套图层，无法在单个响应中返回。
...
**解决方案：** 验证 Figma MCP 服务器的资源端点是否可访问。服务器在 `localhost` URL 上提供资源。直接使用这些 URL 而无需修改。
...
**对于设计师：** 确保 实现 将与他们的设计具有像素级精度。
**对于开发人员：** 提供一个结构化的方法，消除猜测并减少来回修改。
**对于团队：** 保持设计系统完整性的一致、高质量实现。
