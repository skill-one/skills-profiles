...
- IMPORTANT: 如果 Figma MCP 服务器返回图像或 SVG 的本地源，请直接使用该源
...
```markdown
---
description: 使用 Figma MCP 服务器实现 Figma 设计的规则。涵盖组件组织、样式约定、设计令牌、资源处理和 Figma 到代码的工作流程。
...
自定义 `globs` 模式以匹配 Figma 源代码将在项目中存在的目录（例如，`"src/**/*.tsx"` 或 `["src/components/**"，"src/pages/**"]`）。
...
写：“始终使用来自 `src/components/ui/Button.tsx` 的 Button 组件，带有 variant 属性（'primary' | 'secondary' | 'ghost'）”
...
创建良好规则的时间投资在每次 Figma 实现任务中都会成倍回报。
