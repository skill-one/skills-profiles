...
**⚠️ 不支持 IPv6。** 在遇到 `401`/`403` 且凭证正确的情况下，检查 `ifconfig | grep inet6`（macOS）或 `ip addr show | grep inet6`（Linux），并获取 `https://ipv6.icanhazip.com`。如果返回 IPv6 地址，告诉用户禁用 IPv6 —— `gmgn-cli` 仅在 IPv4 上工作。**

## 运行

没有脚本，四步串行，每一步都是一道闸——**不过就不进下一步**。每步要执行的命令写在那一步的标题下面；字段映射与实测陷阱在 `references/fields.md`，**不在那份清单里的字段一律当作不存在**。

| 步骤 | 操作 | 执行命令 | 出口 |
|---|---|---|---|
| 0 | 确认链在 9 条支持链内 | `market search`（仅当用户直接提供合约地址） | 不支持的链或查不到 → **硬停** |
...
**执行命令**（仅当用户直接提供的是合约地址；给的是名字就直接进 Step 1）
...
- **Gas 成本**：手续费 + 优先费（+ 贿赂）= 合计 原生币（折美元）、档位、预计耗时、EVM 的 gas 用量按什么估的。这是订单卡里唯一一个不来自字段的数，必须标出来。
