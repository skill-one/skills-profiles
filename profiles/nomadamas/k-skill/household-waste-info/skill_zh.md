...
如果 `npx` 不可用，请安装 Node.js 18+ 或遵循 https://github.com/NomaDamas/k-skill#readme，或阅读源代码说明 https://github.com/NomaDamas/k-skill/blob/main/household-waste-info/instruction.md。

## 即使没有 CLI 也必须遵守的硬性规则

- 未经用户事先明确批准，绝不执行支付、消息/邮件发送、最终提交、取消或公开发布操作。
- 绝不索取、打印或存储明文凭证，无论是在聊天、文件还是 shell 参数中。
