...
belt app run falai/flux-dev-lora --input '{
  "prompt": "日落时分宁静的山湖，雪山倒映在平静的水面，黄金时刻的光线，风景摄影",
  "width": 1248,
  "height": 832
}'
...
| 模型 | App ID | 适合场景 | 动画风格 |
|-------|--------|----------|-------------|
| **Wan 2.5 i2v** | `falai/wan-2-5-i2v` | 真实动态，自然运动 | 照片级真实，微妙 |
| **WAN-I2V (Pruna)** | `pruna/wan-i2v` | 经济高效，480p/720p | 自然，高效 |
...
| 场景 | 最佳模型 | 原因 |
|----------|-----------|-----|
| 水景带水/云 | **Wan 2.5 i2v** | 最擅长自然真实动态 |
| 人像带微妙表情 | **Wan 2.5 i2v** | 保持面部保真度 |
| 产品带布料 | **Fabric 1.0** | 专精于材料物理 |
| 旗帜飘动，窗帘流动 | **Fabric 1.0** | 布料模拟 |
| 插画/艺术图像 | **Seedance 2.0** | 符合风格化内容 |
| 通用"赋予生命" | **Seedance 2.0** | 全能型，最高1080p |
| 快速测试/迭代 | **Seedance 2.0 Fast** | 更快生成 |
...
AI视频模型在**轻柔、微小的动态**下效果优于戏剧性动作。请求过多运动会导致失真和伪影。
...
# 景观动画
belt app run falai/wan-2-5-i2v --input '{
  "prompt": "缓慢向右摇移，水面倒映移动的云朵，树木轻微随风摇曳，温暖金色光线，宁静缓慢",
  "image": "landscape.png"
}'

# 人像动画
belt app run falai/wan-2-5-i2v --input '{
  "prompt": "轻微呼吸运动，轻微转头，自然眨眼，头发轻柔移动，环境光线变化",
  "image": "portrait.png"
}'

# 产品拍摄动画
belt app run bytedance/seedance-2-0 --input '{
  "prompt": "缓慢360度环绕产品，轻柔聚光灯移动，轻微反光变化，高端产品展示，平滑运动",
  "image": "product.png",
  "generate_audio": true
}'
...
belt app run falai/fabric-1-0 --input '{
  "prompt": "布料在轻柔风中流动和涟漪，自然布料物理，轻柔运动",
...
belt app run bytedance/seedream-4-5 --input '{
  "prompt": "电影感景观，黎明时分的雾气山脉，前景湖面，戏剧性云层，黄金时刻，4K质量，专业摄影",
  "size": "2K"
}'
...
动态照片是一张静态照片，其中只有一个元素在动（例如，冻结场景中的瀑布运动）。要实现这一点：
...
```bash
belt app run falai/wan-2-5-i2v --input '{
  "prompt": "只有瀑布在运动，其他一切保持完美静止，水流顺畅，场景其他部分冻结",
