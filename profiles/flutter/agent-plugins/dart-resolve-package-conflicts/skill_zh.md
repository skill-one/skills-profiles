...
Dart对依赖项执行严格的单版本规则：一个项目和所有其传递依赖项都必须解析为任何给定包的单一、共享版本。这防止了运行时类型不匹配，但引入了“版本锁定”的风险。
...
*   **可解析：** 考虑项目中所有其他依赖项后，可以解析的绝对最新版本。
*   **最新：** 包的最新发布版本（不包括预发布版本）。

## 版本约束

*   **使用尖号语法：** 在`pubspec.yaml`中始终使用尖号语法（例如，`^1.2.3`）来指定依赖项。这允许`pub`在解析期间选择更新的、非破坏性版本（最高到但不包括下一个主版本）。
*   **收紧开发依赖项：** 将`dev_dependencies`的下限设置为当前使用的确切版本。这减少了解析复杂性，并防止选择较旧、不兼容的开发工具。
*   **在CI中强制锁定文件：** 在CI/CD管道中使用`dart pub get --enforce-lockfile`，以确保本地测试使用的确切版本在生产环境中使用。
...
- [ ] 查看可升级列，识别无需修改`pubspec.yaml`即可更新的包。
- [ ] 查看可解析列，识别需要修改`pubspec.yaml`中的约束才能更新的包。
- [ ] 识别任何被撤回或停止维护的包。
...
- [ ] **如果更新到“可升级”版本：**
  - [ ] 运行`dart pub upgrade`。
  - [ ] 运行`dart pub upgrade --tighten`以自动更新`pubspec.yaml`中的下限，以匹配新解析的版本。
- [ ] **如果更新到“可解析”版本（主版本更新）：**
  - [ ] 手动编辑`pubspec.yaml`，将版本约束提升到与“可解析”列匹配（例如，将`^0.11.0`更改为`^0.12.1`）。
...
当`pub`找不到满足所有约束的特定版本集，或处理被撤回的包版本时，需要外科手术式地操作锁定文件。
...
如果`package_a`被撤回或在冲突中被锁定，从`pubspec.lock`中仅删除其块。
