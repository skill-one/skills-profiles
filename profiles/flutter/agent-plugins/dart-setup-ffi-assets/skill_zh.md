...
2.  `hook/link.dart`：链接构建的代码资源，应用高级的树摇优化以剥离未使用的原生符号并压缩运行时二进制文件的大小。
...
*   **钩子位置**：编译和打包钩子必须严格位于包根目录下的 `hook/` 目录内：
...
由于 `package:native_toolchain_c` 将实际动态编译委托给主机操作系统的默认工具链，开发机器**必须**预装以下编译器包之一：
...
使用 `CLibrary.build` 实现 `hook/build.dart`。这会将库构建为动态库（例如 `.so`、`.dylib` 或 `.dll`）在钩子的目标目录中：
...
*   **编译速度**：预编译下载以毫秒为单位执行，而本地编译可能需要长达数分钟的多分钟编译过程。
...
### 2. 验证目标输出
导航到您的包目标目录，并确认为主机系统创建了动态二进制资源：
*   **macOS**：确认 `.dart_tool/resources/` 或目标目录包含 `.dylib` 文件。
*   **Linux**：确认 `.dart_tool/resources/` 或目标目录包含 `.so` 文件。
*   **Windows**：确认 `.dart_tool/resources/` 或目标目录包含 `.dll` 文件。
...
3. 查询导出的动态符号表：
   ...
   *   **Windows**（使用 MSVC 开发者命令提示符）:
       ...
       ```
4. **确认目标导出**：确认命令输出仅包含显式保留的入口点函数（例如 `sqlite3_libversion`），并且不输出任何未引用/剥离的符号。
5. **无捆绑场景**：如果应用程序没有导入或调用任何原生库的方法：
   - 确认链接钩子记录：`Skipping linking as no symbols are to be kept.`
   - 确认没有库被构建/放置在生产捆绑包中（没有生成 `.dylib`/`.so`/`.dll` 文件，从而节省捆绑包大小）。
...
3. 启动单元测试：
   ...
   dart test
   ```
4. 确认测试套件成功使用主机编译器编译本地源文件，没有编译错误，并且永远不会尝试网络下载请求。
