...
使用标准的Dart测试范例来组织你的测试套件。对于Dart项目使用`package:test`，对于Flutter项目使用`flutter_test`。
...
### 1. 运行带有VM服务的测试
执行测试时暂停隔离在退出时，并在特定端口（例如，8181）上暴露VM服务。
...
```
*可选：附加`--function-coverage`和`--branch-coverage`以收集更深层次的指标（需要Dart VM 2.17.0+）。*
