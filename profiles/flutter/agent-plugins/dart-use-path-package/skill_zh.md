...
* **POSIX目标的规则**：优先选择**两参数边界连接**（`p.join(home, '.local/share/app/bin/config.json')`）。这可以防止在变量边界处出现重复斜杠错误（`//`），同时保持单行可读性和精确字符串可搜索性。

### 规范化与规范化(`p.normalize`与`p.canonicalize`)
* `p.normalize(path)`纯粹按字典顺序解析`.`和`..`段，而无需查询文件系统或标准化大小写。
...
* 通过正则表达式（`RegExp(r'^(.*?):(\d+(?:-\d+)?)$')`）提取尾随的`:行列`后缀，*然后再*将文件路径传递给`package:path`。
...
### Git路径与仓库元数据
* Git仓库树对象、`.gitignore`模式规则、`.gitattributes`以及git跟踪的符号链接严格使用POSIX正斜杠（`/`），即使在Windows上也是如此。
* 将原生Windows反斜杠（`\`）插入`.gitignore`或git命令会导致Git将`\`视为转义字符而不是目录分隔符，从而静默地破坏模式匹配。
...
在使用`package:file`（例如CLI应用程序或使用`MemoryFileSystem`测试的服务）的代码库中，避免在`File`或`Directory`路径上调用顶层`p.*`函数。

* 顶层`p.*`函数绑定到*正在运行测试的主机操作系统*。
* 如果单元测试在Linux或macOS运行器上创建了一个`MemoryFileSystem(style: FileSystemStyle.windows)`，全局`p.split(file.path)`将使用`/`而不是`\`进行分割，从而破坏测试。
...
避免使用`.lastIndexOf('.')`和`.substring()`算术来提取文件扩展名或插入内容哈希。`p.extension`原生支持通过其可选`level`参数进行多级扩展。

* **多点词干细微差别**：调用`p.extension('main.dart.wasm', 2)`返回`'.dart.wasm'`，因为它盲目地捕获最后两个点分隔的段。当对可能具有多级词干（例如`main.dart.wasm`与`main.dart.js.map`）的文件进行哈希或移除扩展名时，检查`p.extension(filename, 2)`是否匹配已知的复合扩展（或`.endsWith('.map')`）之前，再回退到单级`p.extension(filename)`：
