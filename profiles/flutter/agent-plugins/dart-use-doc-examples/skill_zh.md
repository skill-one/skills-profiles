...
当编写需要多行代码示例的文档时，通常应将这些示例提取到独立的 `.dart` 文件中，并使用 `{@example}` 指令注入，而不是在 `///` 注释中内联编写。这确保了示例可以被分析、检查并执行。
...
*   **`lang`**：Markdown 围栏的语言。从文件扩展名自动检测（例如，`dart`），但可以显式指定（例如，`lang=text`）。
...
## 2. 使用区域
通常，外部示例文件包含导入、设置或 `void main()` 包装器，您不希望在文档中显示这些内容。您可以通过在 `{@example}` 指令路径中追加 `#<区域>` 来提取特定的代码块，并在目标文件中用 `#region` 和 `#endregion` 注释将该代码包裹起来。
...
## 3. 隐藏设置代码
如果您的提取区域中的特定代码行对编译器/分析器是必要的，但对文档阅读者无关紧要（或分散注意力），则在该行追加 `#hide`。
...
```
在生成的文档中，仅会显示 `final data = await fetch(mockServer.url);`。带有 `#hide` 的行会被完全移除。
...
*   **格式无关性**：标记系统是完全格式无关的。Dartdoc 只是运行正则表达式来移除包含标记字符串的行，这意味着它在非 Dart 文件中（例如，在 YAML 注释 `# #region` 或 HTML 注释 `<!-- #region -->` 中）工作方式完全相同。

## 5. 位置和路径解析
`{@example}` 指令是一个块级指令。它必须单独成行并以 `///` 开头。其内部 `<路径>` 解析器遵循严格的 URI 引用规则：
...
*   **边界强制**：使用 `..` 段向上遍历是完全可以接受的，但 dartdoc 原生地在包根目录处停止目录遍历（它永远不会离开包）。
*   **无网络 URL**：绝对 URI（例如，以 `https://` 开头的）严格不支持。示例文件必须原生地位于本地文件系统的某个位置。
*   **分隔符和编码**：因为 dartdoc 将路径解析为 URI，您必须始终使用正斜杠（`/`）作为文件夹分隔符（即使在 Windows 上也是如此）。您可以原生地包含 URI 编码字符（如 `%20` 用于空格），只要它们符合 URI 引用规则。
