...
一个现代化的、TypeScript优先的ORM，零依赖、编译时类型安全、SQL语法。针对边缘运行时和服务端无服务器环境进行了优化。
...
- **[🔍 查询模式](./references/query-patterns.md)** - 子查询、CTE、原生SQL、预编译语句、批量操作。优化查询或处理复杂过滤时加载。

- **[⚡ 性能](./references/performance.md)** - 连接池、查询优化、N+1预防、预编译语句、边缘运行时集成。扩展或优化数据库性能时加载。

- **[🔄 vs Prisma](./references/vs-prisma.md)** - 功能对比、迁移指南、何时选择Drizzle而非Prisma。评估ORM或从Prisma迁移时加载。

## 警示

**如果出现以下情况请停止并重新考虑：**
- 对JSON列使用`any`或`unknown`而不带类型注解
- 不使用`sql`模板构建原生SQL字符串（SQL注入风险）
- 不使用事务进行多步骤数据修改
- 生产查询中不分页获取所有行
- 外键或频繁查询列缺少索引
- 大表查询时未指定列使用`select()`
...
- **typescript**: 高级TypeScript模式：类型安全的查询
- **nextjs**: Drizzle与Next.js服务器动作和API路由
- **sqlalchemy**: SQLAlchemy模式：Python开发者学习Drizzle

[如果部署在您的包中，这些技能提供完整文档]
