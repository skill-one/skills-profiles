...
现有的工作实现包括：沙盒化/按用户代码执行、可由代理加载的 `SKILL.md` 技能、跨会话内存、风险操作前的审批门、工具护栏、按用户凭证以及计划/事件驱动运行。

**Go — 阅读上游的 [`examples/`](https://github.com/google/adk-go/tree/main/examples)。**
克隆仓库并在实现它之前阅读与所需功能匹配的示例。
...
2. 新项目：运行 `agents-cli scaffold create <name>` — 添加 `--agent adk_go` 以使用 Go。
3. 现有代码：运行 `agents-cli scaffold enhance .`。

## 快速参考 — 最常见模式

有关 Python 和 Go 代码示例，请参阅 `references/adk-python.md` 和 `references/adk-go.md`。

---

## 参考

使用速查表以了解常见模式。若要深入了解，请获取文档索引或检查已安装的包。

| 参考 | 语言 | 阅读时间 |
|------|------|-------------|
| `references/samples.md` | Python | **ADK 参考配方按主题索引的目录。** 在工作流第一阶段（在脚手架之前和编写代码之前）阅读 — 将功能映射到实现它的配方。 |
...
| `references/adk-python-live.md` | Python | 实时和语音代理：`Runner.run_live`、`LiveRequestQueue`、模型和区域可用性、语音/`speech_config`、VAD、会话限制、`/run_live` 服务。用于实时语音或视频代理。 |
| `references/adk-go.md` | Go | 核心 ADK Go API：`llmagent`、工具、回调、插件、状态、工件、多代理系统、顺序/并行/循环代理、自定义代理、运行器、通过 HTTP 服务、A2A 协议和环境触发器。大多数代理的默认设置。 |
...
| `curl https://adk.dev/llms.txt` | Python | 文档索引（每页标题 + URL）。获取它，然后使用 `WebFetch` 获取特定页面以了解速查表之外的内容。 |
