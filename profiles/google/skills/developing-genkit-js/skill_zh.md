...
`.prompt` 文件将提示内容与代码分离，使用 YAML 前置部分和一个 Handlebars 模板。参见 [Dotprompt](references/dotprompt.md)：`promptDir`、`ai.prompt()`（调用/流式传输/渲染）、变体、部分、通过 `ai.defineSchema` 的命名模式，以及 `tools`/`maxTurns`/`returnToolRequests`/`use`（中间件）前置部分字段。
...
Genkit 最近经历了一次重大破坏性 API 变更。您的知识已过时。您**必须**查阅文档。推荐：
...
5.  **确保正确性**：
    ...
    -   通过跟踪验证，而不是盲目运行。直接运行应用程序（`node`/`tsx`/`npm start`）**不会**捕获开发跟踪。参见 [CLI 使用](#cli-usage-recommended) 了解如何运行您的应用程序并捕获跟踪。
6.  **处理错误**：
...
`genkit start` 无干扰地包装任何使用 Genkit 库的 Node.js 程序，运行应用程序本身不变，同时捕获每个 Genkit 操作的跟踪，以便您可以从终端**证明工具实际上被调用并检查模型 I/O**。它转发 stdio，因此依赖 stdin/stdout 的交互式 CLI 工具可以正常工作。直接运行应用程序（`node`/`tsx`/`npm start`）会跳过跟踪捕获，因此您是在盲目调试。

**主要模式（默认）**：在您的正常运行命令前缀 `genkit start --`。这将收集您的程序运行的所有 Genkit 代码的遥测数据，无论是由开发 UI、您的 Web 服务器/Web UI 还是普通脚本触发：
```bash
genkit start -- npx tsx --watch src/index.ts
genkit start --noui -- npx tsx src/index.ts   # 相同，不显示开发 UI（仍然是一个持久的服务器）
```
`genkit start` 运行，直到您使用 Ctrl+C 停止。这对于常见情况（您的 Web/移动应用程序调用的服务器，或您自己退出的交互式 CLI）是预期且正确的。`--noui` 仅丢弃开发 UI；它**不是**一个一次性命令，并且不会自行退出。**不要**在自动化/非交互式环境中使用 `genkit start` 作为阻塞步骤。

**非交互式使用（代理/CI）**：在 `--` 之前添加全局 `--non-interactive` 标志，以便 CLI 使用默认值并且永远不会在提示上阻塞（例如首次运行的分析通知）：`genkit start --non-interactive -- npx tsx src/index.ts`（与 `flow:run` 也适用）。

**运行流程（`flow:run`）**：从 CLI 调用特定流程。在 `--` 后附加您的运行命令以启动仅为此运行而运行的时间（命令按原样运行以注册您的流程）：
```bash
genkit flow:run myFlow '{"data": "input"}' -- npx tsx src/index.ts
```
这是**自终止的**：它运行一次流程，打印一个 `Trace ID`，然后退出（使用 `genkit trace:get <id>` 查看它）。这使得它成为快速、非交互式检查的正确选择，必须自行退出，而不会阻塞 `genkit start` 或直接运行应用程序（后者会跳过跟踪）。始终显式传递输入 JSON：`flow:run` 在省略时发送 `undefined`，并且**不会**回退到模式 `.default()`。注意：`flow:run` 运行的是流程（`ai.defineFlow`），而不是代理（`ai.defineAgent`）；您不能直接 `flow:run` 代理。要从 CLI 运行代理，请将一个回合包装在一次性流程中并运行该流程（参见 [代理](references/agents.md)）。
...
对于机器可读输出，传递 `--format json` 以获取干净的 JSON，您可以将其传递给 `jq` 或其他解析器。**默认**输出是面向人类的（横幅/日志行，大型跟踪可能被截断），因此不要直接传递这种形式；使用 `--format json`、grep 或开发 UI 跟踪查看器。
