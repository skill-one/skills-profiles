...
1.  **识别主要数据源**：快速判断用户需要防火墙日志、威胁日志、Cloud NAT、VPC 流日志还是指标。
2.  **执行并呈现**：执行最低限度的查询以获得直接答案。
3.  **最终终止**：一旦识别出请求的数据，无论其值（包括 0、null 或“无流量”），都应呈现发现结果并在同一轮次中调用完成工具。除非特别指示需要排查预期繁忙的资源，否则不要试图寻找“活跃”或“更繁忙”的资源以提供“更好的”答案。

## 日志与遥测概述

-   **威胁日志**：来自 Cloud Firewall Plus 和 Cloud IDS 的专业日志，通过深度包检测识别恶意流量模式（例如，SQL 注入或恶意软件）。
-   **VPC 流日志**：捕获流向和流离网络接口的样本 IP 流量。用于流量分析、量级趋势和顶级流量源。
...
-   **连接性测试**：用于路径诊断的静态分析工具。用于识别端点之间的防火墙或路由配置错误。
...
-   **元数据感知（BigQuery）**：子网可能配置了 `EXCLUDE_ALL_METADATA`，导致 VPC 流日志中的 VM 名称显示为 NULL。如果按 VM 名称查询无结果，请重试使用内部 IP 地址（`jsonPayload.connection.src_ip`）。
...
2.  **干运行**：在执行修正后的查询之前，使用 `bq query --use_legacy_sql=false --dry_run "{query_text}"` 验证字段引用，而不会产生成本或执行时间。
...
-   **始终**在识别出直接答案后立即呈现。
-   **绝不**在显示结果之前运行超过 2 个探索性查询。
-   **绝不**在未经明确用户许可的情况下执行二次验证（例如，在发现防火墙阻止后不要检查 VPC 流量）。
...
-   **绝不**在主要数据源（例如，Cloud Monitoring 指标）已提供结论性答案的情况下查询第二个数据源（例如，BigQuery 日志）。**不要**比较指标和日志以“验证”准确性，除非用户特别询问它们为何不同。
-   **无差异循环**：如果 Tool A 提供结果（例如，80,000 计数）而 Tool B 提供不同结果（例如，1,000 计数），**不要**启动深入调查以解释差异。呈现主要工具的结果并停止。
-   **始终**在第一轮次中执行时间范围计算（例如，“12 小时前”）以节省步骤。
-   **最终接受无活动**：将“0”、“无流量”、“未找到数据”或“未找到记录”的结果视为在请求的时间范围内和资源上的结论性发现。你必须将其报告为最终状态并立即终止。
-   **标准化发现路径**：对于所有“Top-N”或基于量的发现任务（例如，“最高流量”、“最多命中”、“顶级流量源”），你必须使用 _AllLogs 数据集的 BigQuery 聚合。禁止使用 Monitoring API 手动聚合单个时间序列点，因为步骤效率低下。
-   **禁止辅助脚本**：执行所有数据检索和解析逻辑作为直接工具调用（bq、curl、gcloud）。**不要**编写或执行本地 shell 脚本（.sh）或 python 文件，因为它们会引入可避免的环境和权限错误，导致调查超时。
-   **发现效率**：对于量级分析（例如，“多少连接”或“按字节最多的 IP”），VPC 流日志（_AllLogs）的 BigQuery 聚合是**事实来源**。如果 BigQuery 数据可用，它就是结论性的。**不要**查询 Monitoring API 以“双重检查”BigQuery 计数。
