...
本技能为单例开发者提供了一个简化的、非交互式的“顺利路径”，以开始使用 [Google Cloud](https://cloud.google.com/)。它涵盖了从环境验证和身份验证到项目选择、计费账户链接以及下游安全链的所有内容。

> [!IMPORTANT]
> 对于执行此技能的自主代理：
> 1. **变更前检查审计**：在提议或执行任何项目或计费更改之前，始终执行静默的预执行状态审计。
> 2. **单问题策略**：在交互式执行期间，一次只向用户询问一个操作参数或确认。
> 3. **非交互式输出**：将非交互式覆盖（`--quiet`，`--format="json"`）附加到所有变更命令中，以确保确定性、机器可解析的输出并防止终端挂起。
> 4. **首次交互规则（触发轮次）**：当开发者使用通用入门请求（例如说“我想开始使用 Google Cloud”）首次触发此技能时：
>    - **引导性前缀指导**：主动包含一个简短的引导性前缀，指导开发者创建 Google Cloud 账户（指向控制台 `https://console.cloud.google.com/`）并运行 `gcloud auth login` 以授权他们的工作站，即使他们看起来已经登录。
>    - **首次交互单问题**：执行静默的预飞行审计，但不要在首次交互中呈现完整的参数摘要表或请求最终同意。相反，向开发者提出一个初始操作问题（例如，*"您希望重用现有的活动项目，还是创建一个全新的项目？"*）。
...
对于个人开发者，入门 Google Cloud 涉及验证本地终端工具、建立经过身份验证的会话、选择或实例化工作区（[项目](https://docs.cloud.google.com/resource-manager/docs/cloud-platform-resource-hierarchy.md.txt)），并将其链接到活动计费账户。Google Cloud 为首次用户提供了免费层和 300 美元的免费试用。[在此了解更多](https://docs.cloud.google.com/free/docs/free-cloud-features)。
...
1. 检查 `gcloud` CLI 二进制文件是否已安装并可访问：
   ...
   which gcloud
   ```
2. 检查是否存在活动的经过身份验证的会话：
   
   ```bash
   gcloud auth list --format="json"
   ```
3. 如果 `which gcloud` 的预执行审计返回有效路径，则直接进入第 2 部分：身份验证和会话路由。
4. 如果二进制文件缺失，则停止执行，并指示代理使用 [gcloud 技能](https://github.com/google/skills/tree/main/skills/cloud/gcloud) 安装 gcloud。向开发者提供官方 [Google Cloud CLI 安装指南](https://docs.cloud.google.com/sdk/docs/install-sdk) 以获取设置身份验证的步骤。
...
1. **执行凭证身份验证**：
   ...
   > [!IMPORTANT]
   > **新用户/未身份验证指导**：
   > 如果预执行状态审计或命令失败确认开发者未经过身份验证（例如，`gcloud auth list` 为空或缺少活动凭证）：
...
Google Cloud 资源按 **项目** 组织。当开发者在控制台注册免费试用时，Google Cloud 会自动创建一个默认项目（例如，“我的第一个项目”）。始终首先审计活动环境，以重用现有项目并防止令牌燃烧冲突错误。
...
要在 Google Cloud 上部署资源，您的项目必须链接到一个活动的 Cloud 计费账户。
...
3. **发现可用计费账户**：
   如果项目未链接，查询与经过身份验证的用户身份链接的可用计费账户句柄：
...
入门设置现已完成。为了保护您的环境并部署工作负载，您可以链接到下游的专业技能：
