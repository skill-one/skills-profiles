...
在采取行动之前，先运行 CodeStable 的预检：阅读 `.codestable/attention.md`（如果缺失，先运行 `cs-onboard`）；不要用 `AGENTS.md`/`CLAUDE.md` 等外部入口来代替它；详细规则见 `.codestable/reference/execution-conventions.md`。

`.codestable/requirements/` 是项目的"能力清单"——每份文档描述**一个能力因什么问题而产生、怎么解决、边界在哪**，用通俗易懂的语言，非技术读者也能看懂。架构文档讲"怎么搭"，需求文档讲"为什么要有"。
...
- feature-acceptance 第 6 节触发：draft req 对应的能力实现完成 → `update` 升级为 `current`（保留愿景追加变更日志）；从未写过 req 的已存在能力 → `backfill` 直接落 `current`；已有 current req 的能力改了边界 / 用户故事 / pitch → `update` 刷新
