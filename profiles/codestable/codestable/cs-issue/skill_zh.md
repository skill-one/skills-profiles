...
要彻底诊断问题；在获得修复授权后，再修复根本原因并证明其已修复。

## 开始工作

- 如果有 `.codestable/attention.md`，先阅读。
- 根据错误信息和相关模块关键词，检索 `.codestable/lessons/`、项目文档，以及现有的 v1 只读知识目录：`.codestable/roadmap/`、`.codestable/features/`、`.codestable/issues/`、`.codestable/refactors/`、`.codestable/goals/`、`.codestable/compound/`、`.codestable/audits/`、`.codestable/brainstorms/`、`.codestable/feedback/`；这个坑可能已经有人踩过，如果命中要报告来源路径。上述 v1 目录不得继续生成、原地改写或批量迁移，新结论按归属进入 v2 Epic、项目文档、ADR 或 lesson。
