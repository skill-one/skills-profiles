...
将一个功能开发出来，流程强度与风险相匹配：普通改动直接进行，高风险改动需先对齐设计。

## 开工

- 有 `.codestable/attention.md` 就先阅读。
- 按功能关键词检索 `.codestable/lessons/`、项目文档，以及现有的 v1 只读知识目录：`.codestable/roadmap/`、`.codestable/features/`、`.codestable/issues/`、`.codestable/refactors/`、`.codestable/goals/`、`.codestable/compound/`、`.codestable/audits/`、`.codestable/brainstorms/`、`.codestable/feedback/`；命中需报告来源路径。上述 v1 目录不得继续生成、原地改写或批量迁移，新结论按归属进入 v2 Epic、项目文档、ADR 或 lesson。
