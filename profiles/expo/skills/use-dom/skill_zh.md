...
DOM 组件允许 Web 代码在原生平台的 WebView 中按原样运行，同时在 Web 上原样渲染。这使得您可以在 Expo 应用中使用 Web 专用库（如 `recharts`、`react-syntax-highlighter` 或任何 React Web 库），而无需修改。

## 何时使用 DOM 组件

当您需要以下功能时，请使用 DOM 组件：

- **Web 专用库** — 图表（recharts、chart.js）、语法高亮器、富文本编辑器或任何依赖 DOM API 的库
...
- **原生性能至关重要** — WebView 会增加额外开销
- **简单 UI** — React Native 组件对于基本布局更高效
- **深度原生集成** — 使用本地模块代替以访问原生 API
...
4. **仅可序列化属性** — 字符串、数字、布尔值、数组、普通对象
5. **在组件文件中包含 CSS** — DOM 组件在隔离上下文中运行
...
由于它们在隔离上下文中运行，CSS 导入必须在 DOM 组件文件中：
...
export default function Component({
  ...
  return <div>{IS_DOM ? "在 DOM 组件中运行" : "原生运行"}</div>;
}
```
