...
代理 / 非交互式 Shell：`belt login --key <api-key>`，或设置 `INFSH_API_KEY`（每个命令都会读取，无需登录）。
在没有 TTY 的情况下，`belt login` 会打印一个 URL；浏览器随后会显示一个认领码。在 5 分钟内，在同一台机器上运行 `belt login --code <code>`。

> **安装程序做什么？** [安装脚本](https://cli.inference.sh) 会检测您的操作系统和架构，从 `dist.inference.sh` 下载正确的二进制文件，验证其 SHA-256 校验和，并将其放置在您的 PATH 中。就是这样——无需提升权限，无需后台进程，无需遥测数据。如果您安装了 [cosign](https://docs.sigstore.dev/cosign/system_config/installation/)，安装程序会自动验证 Sigstore 签名。
>
> **手动安装**（如果您不想将内容管道到 sh）：
> ```bash
> # 下载二进制文件和校验和
...
# 发布到 Twitter
belt app run x/post-create --input '{"text": "来自 AI 的问候！"}'
...
| 不等待运行 | `belt app run <app> --input input.json --no-wait` |
...
| **视频** | Veo 3.1, Seedance 1.5, Wan 2.5, OmniHuman, Fabric, MMAudio |
...
# LLMs (Claude, Gemini, Kimi, 通过 OpenRouter 的 GLM)
npx skills add inference-sh/skills@llm-models

# 网络搜索 (Tavily, Exa)
npx skills add inference-sh/skills@web-search

# AI 虚拟形象 & 唇形同步 (OmniHuman, Fabric, PixVerse)
...
- [代理技能概述](https://inference.sh/blog/skills/skills-overview) - AI 能力的开放标准
- [入门](https://inference.sh/docs/getting-started/introduction) - inference.sh 简介
...
- [工作流 vs 代理](https://inference.sh/blog/concepts/workflows-vs-agents) - 何时使用每个
- [代理运行时的重要性](https://inference.sh/blog/agent-runtime/why-runtimes-matter) - 运行时优势
