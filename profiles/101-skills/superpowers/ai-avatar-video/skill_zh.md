...
**从 P-Video-Avatar 开始** — 它比其他替代方案快 18 倍，便宜 6 倍，内置 TTS、动态背景和 1080p 支持。
...
| PixVerse Lipsync | `falai/pixverse-lipsync` | 高度逼真的口型同步 | 否 |
...
| OmniHuman 1.5 | ~28s/s (15 倍慢) | $0.16 (6.4 倍贵) |
| Fabric 1.0 | ~34s/s (18 倍慢) | $0.14 (5.6 倍贵) |

## 示例

### P-Video-Avatar（推荐）

从肖像 + 文本脚本生成头像，并使用内置 TTS：
...
对于没有内置 TTS 的模型（OmniHuman、PixVerse），首先生成语音：

```bash
# 1. 生成语音 — Inworld TTS-2 用于表达性角色声音
belt app run inworld/text-to-speech-2 --input '{
  "text": "[友好] 欢迎来到我们的产品演示！[兴奋] 我将向您展示三个将改变您工作方式的功能。",
  "voice_id": "Sarah",
...
belt app run bytedance/omnihuman-1-5 --input '{
  ...
  "audio_url": "<步骤 1 中的 audio-url>"
}'
```

> **提示**：对于大多数用例，使用内置 TTS 的 P-Video-Avatar 更简单 — 无需单独的音频步骤。仅在您需要 OmniHuman（多角色）或 PixVerse（逼真口型同步）时使用此工作流程。
...
```bash
# 1. 生成 relatable UGC 风格肖像
belt app run pruna/p-image --input '{
  "prompt": "休闲自拍式照片，年轻女性，在舒适的房间内，自然光线，看向镜头，温暖的微笑，真实感",
  "aspect_ratio": "9:16"
}'
...
for voice in "Zephyr (Female)" "Puck (Male)" "Aoede (Female)"; do
  belt app run pruna/p-video-avatar --input "{
    \"image\": \"https://portrait.jpg\",
    \"voice_script\": \"这彻底改变了我的早晨习惯。五分钟就完成了。\",
    \"voice\": \"$voice\",
    \"voice_prompt\": \"休闲、真实，像真实的证言\",
    \"video_prompt\": \"人在明亮的厨房里对着镜头说话\",
    \"resolution\": \"1080p\"
  }"
done
```
...
- **UGC 与营销**：产品演示，UGC 风格的广告，AI 演讲者
- **教育**：课程视频，解释视频
- **本地化**：从一张图像将内容配音到 10 种语言
- **社交媒体**：一致的虚拟影响者内容
- **企业**：培训视频，公告
- **游戏**：角色头像，NPC 对话
...
- 使用 `pruna/p-image` 生成肖像，使用 `9:16` 宽高比生成垂直视频
- OmniHuman 1.5 支持一张图像中的多个人物
...
- [实时更新结果](https://inference.sh/docs/api/sdk/streaming) - 实时进度更新
