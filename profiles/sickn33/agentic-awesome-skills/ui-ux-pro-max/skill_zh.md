...
- `color-contrast` - 正常文本的对比度至少为4.5:1
...
- `touch-target-size` - 最小44x44px的触摸目标
...
**始终使用`--design-system`**以获取带推理的全面建议：
...
| `style` | UI风格、颜色、效果 | glassmorphism, minimalism, dark mode, brutalism |
...
| `prompt` | AI提示、CSS关键词 | (style name) |

### 可用技术栈

| 技术栈 | 重点 |
|-------|-------|
| `html-tailwind` | Tailwind工具、响应式、a11y (默认) |
...
1. **关键词要具体** - "医疗保健SaaS仪表盘" > "app"
2. **多次搜索** - 不同关键词会揭示不同见解
3. **组合域** - 风格+排版+颜色=完整设计系统
...
| **稳定的悬停状态** | 使用颜色/透明度过渡悬停 | 使用会改变布局的缩放变换 |
| **正确的品牌标志** | 使用Simple Icons的官方SVG | 猜测或使用错误的标志路径 |
| **一致的图标尺寸** | 使用固定的viewBox（24x24）配合w-6 h-6 | 随机混合不同图标尺寸 |
...
| 规则 | 做 | 不要 |
|------|----|----- |
| **光模式玻璃卡片** | 使用`bg-white/80`或更高不透明度 | 使用`bg-white/10`（太透明） |
| **光模式文本对比度** | 使用`#0F172A`（slate-900）作为文本 | 使用`#94A3B8`（slate-400）作为正文 |
| **光模式弱文本** | 最小使用`#475569`（slate-600） | 使用gray-400或更浅 |
| **边框可见性** | 使用`border-gray-200`在光模式下 | 使用`border-white/10`（不可见） |
...
| **一致的容器宽度** | 使用相同的`max-w-6xl`或`max-w-7xl` | 混合不同容器宽度 |
