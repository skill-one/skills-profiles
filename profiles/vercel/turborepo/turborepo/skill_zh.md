...
**不要**在可以存在于包中的情况下，将任务逻辑放在根 `package.json` 中。这会破坏 Turborepo 的并行化。
...
根任务 (`//#taskname`) 仅用于那些确实无法存在于包中的任务，例如 Vitest 项目的 `//#test`、仓库范围的发布脚本，或不会调用 `turbo` 的工具。
...
**简写 `turbo <task>` 仅用于一次性终端命令**，由人类或代理直接输入。永远不要将 `turbo build` 写入 package.json、CI 或脚本。
...
**`--affected` 是运行仅更改包的主要方式。** 它与 `main` (回退到 `master`) 比较——不是仓库配置的默认分支——并包含依赖项。设置 `TURBO_SCM_BASE` 用于任何其他基础分支。
...
**推荐在 package.json 脚本和 CI 工作流中使用 `turbo run`。** 简写 `turbo <task>` 仅用于交互式终端使用。
...
根 `package.json` 脚本必须委托给 `turbo run`，而不是直接运行任务。
...
1. **如果依赖项已声明** (例如，在 package.json 中 `"@repo/types": "workspace:*"`)，则删除 `prebuild` 脚本。Turbo 的 `dependsOn: ["^build"]` 会自动处理。
...
使用 `futureFlags.globalConfiguration`，这个问题会减少，因为 `global.inputs` 文件被折叠到每个任务的输入中 (而不是全局哈希)。任务可以排除特定文件：
...
一个大型 `env` 数组 (即使 50+ 个变量) **不是**一个问题。这通常意味着用户在声明构建的环境依赖项方面非常彻底。不要将此视为问题。
...
Turborepo 不加载 `.env` 文件 - 你的框架会加载。但 Turborepo 需要知道有关更改的信息：
...
单一仓库根目录中的 `.env` 文件是一个反模式——即使是小型单一仓库或启动模板。它创建了包之间的隐式耦合，并使它不明确哪些包依赖于哪些变量。
...
- 不清楚哪些包消费哪些变量
- 所有包都会收到所有变量 (即使它们不需要)
- 缓存失效粒度粗粒度 (根 .env 变化使所有内容失效)
- 安全风险：包可能会意外访问其他包打算用于其他包的敏感变量
- 小错误会导致坏习惯——启动模板应该模仿正确的模式
...
如果你有需要并行执行并具有缓存失效的任务，请添加一个 `transit` 任务 (见下文).

### 具有 `^dev` 模式的开发任务 (用于 `turbo watch`)

根 `turbo.json` 中的 `dev` 任务具有 `dependsOn: ["^dev"]` 和 `persistent: false` 可能看起来不寻常，但这是**正确的 `turbo watch` 工作流**：
...
- **`turbo watch`** 重新运行包的 one-shot dev 脚本，以保持类型同步

**预期用法：** 运行 `turbo watch dev` (不要 `turbo run dev`). 监视模式在文件更改时重新运行 one-shot 任务，同时保持持久化任务运行。

**替代模式：** 使用 `prepare` 或 `generate` 这样的单独任务名称，以使意图更清晰：
...
`transit` 任务创建依赖关系，但与任何实际脚本不匹配，因此任务可以并行执行并具有正确的缓存失效。
...
使用 `futureFlags.globalConfiguration`，相同的配置移动到 `global` 下——`.env` 成为每个任务的输入，而不是全局哈希输入：
...
| 文件                                        | 目的                                         |
| ------------------------------------------- | ----------------------------------------------- |
| [watch/RULE.md](./references/watch/RULE.md) | turbo watch, interruptible tasks, 开发工作流 |
