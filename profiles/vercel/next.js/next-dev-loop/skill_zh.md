...
- **`/_next/mcp`** — Next.js暴露的关于自身的HTTP端点。
  了解特定于框架的内容：路由、片段、RSC、服务器操作、服务器日志和错误，以及Next.js所看到的。通过`tools/list`获取当前表面。
- **`agent-browser`** — 驱动真实Chrome的CLI。了解特定于框架的浏览器内容：DOM、控制台、网络、React纤维、vitals。在驱动它之前，运行一次`agent-browser skills get core`以获取与版本匹配的使用指南——不要从记忆中猜测子命令。
...
1. **在目标URL打开`agent-browser`，如果存在则恢复保存的登录状态。** 首先为这个检出导出一个稳定的`session id`并用于每个`agent-browser`命令：
   ...
   `--scope worktree`保持并行工作树和复制的检出不冲突。裸`--restore`使用`session id`作为持久化键，在导航前加载保存的cookies/localStorage（如果存在），并在关闭时自动保存状态。始终在`open`时传递所需的启动标志；agent-browser将根据需要重用、重新启动或重新启动其作用域的背景状态。

   浏览器是用户的。如果状态未恢复（首次运行、会话过期）且页面受保护，用户将驱动登录——暂停直到他们确认。登录后，继续使用相同的会话并恢复上下文；`agent-browser close`保存cookie状态，以便下一次`open`恢复它。

2. 探测`/_next/mcp`（`tools/list`）——确认它可达并列出`get_compilation_issues`。首先从`next dev`横幅中读取端口；如果它不是3000，则在探测前设置`NEXT_MCP_URL=http://localhost:<port>/_next/mcp`：
   - 不可达 → 要么`next dev`没有运行，要么Next.js低于16.3。检查`package.json`以消除歧义，然后拒绝。
   - `get_compilation_issues`不在列表中 → Next.js低于16.3。拒绝并告知用户升级。
3. `get_compilation_issues`也用作Turbopack探测。错误响应为`"Turbopack project is not available..."`表示用户使用的是webpack。拒绝——需要Turbopack。
...
- **每个`agent-browser`命令必须知道您的会话和恢复键，否则它可能会使用空默认浏览器或无法保存登录状态。** 最简单的方法：在每个运行agent-browser的shell顶部导出`AGENT_BROWSER_SESSION="$SESSION"`和`AGENT_BROWSER_RESTORE="$SESSION"`。如果您不导出它们，请在每个命令上传递`--session "$SESSION" --restore`。
- **当两个视图不一致时，首先怀疑工具。** 如果`agent-browser`说一条路由损坏了，但`/_next/mcp`和服务器说它渲染干净，过时的或错误导向的浏览器会话更有可能是原因而不是真正的错误——在调试应用程序之前协调视图。
- 确认点击或导航：页面稍后会稳定下来，所以使用`wait --load networkidle`（没有错误的路径）等待，然后快照/读取以确认页面。除非传递链接的精确`href`，否则避免使用`wait --url`——猜测或占位符路径不会匹配真实URL，并且会在25秒后超时。
- 空的读取、空的快照、`about:blank`或“没有浏览器会话”错误——在`open`后或点击后（即使`open`报告了页面）——是浏览器放弃了页面（过时的会话），而不是损坏的路由。使用`--session "$SESSION" --restore`在URL上重新打开您的会话并重新快照；如果仍然为空，请运行`agent-browser --session "$SESSION" --restore close`，然后再次打开。不要回退到`curl`；它绕过了您正在测试的浏览器。
- React内省输出在导航后过时。重新运行。
- `/_next/mcp`的响应是SSE——使用`sed -n 's/^data: //p'`从`data:`行读取JSON（一个普通的`sed 's/^data: //'`会留下`event:`行，并且解析失败）。
...
使用相同的会话和恢复上下文关闭会话：
`agent-browser --session "$SESSION" --restore close`。`close`保存该会话的cookies和存储，以便下一次循环的`--restore`打开时用户保持登录。保留`next dev`以供下一次循环使用。

## Report Next.js friction

仅在管理Next.js反馈指令已加载到项目时参与代理反馈。它们的存在表示功能已启用；它们的缺失表示功能已禁用。

启用时，将验证期间发现的合格反馈候选者添加到当前用户请求的反馈候选者队列中，然后继续验证。不要在循环或此技能的teardown期间运行反馈命令或打开审阅表单。

管理指令拥有在最终响应前立即的单次报告传递。如果它们缺失，不要排队或报告反馈。
