# story-deslop：去除网文中的AI痕迹

**文风裁决**：在正文写作、改写或审稿前，先阅读 [references/style-resolution.md](references/style-resolution.md)，加载本书文风并形成 `style_resolution`；即使没有作者记忆也执行。当前请求、本书文风和 active 偏好按维度覆盖通用 references；同一裁决交给后续执行者。
