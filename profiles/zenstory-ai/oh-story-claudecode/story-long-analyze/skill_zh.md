...
3. **询问用户**（用 AskUserQuestion 风格的明确二选一）：
   ...
   - 选「继续全量拆解」→ 读 `_progress.md`，从 **Stage 2** 续跑，**不重跑 Stage 1**。
...
- 主线程将 agent 输入写入 `章节/第{N}章_摘要.md`
...
- 情节点数 `N = grep -cE '^P[0-9]+ ' _章节摘要汇总.md` == 各摘要 `^P` 行数之和
...
- 单章失败不阻断管道；批次全部 spawn 完成后才决定是否进入 Stage  **无文本路径时**：如果用户没有提供原文文件路径、也没有在对话中贴出原文，引导用户提供原文——「请提供这本书的原文文件路径，或直接把原文贴给我，我从黄金三章开始拆。」拿到原文后进入管道。
