...
### 文本提取
如果您只需要读取文档的文本内容，您应该使用 pandoc 将文档转换为 Markdown 格式。Pandoc 提供了出色的支持以保留文档结构，并且可以显示跟踪的更改：
...
### 原始 XML 访问
您需要原始 XML 访问功能用于：注释、复杂格式、文档结构、嵌入媒体和元数据。对于这些功能中的任何一项，您都需要解包文档并读取其原始 XML 内容。
...
* `word/comments.xml` - 文档.xml 中引用的注释
* `word/media/` - 嵌入的图像和媒体文件
* 跟踪更改使用 `<w:ins>`（插入）和 `<w:del>`（删除）标签

## 创建 Word 文档

当从零开始创建 Word 文档时，请使用 **docx-js**，它允许您使用 JavaScript/TypeScript 创建 Word 文档。

### 工作流程
1. **必须 - 读取整个文件**：完整阅读 [`docx-js.md`](docx-js.md)（约 500 行）从头到尾。**在读取此文件时切勿设置任何范围限制。** 在继续文档创建之前，请先阅读完整文件内容以了解详细语法、关键格式规则和最佳实践。
2. 使用 Document、Paragraph、TextRun 组件创建 JavaScript/TypeScript 文件（您可以假设所有依赖项都已安装，如果没有，请参考下面的依赖项部分）
3. 使用 Packer.toBuffer() 导出为 .docx

## 编辑现有 Word 文档

当编辑现有 Word 文档时，请使用 **文档库**（一个用于 OOXML 操作的 Python 库）。该库自动处理基础设施设置并提供文档操作方法。对于复杂场景，您可以通过库直接访问底层 DOM。

### 工作流程
1. **必须 - 读取整个文件**：完整阅读 [`ooxml.md`](ooxml.md)（约 600 行）从头到尾。**在读取此文件时切勿设置任何范围限制。** 请阅读完整文件内容以了解文档库 API 和直接编辑文档文件的 XML 模式。
...
此工作流程允许您使用 Markdown 规划全面的跟踪更改，然后再在 OOXML 中实现它们。**关键**：为了实现完整的跟踪更改，您必须系统地实现所有更改。

**批量策略**：将相关的更改分组为 3-10 个更改的批次。这使调试变得易于管理，同时保持效率。在转到下一个批次之前测试每个批次。

**原则：最小化、精确的编辑**
在实现跟踪更改时，仅标记实际发生更改的文本。重复未更改的文本使编辑更难审查，并且看起来不专业。将替换分解为：[未更改的文本] + [删除] + [插入] + [未更改的文本]。通过从原始中提取 `<w:r>` 元素并重新使用它来保留原始运行的建议 RSID，以保留未更改的文本。
...
2. **识别和分组更改**：审查文档并识别所有需要的更改，将它们组织成逻辑批次：
   ...
   **批量组织**（每个批次分组 3-10 个相关更改）：
   - 按章节："批次 1：第 2 节修订案"，"批次 2：第 5 节更新"
   - 按类型："批次 1：日期更正"，"批次 2：当事人名称更改"
   - 按复杂性：从简单的文本替换开始，然后处理复杂结构更改
   - 顺序："批次 1：第 1-3 页"，"批次 2：第 4-6 页"

3. **阅读文档和解包**：
   - **必须 - 读取整个文件**：完整阅读 [`ooxml.md`](ooxml.md)（约 600 行）从头到尾。**在读取此文件时切勿设置任何范围限制。** 特别注意“文档库”和“跟踪更改模式”部分。
...
4. **分批实现更改**：将更改逻辑分组（按章节、按类型或按邻近性），并在单个脚本中一起实现它们。这种方法：
   ...
   **建议的批量分组**：
   ...
   - 按邻近性（例如，“第 1-3 页上的更改”，“文档前半部分的更改”）

   对于每个相关的更改批次：

   **a. 将文本映射到 XML**：在 `word/document.xml` 中 Grep 文本以验证文本如何分布在 `<w:r>` 元素中。

   **b. 创建并运行脚本**：使用 `get_node` 找到节点，实现更改，然后 `doc.save()`。见 **“文档库”** 部分在 ooxml.md 中以获取模式。
...
为了视觉分析 Word 文档，请使用两步过程将它们转换为图像：
