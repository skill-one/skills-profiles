...
在编写任何代码之前，通过扫描项目并询问用户结构化问题来收集需求。这确保了实施符合他们的需求。
...
- **现有认证** — 查找 `package.json` 或导入中的现有认证库（`next-auth`、`lucia`、`clerk`、`supabase/auth`、`firebase/auth`）。
- **包管理器** — 检查 `pnpm-lock.yaml`、`yarn.lock`、`bun.lockb` 或 `package-lock.json`。

使用您发现的内容预填默认值，并跳过您已经可以自信回答的问题。

### 第二步：询问规划问题

使用 `AskQuestion` 工具在单个调用中询问用户**所有适用的问题**。跳过您从扫描中已经有一个自信答案的问题。将它们分组在像“认证设置规划”这样的标题下。
...
5. **社交提供者**（仅在选择了社交 OAuth 的情况下询问——在后续调用中询问）
   - 提示：“您需要哪些社交提供者？”
   - 选项：Google | GitHub | Apple | Microsoft | Discord | Twitter/X
   - `allow_multiple: true`

6. **电子邮件验证**（仅在选择了电子邮件和密码的情况下询问——在后续调用中询问）
   - 提示：“您是否需要要求电子邮件验证？”
   - 选项：是 | 否

7. **电子邮件提供者**（仅在电子邮件验证为是的情况下，或在功能中选择密码重置的情况下询问——在后续调用中询问）
   - 提示：“您希望如何发送电子邮件？”
   - 选项：Resend | 暂时模拟（console.log）

8. **功能 & 插件**（始终询问，允许多选）
   - 提示：“您需要哪些附加功能？”
   - 选项：双因素认证（2FA） | 组织/团队 | 管理员仪表板 | API 带宽令牌 | 密码重置 | 全部都不需要
   - `allow_multiple: true`

9. **认证页面**（始终询问，允许多选——根据之前的答案预选）
   - 提示：“您需要哪些认证页面？”
   - 选项根据之前的答案变化：
     - 始终可用：登录 | 注册
...
收集答案后，以 Markdown 清单形式呈现简洁的实施计划。示例：
...
### 步骤
1. 安装 `better-auth`（CLI 作为 `auth` 包提供，通过 `npx auth@latest` 运行，无需安装）
...
实施结束后，指导用户彻底了解剩余的下一步（例如，设置 OAuth 应用凭据、部署环境变量、测试流程）。
...
- `emailVerification.sendVerificationEmail` - 电子邮件验证处理程序
- `emailAndPassword.sendResetPassword` - 密码重置处理程序
...
| 适配器 | 命令 |
|---|---|
| 内置 Kysely | `npx auth@latest migrate`（直接应用） |
| Prisma | `npx auth@latest generate --output prisma/schema.prisma` 然后 `npx prisma migrate dev` |
| Drizzle（开发） | `npx auth@latest generate --output src/db/auth-schema.ts` 然后 `npx drizzle-kit push` |
| Drizzle（生产） | `npx auth@latest generate --output src/db/auth-schema.ts` 然后 `npx drizzle-kit generate` 然后 `npx drizzle-kit migrate` |
...
| Drizzle（pg） | `drizzleAdapter(db, { provider: "pg" })` 来自 `better-auth/adapters/drizzle` |
| Drizzle（mysql） | `drizzleAdapter(db, { provider: "mysql" })` 来自 `better-auth/adapters/drizzle` |
| Drizzle（sqlite） | `drizzleAdapter(db, { provider: "sqlite" })` 来自 `better-auth/adapters/drizzle` |
...
- [ ] `advanced.useSecureCookies: true` 在生产环境中
- [ ] `trustedOrigins` 配置
- [ ] 启用速率限制
- [ ] 启用电子邮件验证
- [ ] 实现密码重置
