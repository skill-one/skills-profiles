...
Lakebase Postgres 是 Neon 的核心数据库，它基于 lakebase 架构（云对象存储直接构建的 OLTP）运行，将存储与计算解耦，提供自动扩展、分支、即时恢复和零扩展功能。它与 Postgres 完全兼容，适用于支持 Postgres 的任何语言、框架或 ORM。
...
如果已提供 `DATABASE_URL`（提示、环境或代码库），或 `.neon` 文件指向某个项目，请使用它们。不要列出组织或为模式工作创建第二个项目。

否则，使用 CLI（默认）或 MCP 服务器列出组织和项目。让用户选择现有项目或创建新项目。

### 2. 获取连接字符串

如果已提供 `DATABASE_URL`，请使用它。不要通过 CLI 或 MCP 获取另一个。
...
在将迁移应用到生产环境之前，先在生产分支上使用生产类似的数据进行测试。

运行迁移时，使用**直接（非池化）**连接字符串，而不是池化连接。`neon connection-string` 默认返回直接字符串；确保主机名不包含 `-pooler` 后缀。

## 故障排除和 Neon 特定性能

在手动编写目录查询之前，使用 Neon 的预定义只读诊断工具。Neon CLI `neon inspect db` 子命令和 Neon MCP 服务器的 `inspect_database` 工具运行相同的检查。
...
| 索引未使用或表被大量扫描？     | `unused-indexes`, `seq-scans`        |
| 有哪些运行了 5+ 分钟或持有锁？    | `long-running-queries`, `locks`      |
...
- 顺序扫描对于小型表或读取表的大部分数据可能是正确的。在添加索引之前，检查表大小、选择性和查询计划。
- `bloat` 是一个统计估计。在运行 `VACUUM FULL`、`REINDEX` 或类似修复之前，确认影响并计划锁或维护。
- 缓存和 Postgres 统计信息在计算重新启动时（包括零扩展暂停）会重置。在解释新鲜的 `lfc-hit-rate`、`working-set`、`vacuum-stats` 或 `pg_stat_statements` 结果之前，运行代表性工作负载。
- 全局计算检查（`lfc-hit-rate`、`working-set` 和 `replication-slots`）即使检查每个数据库时也只运行一次。
- 一个失败的数据库可能会使所有数据库检查失败；使用显式的 `databaseName` 重试相关检查以隔离它。

### 按查询检查 Neon 缓存行为

标准 `EXPLAIN (ANALYZE, BUFFERS)` 报告 Postgres 共享缓冲区活动，但它不显示 Neon 的本地文件缓存 (LFC) 或页面预取。对于安全的只读查询，添加 Neon 的 `FILECACHE` 和 `PREFETCH` 选项：
...
`FILECACHE` 和 `PREFETCH` 为此查询提供指标，并且不需要 `neon` 扩展。相比之下，`neon inspect db lfc-hit-rate` 和 `working-set` 提供全局统计信息，并且需要该扩展。

MCP 的 `explain_sql_statement` 工具可以生成标准计划，但不暴露 `FILECACHE` 或 `PREFETCH` 选项。要通过 MCP 收集这些 Neon 特定指标，请使用上面明确的只读 `EXPLAIN` 语句并使用 `run_sql`。

由于 `ANALYZE` 会执行语句，因此仅在执行安全时使用它；不要为可变 SQL 自动运行它。仔细比较冷缓存和热缓存运行，因为第一次执行可以填充缓存并实质性地改变后续结果。

### 性能工作流

1. 复现症状并记录其时间窗口。
2. 运行上表中与问题最相关的最小 `inspect` 检查。
3. 在更改模式或计算之前，识别特定查询。使用 MCP `explain_sql_statement` 获取标准计划，或在 LFC 或预取行为重要时的 Neon 特定 `EXPLAIN` 语句。
4. 如果瓶颈是查询形状、索引、模式、锁定或清理行为，加载 `postgres-best-practices` 并将检查结果和查询计划传递下去。在此技能中保留 Neon 计算、缓存、连接和平台决策。
5. 重新运行相同的检查和工作负载以验证更改。

当用户需要按平均执行时间对查询进行排名并具有自定义阈值和限制时，使用 MCP `list_slow_queries` 而不是 `inspect_database`。在上述显式 `EXPLAIN` 情况之外，仅在预定义检查无法回答问题时，使用 `run_sql` 仅用于只读诊断 SQL。

## 自动扩展

当用户需要计算随工作负载自动扩展并希望获得 CU 大小和运行时行为的指导时，使用此功能。

链接：https://neon.com/docs/introduction/autoscaling.md

## 零扩展

当用户需要优化空闲成本并讨论暂停/恢复行为（包括冷启动权衡）时，使用此功能。

要点：

- 空闲计算在默认 5 分钟后自动暂停；超时是可配置的，并且暂停只能在 Launch 和 Scale 计划中禁用。
...
当用户需要时间点恢复或希望在不使用传统备份恢复工作流的情况下恢复数据状态时，使用此功能。
...
当用户需要读密集型工作负载，并且需要专用的只读计算而不重复存储时，使用此功能。
...
当用户处于无服务器或高并发环境，并需要安全的可扩展 Postgres 连接管理时，使用此功能。

要点：

- Neon 池化使用 PgBouncer。
- 将 `-pooler` 添加到端点主机名以使用池化连接。
- 池化在无服务器运行时具有突发并发性时尤其重要。

链接：https://neon.com/docs/connect/connection-pooling.md

## IP 允许列表

当用户需要通过可信网络、IP 或 CIDR 范围限制数据库访问时，使用此功能。

链接：https://neon.com/docs/introduction/ip-allow.md

## 逻辑复制

当用户需要集成 CDC 管道、外部 Postgres 同步或基于复制的数
