...
资源型技能，负责基于 `media_id` 下载媒体文件到本地，以及将本地文件上传为 `media_id`。是其他技能（微盘、邮件等）处理 `media_id` 相关操作的基础依赖：`upload` 会产出新的 `media_id`，但本技能不负责搜索/发现其他业务场景中已存在的 `media_id`（如邮件附件、微盘文件的 `media_id` 由对应业务技能产出），也不解析文件内容。
...
- 解析/识别文件内容（正文提取、OCR、看图问答、PDF/Word/Excel 解析等） → 本技能只负责把文件下载到本地拿 `file_path`，如需查看内容请直接通过 `file_path` 读取该本地文件
- 搜索/发现其他业务场景中已存在的 `media_id`（如邮件附件、微盘文件列表/搜索等） → 由对应业务技能负责产出并返回 `media_id`，本技能只接收已有的 `media_id` 做下载；本地文件转`media_id` 的场景仍走本技能的 `upload`
...
- **不做内容解析**：本技能只负责文件的下载落地与上传，`download` 拿到 `file_path` 后如需查看内容，直接通过 `file_path` 读取，不在本技能职责范围内。
...
| 依赖场景 | 说明 |
|---|---|
| `wecomcli-email` | 邮件附件/内嵌图片的 `media_id`，使用本技能的 `download` 下载到本地后通过 `file_path` 读取 |
| `wecomcli-disk` | 上传文件到微盘时若已有 `media_id`，直接作为 `disk files upload` 的 `file_content_media` 使用，无需再走本技能；若只有本地路径且需要先转成 `media_id`，可用本技能的 `upload` |
