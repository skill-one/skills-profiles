...
- **在线文档形态一律不走下载**：若搜索返回的 `type` 是 `smartsheet` / `smartpage` / `sheet` / `word` / `ppt` / `journal` / `collect` / `mind` / `flow`，**禁止**把它们的 `id` 或 `doc_url` 当 `file_id` / `url` 传入本接口，会失败或拿到无效文件。其中 `smartsheet` / `smartpage` / `sheet` / `word` 有对应的下游 skill 可读正文，路由见文末【跨技能依赖】表；`ppt` / `journal` / `collect` / `mind` / `flow` 目前**没有下游 skill 可读正文**，直接告知用户暂不支持，引导其用 `doc_url` 在企业微信客户端内打开查看。
