...
belt app run falai/flux-dev-lora --input '{
  "prompt": "日落时分宁静的山湖，雪山顶峰倒映在静止的水面，黄金时刻的光线，风景摄影",
  "width": 1248,
  "height": 832
}'
...
| **WAN-I2V (Pruna)** | `pruna/wan-i2v` | 经济高效，快速，480p/720p | 自然，高效 |
...
| 场景 | 最佳模型 | 原因 |
|----------|-----------|-----|
| 带水域/云层的风景 | **Wan 2.5 i2v** | 最擅长自然真实动态 |
| 带微妙表情的肖像 | **Wan 2.5 i2v** | 保持面部保真度 |
| 带布料/织物的产品 | **Fabric 1.0** | 专业于材料物理 |
| 招旗飘动，窗帘流动 | **Fabric 1.0** | 布料模拟 |
...
AI视频模型在**轻柔、微小的运动**下效果优于戏剧性动作。请求过多运动会导致失真和伪影。
...
# 风景动画
belt app run falai/wan-2-5-i2v --input '{
  "prompt": "缓慢向右摇移，水面倒映移动的云朵，树木轻微随风摇动，温暖金色光线，宁静缓慢",
  "image": "landscape.png"
}'

# 肖像动画
belt app run falai/wan-2-5-i2v --input '{
  "prompt": "轻微呼吸运动，轻微转头，自然眨眼，头发轻柔运动，柔和环境光线变化",
  "image": "portrait.png"
}'
...
# 建筑可视化
belt app run falai/wan-2-5-i2v --input '{
  "prompt": "缓慢前移穿过入口，轻微摄像机向上倾斜，环境光线透过窗户，光束中漂浮的尘埃",
...
belt app run bytedance/seedream-4-5 --input '{
  "prompt": "电影感风景，黎明时分的雾气山脉，前景湖面，戏剧性云层，黄金时刻，4K质量，专业摄影",
  "size": "2K"
}'
...
```bash
belt app run falai/wan-2-5-i2v --input '{
  "prompt": "只有瀑布在移动，其他一切保持完美静止，水流平稳，场景其他部分冻结",
