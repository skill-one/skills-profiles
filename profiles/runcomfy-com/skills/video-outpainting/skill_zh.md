...
> 避免使用：严格缝合质量要求的英雄级广告投放——使用 ComfyUI 扩展工作流。
...
- **预期缝合处质量差异**。Wan 2-7 没有专门针对扩展进行训练；对于英雄级交付，请使用 ComfyUI 工作流。

---

## 当你需要英雄级缝合质量时

上述端点可以很好地处理大多数用途的宽高比切换。对于需要严格时间一致性、缝合处理和运动感知填充的空间帧扩展，RunComfy 提供了专门的 ComfyUI 工作流：
...
这些是 GUI 工作流，不是 CLI 端点。CLI 无法访问它们——请在 RunComfy ComfyUI 云中打开它们。
...
该技能选择 Wan 2-7 编辑视频进行提示形状的画布扩展，并调用 `runcomfy run` 带有扩展形状的 JSON 正文。CLI 向模型 API 发送 POST 请求，轮询请求状态，并将结果下载到 `--output-dir`。
...
- **生成文件大小上限**：CLI 会中止任何单个下载大于 2 GiB 的请求。
