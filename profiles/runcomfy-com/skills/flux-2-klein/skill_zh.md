...
Flux 2 Klein 的独特优势是 **以延迟优先的创意迭代**：亚秒级的反馈支持实时艺术指导会话和快速产品可视化，而批量风格的模型无法持续。选择它时，**迭代速度比极限分辨率更重要**。
...
| `steps` | int | 否 | 25 | 4–50。**步骤精炼架构**——4–8 足够用于构思；~25 用于润色；>25 购买不到多少。 |
...
延迟优先的变体。亚秒级 4 步推理。用于实时迭代 / 构思。

与 9B 相同的字段集。默认 `steps` 实际上是 4——此变体是为该步数构建的。

### 参考图像（两个变体）

最多支持 **4 个同时的参考图像** 在同一端点上用于风格迁移 / 指导构图。JSON 体的确切字段名在模型的 API 标签上记录[此处](https://www.runcomfy.com/models/blackforestlabs/flux-2-klein/9b/text-to-image?utm_source=skills.sh&utm_medium=skill&utm_campaign=flux-2-klein)——通过 CLI 原样传递。参考图像的使用支持无需单独 `/edit` 端点的编辑式工作流。
...
CLI 提交，每 2 秒轮询一次直到终端，然后下载任何 `*.runcomfy.net` / `*.runcomfy.com` URL 到 `--output-dir`。标准输出是结果 JSON。标准错误是进度。
...
**主语优先的声明性语法**。Flux 2 Klein 训练的结构是 *"主语 + 动作 + 场景 + 风格 + 光照 + 相机 + 质量"*。前置主语；以指令结尾。示例：`"一只充满活力的蜂鸟在飞行中吸食鲜艳的粉色木槿花蜜，翠绿色的蓝宝石羽毛在晨光中闪烁，浅景深热带花园背景，微距摄影，锐利细节，电影级光照"`。

**具体性胜过华丽的语言**。"4k 产品照片，柔光箱光照，反光桌面，35mm，f/2.8" 指导具有可预测性。"一个真的很漂亮的产品图像" 则没有。

**按阶段调整步数**。
- **构思**：4B 变体上 4–8 步——亚秒级反馈用于实时探索。
- **细化**：4B 上 8–15 步，锁定主体 + 构图。
- **润色**：9B 变体上 ~25 步——纹理，微观细节，精细排版。

**多参考图像对齐**。传递参考图像时，**保持它们的审美对齐**。在同一调用中混合水彩 + 照片真实 + 3D 渲染会混淆编辑器。在所有参考中保持一致的视觉注册。

**条件编辑**：先说明保持什么，再说明改变什么。*"与参考图像相同的构图和光照，但将背景从海滩改为工作室山脉。"* 此模式保持构图稳定。

**对于文本渲染**（Klein 拥有 8B Qwen3 嵌入器，不错但不如 GPT Image 2）：添加 `"清晰的排版，高对比度标签"` 并将步数增加到 ~25 如果文本出来是模糊的。对于图像内文本或多语言渲染，改为 GPT Image 2。
...
| **消费者 GPU 友好推理** | 4B 变体可在普通硬件上运行；对于自托管比较相关，但 RunComfy 托管也可以 |
...
```
与参考图像相同的构图和光照，但瓶标现在是蓝色，白色无衬线字体
阅读 "AURA"；保持瓶身轮廓，桌子，阴影与参考完全一致
```

## 限制

- **分辨率极限 ~2K**——对于更高原生分辨率，路由到 Seedream 5。
...
1. 技能调用 `runcomfy run blackforestlabs/flux-2-klein/<变体>/text-to-image` 并带有匹配模式的 JSON 正文。
2. CLI POST 到 `https://model-api.runcomfy.net/v1/models/blackforestlabs/flux-2-klein/<变体>/text-to-image` 并带有用户的令牌。
3. 模型 API 返回 `request_id`；CLI 每 2 秒轮询一次 `GET .../requests/<id>/status`。
4. 在终端状态下，CLI 获取 `GET .../requests/<id>/result` 并下载任何主机以 `.runcomfy.net` 或 `.runcomfy.com` 结尾的 URL 到 `--output-dir`。其他 URL 被列出但不会下载。
