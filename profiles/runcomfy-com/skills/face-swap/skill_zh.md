...
> 选择原因：多个帧中保持相同身份（SKU 照片、A/B 变体、叙事面板）。身份参考作为 `image_urls[0]`，场景在后。
> 避免原因：精确多参考组合（“从 img 1 的人脸替换到 img 2 的人体”）——使用 **GPT Image 2 Edit**。

**GPT Image 2 Edit** — `openai/gpt-image-2/edit`
> 最高 10 张参考图片，多语言图片内文本重写，布局精确的组合指令。
> 选择原因：英雄静态图片，精确肖像中的人脸必须落到场景中，有明确的角色分配（“image 1”、“image 2”）；保留姿势 + 光照 + 背景，仅替换脸。
...
对于**静态图片**，GPT Image 2 Edit 接受最高**10 张参考图片**并遵循精确的组合指令——使其成为单输出帧上多参考人脸替换的最强路径。
...
当**多个帧需要替换相同的身份**时选择此路由——SKU 照片、A/B 变体、叙事面板。
...
当替换是**一张图片、一条声明性指令、最高保真度保留除脸之外的所有内容**时，Flux Kontext 最适合。
...
- **没有新身份的参考图片**——用散文描述脸。
- **单图片、单镜头、最大保真度**——Flux Kontext 在“保留除 X 之外的所有内容”提示上优于其他路由。
...
RunComfy 上许多人脸替换工作流也作为完整的 **ComfyUI 节点图**（ReActor、Flux PuLID、ACE++、Flux Klein 头部替换）存在——这些无法从此 CLI 直接访问，但可以在平台上作为工作流运行。在上述 CLI 驱动路由不适合时，浏览它们：[runcomfy.com/comfyui-workflows](https://www.runcomfy.com/comfyui-workflows?utm_source=skills.sh&utm_medium=skill&utm_campaign=face-swap)。
...
技能分类用户意图——视频与静态，保留运动与保留身份，单次与批量，照片真实与风格化——并选择其中一个路由。然后它调用 `runcomfy run <model_id>` 并使用匹配的 JSON 正文。CLI POST 到模型 API，轮询请求状态，获取结果，下载任何 `.runcomfy.net` / `.runcomfy.com` URL 到 `--output-dir`。
