...
| 动画化 **并** 口型同步到 **自定义旁白轨道** | **Wan 2.7 + `audio_url`** | 接受您自己的 MP3/WAV（3–30秒，≤15MB）并驱动口型同步到它 |
| 多语言配音变体（同一图像，每次调用不同音频） | **Wan 2.7 + `audio_url`** | 同一镜头，根据语言交换 `audio_url` |
| 多模态 — 图像 + 参考视频 + 参考音频一起 | **Seedance 2.0 专业版** | 最高 9 张图像参考，3 个视频参考（每个 2–15 秒），3 个音频参考 |
| 品牌一致的故事 — 角色参考 + 场景参考 + 音频参考 | **Seedance 2.0 专业版** | 图像保持身份，视频保持场景，音频保持声音 |
| 未指定时的默认值 | **HappyHorse 1.0 I2V** | 最佳综合质量 + 原生音频 |

代理读取此表格，对用户的意图进行分类，并选择匹配的下方子部分。
...
- **不要重述图像** — 模型已经看到了它。专注于变化的内容的标记。
- **明确的保存目标**: "身份稳定的特征", "包装不变", "背景几何稳定"。
- **照明演变**: "边缘光增强", "随着摄像机上升，阴影变短"。
- **每个剪辑一个节奏** — 单一主要动作（orbit OR dolly OR tilt OR 角色动作）。
...
**关于 Wan 2.7 的 i2v 注意**: Wan 2.7 的主要 i2v 动画不在此处专门的端点上。对于纯 i2v（仅通过动作提示动画化的图像），请优先选择 **HappyHorse i2v**。当用户有自定义音频轨道并希望将其口型同步到生成的说话头片段时，专门使用 Wan 2.7。
...
| `resolution` | 枚举 | 否 | `1080p` | `720p` 或 `1080p`。 |
| `duration` | 枚举 | 否 | `5` | 2–15（整秒）。匹配音频长度。 |
| `negative_prompt` | 字符串 | 否 | — | 要避免的具体问题（例如 "没有字幕，没有闪烁"）。 |
| `seed` | 整数 | 否 | — | 可重复性。 |
...
- **使用 `negative_prompt` 指出问题**: `"没有字幕，没有闪烁，没有变形的手"`.
- **多语言配音** — 相同提示，每次调用交换 `audio_url`。锁定种子以跨语言保持视觉一致性。
...
| 字段 | 类型 | 必填 | 默认 | 备注 |
|---|---|---|---|---|
| `prompt` | 字符串 | 是 | — | CN ≤500 字符 OR EN ≤1000 词。 |
...
| `duration` | 整数 | 否 | 5 | 4–15（整秒）。 |
| `resolution` | 枚举 | 否 | `720p` | `480p` 或 `720p`。 |
| `generate_audio` | 布尔值 | 否 | true | 处理中同步语音 / 音效 / 音乐。 |
| `seed` | 整数 | 否 | — | 可重复性。 |
...
runcomfy run bytedance/seedance-v2/pro \
  --input '{
    "prompt": "主题图像 1 中的角色走过咖啡馆，声音语调匹配音频 1。中等特写，缓慢推进，暖光，轻柔的环境音。",
...
- **提示中编号参考**：`"主题来自图像 1，照明来自视频 1，声音来自音频 1"`。Seedance 正确路由提示。
- **参考媒体规格** — 视频 / 音频必须为 2–15 秒；音频 < 15MB。
- **不要混合截然不同的美学** — 如果图像 1 是水彩画，视频 1 是照片逼真，输出会漂移。
...
- **没有多路径混合。** 此技能每次调用选择一个模型。如果用户想要 HappyHorse 动画 + Wan 风格口型同步在同一片段中，那是两个调用 + 粘合（在此范围之外）。
...
技能根据用户意图选择 HappyHorse 1.0 I2V / Wan 2.7 t2v+音频 / Seedance 2.0 专业版，并调用 `runcomfy run <model_id>` 使用匹配的 JSON 正文。CLI 向模型 API POST，轮询请求，获取结果，并将任何 `.runcomfy.net`/`.runcomfy.com` URL 下载到 `--output-dir`。`Ctrl-C` 在退出前取消远程请求。
...
- **第三方内容**: 您传递的图像 / 掩码 / 视频链接由 RunComfy 模型服务器获取，而不是您机器上的 CLI。将外部 URL 视为不受信任；基于图像的提示注入是任何图像编辑 / 视频编辑模型的已知风险。
