...
将静态图像扩展到其原始画布之外——去裁剪、改变宽高比、填充相机未捕捉到的部分。这项技能在 RunComfy 目录中的身份保持编辑端点之间进行路由，为基于文本的扩展、参考式匹配或品牌锁定延续选择正确的端点。
...
> 选择用于：宽高比改变（方形 → 16:9）、去裁剪肖像、用匹配的环境扩展风景照片。
...
> 单指令、高保留编辑。使用格式：`"将画布扩展为 16:9 宽高比。添加匹配的天空和建筑，从现有场景延续。保留原始图像中的所有内容。"`
...
runcomfy run google/nano-banana-2/edit \
  --input '{
    "prompt": "向下扩展画布以显示主体的上半身和手臂。延续现有的服装风格、照明和背景。保留面部和当前可见区域，与输入完全一致。",
...
- **以保留结束**: `"保留 [原始可见区域] 与输入完全一致"`。没有这个提示，Nano Banana 可能会微妙地重新生成原始部分。
...
1. **GPT Image 2 Edit** 配合参考图像（显示所需周围样式）(`images: [原始图像, 样式参考]`)
...
### 恢复裁剪的历史照片
- **路由 2 (GPT Image 2 Edit)** 配合一张或多张时代合适的参考照片

### 多步扩展（扩展，然后重新扩展）
- 链接：扩展传递 1 → 将结果用作传递 2 的输入。每一步扩展约 30–50%，以避免边界处质量下降。
