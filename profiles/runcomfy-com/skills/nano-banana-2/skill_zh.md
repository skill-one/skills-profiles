...
Google **Nano Banana 2** — Gemini 家族中的快速级文本到图像模型 — 在 **RunComfy 模型 API** 上托管。针对创意构思、社交缩略图批量处理和快速草稿进行了优化，具有强大的图像内排版式设计。
...
Nano Banana 2 是 Google 图像生成系列的快速级。当迭代速度和可预测的构图比最大细节更重要时，选择它。

| 您需要 | 使用 |
|---|---|
| 快速草稿、社交缩略图、批量变体 | **Nano Banana 2** |
| 图像内排版式设计（可预测渲染） | **Nano Banana 2** |
...
如果用户明确提到 "Nano Banana" / "nano-banana-2" / "Gemini 图像"，无论是否指定 2 与 Pro，都直接路由到此。如果用户提到 "Nano Banana" 而未指定 2 与 Pro，默认为 **Pro** 用于肖像，**2** 用于其他所有内容。
...
| 字段 | 类型 | 必填 | 默认 | 备注 |
|---|---|---|---|---|
| `prompt` | string | 是 | — | 主体优先描述。 |
| `num_images` | int | 否 | 1 | 1–4。使用 4 进行创意构思轮次。 |
| `seed` | int | 否 | 0 | 重复使用以实现可重复性。 |
...
| `limit_generations` | bool | 否 | true | 限制每个提示符轮次生成一次。 |
...
**主体优先的声明性语法。** "一个电影感的特写肖像，一位美国女性站在东京雨中的霓虹灯下，浅景深，反射的湿街道，超精细，逼真的皮肤纹理" — 主要主体，然后动作、环境、风格、相机。前置主体；后缀指令。

**精确文本引用用于图像内排版式设计。** "标签上写着 'AURA'，干净粗体无衬线字体，居中，白色在黑色上" — 引用实际字符。指定位置和字体样式。不要说 "带有品牌名称" 并希望。

**一致的种子用于细化。** 当迭代单个提示符的小变体时锁定 `seed` — 保持构图稳定。

**网络基础，适度使用。** 仅当提示符命名当前事件 / 真实实体时才打开 `enable_web_search`。增加延迟 + 成本；默认关闭。
...
| 用例 | 为什么 Nano Banana 2 |
|---|---|
| **营销草稿缩略图（4 图批量）** | 0.5K 快速迭代，然后推广获胜者到 2K |
| **社交平台原生** | 支持宽纵横比，包括 9:16, 4:5, 21:9 |
| **图像内排版式设计用于海报 / 卡片** | 当字符被引用时，可预测的文本渲染 |
| **网络基础当前事件图像** | `enable_web_search` 集成最新信息 |
...
```
一个电影感的特写肖像，一位美国女性站在东京雨中的霓虹灯下，浅景深，反射的湿街道，超精细，逼真的皮肤纹理
```

**品牌资产卡片带引用文本：**

```
一个极简 16:9 产品卡片：一个磨砂黑陶瓷马克杯居中在柔和暖灰色纸张背景上，左上角的反光边缘，标题 "Brewed Quietly" 在右上角干净粗体无衬线字体，下方平衡的负空间，电子商务准备，干净工作室灯光
```

**垂直平台原生：**

```
一个 9:16 垂直主角用于一个健康品牌：一个陶瓷茶杯在亚麻托盘上，柔和的晨光侧面照明，顶部手绘衬线大号字体 "Slow Down"，轻柔的蒸汽升起，中性色彩搭配，无杂乱
```
...
- **令牌存储**：`runcomfy login` 将 API 令牌写入 `~/.config/runcomfy/token.json`，模式 0600（仅所有者读写）。设置 `RUNCOMFY_TOKEN` 环境变量以在 CI / 容器中完全绕过文件。
- **输入边界**：用户提示作为 JSON 字符串通过 `--input` 传递给 CLI。CLI 不会 shell 扩展提示；它直接将 JSON 正文通过 HTTPS 传输到模型 API。提示内容没有 shell 注入表面。
- **第三方内容**：您传递的图像 / 掩码 / 视频URL由 RunComfy 模型服务器获取，而不是您的机器上的 CLI。将外部 URL 视为不受信任；基于图像的提示注入是任何图像编辑 / 视频编辑模型的已知风险。
- **出站端点**：仅 `model-api.runcomfy.net`（请求提交）和 `*.runcomfy.net` / `*.runcomfy.com`（下载白名单用于生成输出）。没有遥测，没有回调。
