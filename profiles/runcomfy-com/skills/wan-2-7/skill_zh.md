...
| 多模态电影制作，包含图像+视频+音频参考 + 生成语音 | Seedance 2.0 Pro |
...
| 字段 | 类型 | 必填 | 默认值 | 备注 |
|---|---|---|---|---|
| `prompt` | 字符串 | 是 | — | ~5000 字符 / ~1500 个 token。 |
| `audio_url` | 字符串 | 否 | — | WAV/MP3，3–30s，≤15MB。**驱动唇同步。** 省略 → 自动生成背景音乐。 |
| `aspect_ratio` | 枚举 | 否 | `16:9` | `16:9`，`9:16`，`1:1`，`4:3`，`3:4`。 |
| `resolution` | 枚举 | 否 | `1080p` | `720p` 或 `1080p`。 |
| `duration` | 枚举 | 否 | `5` | 2–15（秒）。 |
| `negative_prompt` | 字符串 | 否 | — | ~500 字符。避免具体问题。 |
| `enable_prompt_expansion` | 布尔值 | 否 | true | 自动重写短提示。用于精确/字面提示（如品牌严格广告文案），禁用 `enable_prompt_expansion: false`。 |
| `seed` | 整数 | 否 | — | 0..2^31-1。用于生成变体时保持一致性。 |

## 如何调用

**默认（5s 1080p 16:9，提示自动扩展）：**
...
**用普通英语描述相机和运动。** "缓慢推镜头"，"固定三脚架，低角度"，"手持跟拍"，"从上方俯冲镜头"。在提示中优先描述拍摄场景。

**每段视频一个主要动作。** 不要堆叠多个竞争动作。选择节奏： "她转身，然后微笑" 而不是 "她转身 AND 微笑 AND 一辆公交车经过 AND..."。

**使用 `negative_prompt` 针对具体问题。** 好： "无字幕，无水印，无闪烁"。差（模糊）： "无不良灯光"。

**提示扩展默认开启。** 短提示会被模型自动重写。对于简洁/字面提示（如品牌严格广告文案），禁用 `enable_prompt_expansion: false`。

**音频规格很重要。** `audio_url` 必须为 3–30s，≤15MB，WAV/MP3。超出范围的文件会被拒绝。匹配音频长度与片段时长。

**迭代种子。** 当您希望同一提示的不同变体保持一致性时，重复使用相同的种子。改变种子以获得真正的多样性。

**反模式：**
- 静态帧描述 → 运动将模糊不清。
- 模糊的负面提示 ("无不良颜色") → 被忽略。
- 音频超出 3–30s / 15MB / WAV-MP3 规格 → 被拒绝。
...
| 用例 | 为什么选择 Wan 2.7 |
|---|---|
| **带自定义旁白的唇同步广告** | `audio_url` 接受您的音频轨道 |
| **多语言配音变体** | 同一提示，不同语言对应不同 `audio_url` |
| **多参考运动控制** | 最高支持 5 个参考媒体（图像 / 视频 / 语音） |
...
```
9:16 垂直短片。咖啡师制作一杯浓缩咖啡，蒸汽升入晨光中，丰富的奶油缓慢形成。手持拍摄特写，浅景深，温馨的咖啡馆氛围。
```

## 限制

- **时长上限 15 秒。** 对于更长的叙事，需要拼接多个调用。
- **无原生 4K** — 1080p 为上限。
- **宽高比** — 仅支持 5 个预定义值。
- **音频规格** — 3–30s，≤15MB，仅支持 WAV/MP3。
- **参考媒体上限 5**（图像 + 视频 + 语音组合）。
- **用于生成语音（无单独音频轨道），请使用 Seedance 2.0 Pro** — Wan 接受音频而非生成音频。
...
该技能调用 `runcomfy run wan-ai/wan-2-7/text-to-video` 并传入符合模式的 JSON 正文。CLI 向 `https://model-api.runcomfy.net/v1/models/wan-ai/wan-2-7/text-to-video` 发送 POST 请求，轮询请求状态，获取结果，并将 `.runcomfy.net`/`.runcomfy.com` URL 下载到 `--output-dir`。`Ctrl-C` 在退出前取消远程请求。
...
- **生成文件大小上限**：CLI 会中止任何单个下载 > 2 GiB，以防止恶意或失控的模型输出导致磁盘空间耗尽。
