...
> 字节跳动音频驱动全身虚拟形象。输入一张肖像+一个音频文件，输出一个视频中人物自然地说话/唱歌/手势。在RunComfy的`/feature/lip-sync`中被列为精选默认选项。
...
> 避免用于："肖像+音频"的简单工作——功能过剩，速度较慢。使用**OmniHuman**。
...
字节跳动的OmniHuman是最强的单次拍摄路径：输入**一张肖像图像+一个音频文件**，输出一个视频中主体自然地对音频说话/唱歌/手势。除了输入之外，无需其他提示词。
...
- **音频质量决定输出质量**——清晰的配音（无音乐底）→更干净的口型同步。如果您的音频是混音，请先分离人声。
...
## 路径2：Wan 2-7带`audio_url`——开源权重唇形同步
...
- **匹配音频的情感语气**——"自信表情" / "热情投入" / "冷静表达"提示面部表情。
- **摄像机语言**——"静态肖像"，"慢速推进"——与常规Wan 2-7 t2v调用相同。
...
当主体是**风格化角色**（插画、动漫、吉祥物）而不是写实肖像，并且您希望全身动作与音频同步时，选择此路径。社区发布的Wan 2-2基础变体。
...
- **精确引用说出的台词**，用`says clearly: "…"`。没有引号，模型会释义或跳过语音。
- **分开描述音频语气**——`"Audio: friendly tone, neutral accent."`——在说台词之外。
...
当虚拟形象工作是**电影级拍摄**的一部分时选择Seedance v2 Pro——从图像参考你的主体，从参考轨道参考你的音频，Seedance将它们用完整动作+镜头控制合成。
...
runcomfy run bytedance/seedance-v2/pro \
  --input '{
    "prompt": "广角鱼眼镜头——主体向镜头传递自信独白，黄金时刻光线透过窗户，浅景深。",
...
### 多语言品牌视频
- **OmniHuman**，使用同一肖像+不同语言的不同音频文件。同一身份，配音片段。

### 风格化吉祥物
- **Wan 2-2 Animate**，带插画角色+音频

### "写脚本，生成视频"（无音频文件）
- **HappyHorse 1.0 t2v**，脚本引用在提示词内
...
2. **OmniHuman**，用该肖像URL+您的语音

### 带自定义唇形同步到特定音频的口播视频
- **Wan 2-7**，带`audio_url`——最灵活的场景+锁定唇动
...
该技能分类用户请求——他们有预录制的音频文件，还是只有脚本？写实肖像还是风格化角色？单次拍摄还是电影级构图？——然后选择上述五个路径之一。然后调用`runcomfy run <model_id>`，匹配JSON正文。CLI向模型API POST，轮询请求状态，获取结果，并将任何`.runcomfy.net` / `.runcomfy.com` URL下载到`--output-dir`。
