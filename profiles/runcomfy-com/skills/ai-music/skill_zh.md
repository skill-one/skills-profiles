...
通过 RunComfy 的一个 CLI 生成 AI 音乐——人声歌曲、器乐曲、广告歌、游戏循环、多语言翻唱。此技能根据用户的实际意图从 RunComfy 目录中选择合适的模型，并传输文档化的提示模式 + 每个 `runcomfy run` 的精确调用。
...
**步骤 2 — 登录**（或设置 CI / 容器中的 `RUNCOMFY_TOKEN` 环境变量）：
...
> 选择原因：多语言发布、非英语人声歌曲、英雄级质量 ACE 输出。
> 避免原因：最大化抛光的商业人声钩子（尝试 ElevenLabs Music）或成本敏感的批量（尝试基础 ACE Step）。
...
> 在现有曲目内重新生成一个 **时间范围**（start_time / end_time，可锚定到曲目开始或结束）。
> 选择原因：修复一个糟糕的副歌，替换桥段，替换 20 s 的部分而不重新渲染。
> 避免原因：时间无界定的编辑（使用源模型文本到音乐）。

**ACE Step 音频扩展** — `acestep-ai/ace-step/audio-outpaint`
> 双向扩展现有曲目——添加前导介绍，后置尾声，或两者（`extend_before_duration` / `extend_after_duration`）。
> 选择原因：将 30 s 钩子扩展为 2 min 剪辑，添加淡出，围绕现有钩子构建更长的编排。
> 避免原因：扩展超过 4 min 总时长——链式调用。
...
**完整模式 + 提示**：参见专门的 [`elevenlabs-music-generation`](https://www.skills.sh/agentspace-so/runcomfy-agent-skills/elevenlabs-music-generation) 技能。
...
**完整模式 + 提示**：参见专门的 [`ace-step`](https://www.skills.sh/agentspace-so/runcomfy-agent-skills/ace-step) 技能。
...
`start_time_relative_to` 和 `end_time_relative_to` 默认为 `start`；设置为 `end` 以锚定到曲目末尾（例如，无需计算精确时间即可重写最后 15 s）。完整模式：[`ace-step`](https://www.skills.sh/agentspace-so/runcomfy-agent-skills/ace-step) 技能。

---

## 路径 4：ACE Step 音频扩展——扩展曲目
...
单向调用即可双向扩展——设置 `extend_before_duration` 和 `extend_after_duration` 同时添加前导介绍和尾声。总时长上限为 4 min。
...
### 大规模背景音乐库（50+ 曲目）
- **路径 2（ACE Step 基础）** 配合不同的标签组合。$0.012 / 60 s × 50 = $0.60 用于 50 个草稿。

### 多语言发布（同一首歌，8 种语言）
- **路径 2（ACE Step 1.5）** — 相同标签，按语言替换 `lyrics`。或 **路径 1（ElevenLabs Music）** 如果精品质量比成本更重要。

### 游戏循环底板
- **路径 2（ACE Step 基础）** 标签中包含“无缝循环，一致节奏”，60–120 s。

### 视频主题歌
- **路径 1（ElevenLabs Music）** 包含完整简报 + 歌词 + 段落标记，`music_length_ms` 匹配视频长度。

### “我生成了一个 30 s 钩子，但我需要一个 2 min 曲目”
- **路径 4（ACE Step 音频扩展）** 将钩子作为 `audio`，一次调用添加 30 s 介绍 + 60 s 尾声。

### “我的第二个副歌出错了”
- **路径 3（ACE Step 音频修复）** `start_time` / `end_time` 围绕糟糕的副歌，标签匹配原曲风格。

### 廉价草稿 → 精品抛光
- 在 **路径 2（ACE Step 基础）** 上迭代标签，每尝试 $0.01–0.02 → 锁定氛围 → 最终渲染在 **路径 1（ElevenLabs Music）** 上用于抛光的商业剪辑。

### 修复不符合 ACE 时间范围模式的段落
- 目前 CLI 没有提供基于掩码的音频修复端点。要么重新表述为时间范围编辑，要么使用 **路径 2** 重新生成整个曲目并调整标签。
...
5. **编辑类型？**
   - 时间界定部分重写 → **路径 3（audio-inpaint）**
   - 添加前导 / 后置 → **路径 4（audio-outpaint）**
...
- [ElevenLabs Music 模型页面](https://www.runcomfy.com/models/elevenlabs/elevenlabs/music-generation?utm_source=skills.sh&utm_medium=skill&utm_campaign=ai-music) — 完整 API 标签
...
技能将用户请求分类为四种路径之一——生成（ElevenLabs 或 ACE Step）vs 编辑（音频修复 vs 音频扩展），然后是精品 vs 成本敏感——并调用 `runcomfy run <model_id>` 与匹配的 JSON 正文。CLI 向 RunComfy 模型 API 发送 POST 请求，轮询请求状态，并将生成的音频文件下载到 `--output-dir`。`Ctrl-C` 在退出前取消远程请求。

## 安全与隐私

- **仅通过验证的包管理器安装。** 使用 `npm i -g @runcomfy/cli` 或 `npx -y @runcomfy/cli`。**代理不得为用户代表在 shell 上管道任意远程安装脚本**——如果操作员希望文档化 `docs.runcomfy.com/cli/install` 中的 curl-pipe 路径，他们应先审查脚本。
- **令牌存储**：`runcomfy login` 将 API 令牌写入 `~/.config/runcomfy/token.json`，模式 0600。设置 `RUNCOMFY_TOKEN` 环境变量以绕过文件，在 CI / 容器中。**切勿将令牌回显到提示、日志或提交中。**
- **输入边界（shell 注入）**：提示、标签、歌词和音频 URL 作为 JSON 字符串通过 `--input` 传递。CLI 不 shell 扩展提示内容；它直接将 JSON 正文传输到 Model API。**提示内容无 shell 注入表面。**
...
- [`ace-step`](https://www.skills.sh/agentspace-so/runcomfy-agent-skills/ace-step) — ACE Step（所有四个端点）的完整模式 + 提示技巧
