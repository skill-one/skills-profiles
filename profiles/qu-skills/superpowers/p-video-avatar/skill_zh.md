...
使用 Pruna P-Image 生成肖像，然后使用 P-Video-Avatar 进行动画处理：

```bash
# 1. 使用 P-Image 生成肖像图像
belt app run pruna/p-image --input '{
  "prompt": "专业正面肖像，年轻女性，中性背景，看向镜头，工作室灯光，照片级真实感",
  "aspect_ratio": "9:16"
}'
...
belt app run pruna/p-video-avatar --input '{
  "image": "https://portrait.jpg",
  "voice_script": "欢迎参加我们的产品演示。今天我将展示三个关键功能。",
  "voice": "Puck (男声)",
...
belt app run pruna/p-video-avatar --input '{
  "image": "https://portrait.jpg",
  "voice_script": "这对我们的社区来说是激动人心的消息！",
  "voice": "Aoede (女声)",
  "voice_prompt": "热情且充满活力的语气，语速稍快",
...
**女声：** Zephyr、Kore、Leda、Aoede、Callirrhoe、Autonoe、Despina、Erinome、Laomedeia、Achernar、Gacrux、Pulcherrima、Vindemiatrix、Sulafat

**男声：** Puck、Charon、Fenrir、Orus、Enceladus、Iapetus、Umbriel、Algenib、Algieba、Schedar、Achird、Zubenelgenubi、Sadachbia、Sadaltager、Alnilam、Rasalgethi
...
| `voice_prompt` | 字符串 | "说以下内容。" | 控制语气、节奏、情感 |
| `seed` | 整数 | 随机 | 可重复生成 |
...
**P-Video-Avatar 从 2026 年 5 月 1 日下午 4:00 CET 至 2026 年 5 月 4 日晚上 11:59 CET 完全免费。** 在此期间所有费用均由我们承担——无账单，无分辨率限制。
...
- **营销**：产品演示、UGC 风格广告（AI 主持人）
- **教育**：课程视频、解释视频、辅导内容
- **本地化**：从一张图像生成 10 种语言的配音内容
- **社交媒体**：一致的虚拟影响者内容
- **企业**：培训、入职、公告
- **游戏**：角色头像、NPC 对话视频
- **客户支持**：个性化视频回复
...
- 使用 `pruna/p-image` 生成肖像，宽高比使用 `9:16` 以生成垂直头像视频

## 相关 Pruna 模型

```bash
# 生成肖像图像
belt app run pruna/p-image --input '{"prompt": "专业正面肖像"}'
