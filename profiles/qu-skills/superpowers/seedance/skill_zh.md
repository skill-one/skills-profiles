...
在提示中使用 **类型 + 索引** 引用资产：`Image 1`，`Image 2`，`Video 1`，`Audio 1`。索引是您提供的数组中该类型的顺序位置。**不要**在提示中使用资产 ID。

**多模态参考公式：**
- 图像参考：`参考 [主体] 从 [Image N] 生成 [场景]，保持 [主体] 一致`
- 视频参考：`参考 [相机运动/动作] 从 [Video N]`
...
| `generate_audio` | 布尔值 | true | 生成同步音频 |
| `duration` | 整数 | 5 | 持续时间（秒）（4-15），或 -1 为自动 |
| `ratio` | 枚举 | adaptive | 21:9, 16:9, 4:3, 1:1, 3:4, 9:16，或 adaptive |
| `resolution` | 枚举 | 720p | 480p, 720p, 1080p（快速：仅 480p, 720p） |
| `seed` | 整数 | -1 | 用于可重复性的种子（-1 为随机） |
| `watermark` | 布尔值 | false | 在输出中添加水印 |
| `safety_identifier` | 字符串 | - | 用于安全策略的唯一终端用户标识符（最多 64 个字符，推荐为用户 ID 的哈希值） |
| `image` | 文件 | - | 第一帧图像（与参考输入互斥） |
| `end_image` | 文件 | - | 最后一帧图像（需要 `image`） |
| `reference_images` | 文件[] | - | 参考图像，最多 9 个（与 image/end_image 互斥） |
| `reference_videos` | 文件[] | - | 参考视频，最多 3 个。每个最长 15 秒，总最长 15 秒。mp4/mov |
| `reference_audios` | 文件[] | - | 参考音频，最多 3 个。每个最长 15 秒，总最长 15 秒。wav/mp3。至少需要一个图像或视频 |

## 定价

| 模型 | 定价 |
|------|------|
| Seedance 2.0 | $4.30-$7.70/M tokens（根据分辨率和输入类型变化） |
| Seedance 2.0 快速 | $3.30-$5.60/M tokens |

Token 公式：`(width x height x fps x duration) / 1024`
