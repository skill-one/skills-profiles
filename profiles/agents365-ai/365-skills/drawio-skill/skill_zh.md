...
| 无特殊样式的标准流程图/思维导图/Gantt/时间轴等 | 如果 draw.io >=30，阅读 `references/mermaid-authoring.md` 并将 Mermaid 转换为原生 `.drawio` |
| 需要 自动布局 的大型图（~15+ 节点） | 使用 `autolayout.py`；在传递任何 `--layout` 值之前，阅读 `references/autolayout.md` |
...
| 绘制为地铁/轻铁地图的流程图、旅程或子系统图 | 使用 `tubemap.py`；阅读 `references/tubemap.md` |
| 形状、云/供应商、AI 或 Databricks 图标 | 阅读 `references/shapes.md` 或 `references/databricks.md`；永远不要猜测形状名称 |
...
从该技能目录运行，或替换 `scripts/` 为此技能脚本目录的绝对路径：
...
2. 从上表中选择编写路径。对于数据支持的图表，优先使用图表 IR 并保留来源。对于大型图，使用导入器或 `autolayout.py`；不要手动放置超过大约十五个节点。
3. 解析明确命名的样式预设，或用户默认预设，如 `references/style-presets.md` 中所述。结构化图表约定和视觉预设组合；它们不互相替换。
4. 生成 `.drawio`，然后运行结构化验证：

   ```bash
   python3 scripts/validate.py diagram.drawio --score
   ```

   当涉及语义元数据或架构策略时，也运行 `diagramctl.py test`。不要将推断的语义发现呈现为已验证的运行时事实。
5. 导出一个不带嵌入式 XML 的草稿 PNG 并进行视觉检查。修复明显的重叠、裁剪、断开连接的边、边穿过节点路由、堆叠的边和难以读取的标签。在两轮后停止自动视觉修复。
...
不要组合 `--width` 和 `-s`。嵌入式 PNG 导出需要 `repair_png.py`；用于视觉的草稿 PNG 不能使用 `-e`。在 Linux 无头环境下，遵循 `references/troubleshooting.md` 而不是临时修改 Electron 标志。如果 CLI 在 macOS 沙盒中崩溃，尝试一次允许的升级运行，然后使用 `encode_drawio_url.py` 或交付 XML；不要重复启动它。

## 编辑和身份

- 使用稳定的语义 ID，并永远不要重用保留 ID `0` 或 `1`。
- 每条边都需要 `<mxGeometry relative="1" as="geometry"/>`。
- 对于本地编辑，仅更改匹配的单元；对于全局方向更改，重新生成/重新布局页面。
- 保留来源、`data-model-id`、语义属性、手动几何形状和手动样式，除非用户请求否则不变。
- 在协调时，默认情况下将删除保留为可评审的褪色元素。
