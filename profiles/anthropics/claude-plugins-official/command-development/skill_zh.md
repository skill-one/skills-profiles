...
> **注意**：`.claude/commands/` 目录是遗留格式。对于新技能，请使用 `.claude/skills/<name>/SKILL.md` 目录格式。两者加载方式相同——唯一区别在于文件布局。有关首选格式的说明，请参阅 `skill-development` 技能。

## 概述

斜杠命令是定义在 Markdown 文件中的常用提示，Claude 在交互式会话期间执行这些命令。理解命令结构、前文选项和动态特性，可以创建强大且可重用的工作流。
...
斜杠命令是一个包含 Claude 在被调用时执行的提示的 Markdown 文件。命令提供：
...
**命令是编写给代理（agent）消费的，而不是人类消费的。**

当用户调用 `/command-name` 时，命令内容成为 Claude 的指令。将命令作为对 Claude 的指令来编写，告诉它该做什么，而不是作为对用户的消息来编写。
...
第一个示例告诉 Claude 该做什么。第二个示例告诉用户会发生什么，但没有给 Claude 指令。始终使用第一种方法。
...
```
修复编号 #123 的问题，遵循我们的编码标准...
修复编号 #456 的问题，遵循我们的编码标准...
```
...
命令可以内联执行 bash 命令，在 Claude 处理命令之前动态收集上下文。这对于包含仓库状态、环境信息或项目特定上下文非常有用。
...
**实现细节：**
有关完整语法、示例和最佳实践，请参阅 `references/plugin-features-reference.md` 中的 bash 执行部分。参考文档包含精确语法和多个工作示例，以避免执行问题
