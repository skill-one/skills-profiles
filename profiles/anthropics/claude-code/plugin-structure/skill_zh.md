...
**使用方式**：命令作为原生斜杠命令集成到 Claude Code 中
...
**使用方式**：用户可以手动调用代理，或 Claude Code 根据任务上下文自动选择

### 技能

**位置**：`skills/` 目录，每个技能一个子目录
**格式**：每个技能在其自己的目录中包含 `SKILL.md` 文件
...
**使用方式**：Claude Code 根据任务上下文与描述匹配自动激活技能
...
**使用方式**：MCP 服务器与 Claude Code 的工具系统无缝集成
...
2. **清晰性**：使用描述性名称表明用途
   - 良好：`api-integration-testing/`，`code-quality-checker.md`
...
2. **多系统测试**：在 macOS、Linux、Windows 上验证
3. **文档依赖**：列出所需工具和版本
4. **避免系统特定特性**：使用可移植的 bash/Python 语法

### 维护

1. **版本一致**：发布时更新 `plugin.json` 中的版本号
2. **优雅弃用**：在移除前清晰标记旧组件
3. **文档重大变更**：注明影响现有用户的变更
4. **彻底测试**：变更后验证所有组件
...
- 确认技能包含 `SKILL.md`（不是 `README.md` 或其他名称）
