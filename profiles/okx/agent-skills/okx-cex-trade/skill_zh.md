...
在 OKX 交易所上进行现货、永续互换、交割期货、**期权**和**事件合约**的订单管理。下单、取消、修改和监控订单；查询期权链和希腊字母；交易二元结果事件合约（是/否，上/下）；设置止盈/止损和跟踪止损；管理杠杆和仓位。**需要 API 凭证。**
...
运行**两个**命令 — `okx auth status --json` 中的 `apiKey` 字段是身份验证二进制程序的内部状态，无论 `~/.okx/config.toml` 是否有 API 密钥配置文件，它始终为 `false`。`okx config show --json` 是 API 密钥存在的唯一权威来源。
...
- `config show --json` 具有非空的 `api_key` 字段**的任何配置文件** → **API 密钥模式**。继续步骤 B。
...
|---|---|---|---|
| 实盘 (实盘) | 真实资金 — 不可逆 | `--profile <实盘配置文件>` | *(默认，无标志)* |
...
- 对于**交易**事件合约（放置/取消/修改预测市场订单）→ 使用 `okx-cex-trade` 与 `okx event place` / `okx event cancel` / `okx event amend`
- 对于网格和 DCA 交易机器人 → 使用 `okx-cex-bot`

> **重要**：当用户询问事件合约或预测市场背景下的“合约”时，路由到此技能 — **不要**路由到 `okx-cex-portfolio`。投资组合不处理事件合约 — 它仅涵盖账户余额、仓位、盈亏和转账。

## 期权合约的 Sz 处理

### ⚠ 关键：下单前始终验证合约面值

在放置任何 SWAP/FUTURES/OPTION 订单之前，调用 `market_get_instruments` 以获取 `ctVal`（合约面值）。**不要**假设合约大小 — 它们因工具而异（例如 ETH-USDT-SWAP = 0.1 ETH/合约，BTC-USDT-SWAP = 0.01 BTC/合约）。
...
→ **模糊** — 这可能意味着名义价值或保证金成本。
  您必须在使用前询问用户澄清：
  - **名义价值**：sz = 美元价值（例如 500 USDT 买入 500 USDT 价值的合约）
  - **保证金成本**：实际仓位 = sz × 杠杆（例如 500 USDT 保证金在 10× = 5000 USDT 名义价值）
  等待用户的答案再继续。
...
当用户为期权指定 USDT 金额时，使用 `--tgtCcy quote_ccy`（名义价值）或 `--tgtCcy margin`（保证金成本）并将金额作为 `--sz` 传递。系统自动将其转换为合约。注意：期权合约通常具有较大的面值（例如 ctVal=1 BTC ≈ $84,000），因此 1 合约的最低 USDT 金额很高。对于期权卖方（`cross`/`isolated` tdMode），`margin` 模式自动考虑杠杆。
...
| # | 命令 | 类型 | 描述 |
|---|---|---|---|
| 1 | `okx spot place` | WRITE | 放置现货订单 (市场/限价/仅限/IOC) |
| 2 | `okx spot cancel` | WRITE | 取消现货订单 |
| 3 | `okx spot amend` | WRITE | 修改现货订单价格或大小 |
| 4 | `okx spot algo place` | WRITE | 放置现货止盈/止损算法订单 |
| 5 | `okx spot algo amend` | WRITE | 修改现货止盈/止损级别 |
| 6 | `okx spot algo cancel` | WRITE | 取消现货算法订单 |
| 7 | `okx spot algo trail` | WRITE | 放置现货跟踪止损订单 |
...
| 12 | `okx spot leverage` | WRITE | 为现货**保证金**设置杠杆（借入）。按合约级别 (`--instId`) 或货币级别交叉 (`--ccy`，需要借入/多币种/投资组合保证金） |
...
| # | 命令 | 类型 | 描述 |
|---|---|---|---|
| 13 | `okx swap place` | WRITE | 放置永续互换订单 |
| 14 | `okx swap cancel` | WRITE | 取消互换订单 |
| 15 | `okx swap amend` | WRITE | 修改互换订单价格或大小 |
| 16 | `okx swap close` | WRITE | 以市场方式关闭整个仓位 |
| 17 | `okx swap leverage` | WRITE | 为工具设置杠杆 |
| 18 | `okx swap algo place` | WRITE | 放置互换止盈/止损算法订单 |
| 19 | `okx swap algo trail` | WRITE | 放置互换跟踪止损订单 |
| 20 | `okx swap algo amend` | WRITE | 修改互换算法订单 |
| 21 | `okx swap algo cancel` | WRITE | 取消互换算法订单 |
| 22 | `okx swap positions` | READ | 开放的永续互换仓位 |
...
| 44 | `okx option greeks` | READ | 由基础资产提供的隐含波动率 + 希腊字母（delta/gamma/theta/vega） |
| 45 | `okx option place` | WRITE | 放置期权订单（看涨或看跌，买方或卖方） |
...
| 51 | `okx option positions` | READ | 带有实时希腊字母的开放期权仓位 |
...
| 57 | `okx event place ...` | WRITE | 放置事件订单（结果必需） |
...
- 放置/取消/修改订单 → `okx spot place/cancel/amend`
- TP/SL 条件 → `okx spot algo place/amend/cancel`
- 跟踪止损 → `okx spot algo trail`
- 查询 → `okx spot orders/get/fills/algo orders`

**Swap/永续**（instId 格式：`BTC-USDT-SWAP`）:
- 放置/取消/修改订单 → `okx swap place/cancel/amend`
- 关闭仓位 → `okx swap close`
...
- 放置/取消/修改订单 → `okx futures place/cancel/amend`
- 关闭仓位 → `okx futures close`
...
- 放置/取消/修改 → `okx option place/cancel/amend`
...
工具 ID (`instId`, API 字段) 格式: `{UNDERLYING}-{TYPE}-{YYMMDD}-{HHMM}-{STRIKE}` 对于 "目标价以上" / "触碰目标" 合约（例如 `BTC-ABOVE-DAILY-260224-1600-70000`），或 `{UNDERLYING}-{TYPE}-{YYMMDD}-{START}-{END}` 对于 "价格方向 (上/下)" 合约（例如 `BTC-UPDOWN-15MIN-260224-1600-1615`）。始终从 `okx event markets <seriesId>` 获取工具 ID — 永远不要猜测或使用占位符。

系列 ID (`seriesId`, API 字段): 可读的（例如 `BTC-ABOVE-DAILY`, `BTC-UPDOWN-15MIN`）或内部随机字符串（例如 `FMQRZ`）。两者都有效。从 `okx event series` 获取。

事件合约交易流程:
1. **发现** → `okx event browse`（首选，在一个调用中返回系列 + 实时市场）— 以分组方式显示结果；突出显示命名系列；始终显示系列 ID
2. **浏览实时市场** → `okx event markets <seriesId> --state live` — 获取可交易的合约的工具体价；如果显示实时价格，它是事件合约价格（0.01–0.99），不是基础资产价格 — 反映了在积极交易时市场隐含的概率
3. **检查事件详情** → `okx event events <seriesId>`
4. **确认 + 放置** → `okx event place <instId> <side> <outcome> <sz>` — 只有在用户明确确认后才能执行
...
边缘情况:
- **已结算结果**: `okx event markets <seriesId> --state expired` — 没有单独的结束工具

**事件合约 sz 规则**:

- **市场订单** (`ordType=market`): `--sz` 是计价货币金额。
- **限价订单** (`ordType=limit` / `post_only`): `--sz` 是合约数量（整数）。每个合约结算为 1 个计价货币单位；每个合约成本 = `px`（事件合约价格，0.01–0.99）。例如 10 合约在 px=0.5 的成本为 5。
- **px 语义**: `px` 是事件合约价格（0.01–0.99），**不是**基础资产价格。在积极交易时，它反映了市场隐含的概率。示例：`px=0.6` 意味着市场将事件定价在大约 60%。

**结果显示**: 过期/结果视图显示翻译后的值。对于 `price_up_down`, 将 `YES/NO` 视为 `UP/DOWN`。

有关事件合约工作流程和分步示例，请阅读 `{baseDir}/references/event-workflows.md`。

有关跨技能工作流程和分步示例，请阅读 `{baseDir}/references/workflows.md`。

### 步骤 2 — 确认配置文件，然后确认写入参数

**读取命令**（订单、仓位、成交记录、获取、获取杠杆、algo 订单）：立即运行。

- `--history` 标志：默认为活动/开放；如果用户明确要求历史记录，才使用 `--history`
- `--ordType` 对于 algo: `conditional` = 单个 TP 或 SL; `oco` = 两者 TP 和 SL 一起
- `--tdMode` 对于 swap/futures: `cross` 或 `isolated`; 现货始终使用 `cash`（自动设置）
- `--posSide` 对于对冲模式: `long` 或 `short`; 在净模式中省略

**写入命令**（放置、取消、修改、关闭、杠杆、algo）：在执行之前确认关键订单详细信息一次：
- 现货放置：确认 `--instId`, `--side`, `--ordType`, `--sz`（如果计价货币金额，则 `--tgtCcy quote_ccy`）

- 互换/永续放置：确认 `--instId`, `--side`, `--sz`, `--tdMode`，并且**当用户指定 USDT 金额时，明确确认订单模式**：`--tgtCcy quote_ccy`（名义价值，sz = 美元价值）或 `--tgtCcy margin`（保证金成本，实际仓位 = sz × 杠杆）。始终说明正在使用哪种模式。
- 期权放置：确认 `--instId`, `--side`, `--sz`, `--tdMode`（如果 USDT 金额 — 系统自动转换为合约）；不要附加 TP/SL
- 事件合约放置：确认 `--instId`, `--side`, `--outcome`, `--sz`, `--ordType`; 对于市场订单 sz 是计价货币金额，对于限价订单 sz 是合约数量 + `--px` 要求
- 互换/永续关闭：确认 `--instId`, `--mgnMode`, `--posSide`
- 杠杆：确认新的杠杆以及对现有仓位的影响。**避免常见 400 错误的预检查**： (a) `--lever` 必须是正数，在工具的最大值内（参见 `okx market instruments` → `lever`）； (b) 在对冲仓位模式下，`--mgnMode isolated` 中 `--posSide` 是必需的 — 每个方面 (`long`, `short`) 必须分别设置，设置一个方面**不会**自动应用于另一个方面； (c) **投资组合保证金账户不能调整 SWAP/FUTURES 的交叉杠杆** — OKX 将拒绝；如果不确定，请运行 `okx account config` 并检查 `acctLv` 首先确认。**如果设置杠杆失败**（错误提到 "取消订单或停止机器人”）: 按优先顺序解决问题 — (1) 首先查询待处理的 algo 订单 (`swap/futures algo-orders --status pending`), 因为这是最常见的阻止因素； (2) 如果没有 algo 订单, 检查活跃的机器人 (`bot grid-orders`). **不要**自动取消订单或停止机器人 — 显示结果并让用户决定

有关每个命令的完整参数详细信息，请阅读相关参考文件。

### 错误建议的纠正保护措施

当 OKX API 错误消息建议的修复涉及**写入操作**（取消订单、关闭仓位、停止机器人/策略、转账等）时，您**必须**不自动执行这些操作。相反：

1. **报告** 错误及其建议给用户，原样输出
2. **诊断** — 运行只读查询以识别阻止因素（例如 `algo-orders --status pending`, `positions`, `bot grid-orders --status active`)
3. **显示发现结果** — 向用户显示发现的内容以及需要取消/关闭/停止的特定项目
4. **等待明确确认** 才执行任何纠正

这适用于所有错误代码，其消息建议破坏性行动，包括但不限于：
- 由待处理的 algo 订单阻止的 Set-leverage (错误代码 59000, 59002, 59007)
- 需要订单/仓位/策略清理的账户设置更改（例如错误代码 59000, 59002, 59007）
- 任何包含短语如 "取消", "关闭", "停止", "转账..." 的错误

**理由**：错误消息列出所有可能的阻止因素，实际阻止因素通常是单个项目（例如单个 TP/SL 订单）。盲目遵循错误文本可能导致用户未打算的仓位关闭或机器人停止。

## 全局说明

- 所有写入命令都需要有效凭证（OAuth 会话或 `~/.okx/config.toml` 中的 API 密钥）
- 身份验证方法和交易模式在“凭证与配置文件检查”步骤 A 中确定；请参阅该部分以获取参数规则
- `--json` 默认返回原始 OKX API v5 响应。添加 `--env` 将输出包装为 `{"env": "<实盘|演示>", "profile": "<名称>", "data": <响应>} — 当您需要知道活动环境和凭证配置文件时，这很有用
- 速率限制：每 2 秒 60 个订单操作每个 UID
- 批量操作（批量取消、批量修改）如果需要，可以通过 MCP 工具直接使用
- 仓位模式 (`net` 与 `long_short_mode`) 影响是否需要 `--posSide`
