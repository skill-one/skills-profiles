...
将自然语言文件（CLAUDE.md、待办事项、偏好设置）压缩成猿人语言以减少输入令牌。压缩后的版本会覆盖原始文件。人类可读的备份保存为`<filename>.original.md`，但不会放在源文件旁边——它存在于一个树外数据目录中（`$XDG_DATA_HOME/caveman-compress/backups/<父目录名>/`，或在 Windows 上为`%LOCALAPPDATA%\caveman-compress\backups\<父目录名>\`），这样技能自动加载器就不会将其重新作为活动文件加载。
...
- 片段式表达：允许 "Run tests before commit" 而不是 "You should always run tests before committing"
...
- 如果不确定某部分是代码还是散文，保留原样
- 原始文件在覆盖前备份为 FILE.original.md —— 存放在树外备份数据目录（见目的），不在源文件旁边
