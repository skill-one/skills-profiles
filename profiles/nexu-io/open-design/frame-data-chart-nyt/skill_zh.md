【模板: NYT 风数据图表帧】
【意图】将一段数据 (CSV / JSON / 一句结论) 制作成《纽约时报》专栏风格的单帧/动画图表，适用于视频片段或推特卡。灵感来源于 hyperframes data-chart。

【画布】1920×1080，暖白底 `#f7f5ee` 或墨黑底 `#0e0e0e` 二选一；文字色和背景相反。

【布局】
- **顶部 kicker** (11px 大写字母间距 0.14em，颜色 = 强调红 `#a91d1d` 或薄荷绿 `#5fb38a`): 数据来源 + 类目，如 "GLOBAL · WEEKLY ACTIVE USERS · 2018–2026"。
- **大字标题** (Cheltenham / Playfair / Source Serif Pro, 5.6vw, 斜体副标可选): 一句结论。**结论必须从用户数据中提炼**，不是描述图。
- **图表区** (占画布 55-65%):
  - 折线: 1-2 条线，主线 ink 实心 2.5px，次线 dashed 1.5px；数据点用 6px 实心圆；关键点旁标注 `2024 · 412M` 黑色 mono 小字。
  - 柱状: 全部 ink 单色或加 1 道强调高亮柱；柱顶大数字；柱底类目斜体 (Cheltenham italic)。
  - 范围带 (range band): 浅灰填充 `#e6e2d2` 包络 + 中线 ink。
- **底部 source + footnote** (10px mono，不透明度 0.6): "Source: 用户数据 · Chart by html-anything"。
