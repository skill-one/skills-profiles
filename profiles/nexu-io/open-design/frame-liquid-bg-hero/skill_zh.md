【模板: 流体背景 Hero】
【意图】可用于视频片头帧、SaaS landing 页面顶部 hero、海报背景图。WebGL 流体效果，但使用 CSS / canvas 降级绘制，确保单文件可双击打开。灵感来源于 hyperframes vfx-liquid-background。

【画布】1920×1080 (横) 或 1080×1920 (竖)，二选一。背景需填满。
...
1. **CSS 多层 radial-gradient 错位呼吸** (最稳定，默认推荐):
   ...
   - 顶层加 1 层 `backdrop-filter: blur(80px)` 让边缘更模糊。
...
- 单文件 HTML，可被 `prefers-reduced-motion` 关闭动效。
