【模板: 胶片漏光电影帧】
【意图】纪录片 / 个人短片 / 视频章节卡的开场单帧 —— 暖橙漏光 + 35mm 颗粒 + 衬线大字, 古典胶片质感。受超帧漏光效果启发。

【画布】
- **2.39:1 字幕框** (推荐): 1920×800, 上下黑边各 140px (`#000`)。
- 或 16:9: 1920×1080, 无字幕框。

【背景】
- 底层: 深暖色 (深红棕 `#1a0d08` / 墨绿 `#0a1410` / 蓝紫 `#0d0e1a`) 或场景描绘 (CSS 渐变模拟天空 / 室内 / 室外)。
- **胶片漏光 (Light Leak)**: 2-3 个右上大 `radial-gradient(ellipse at top right, #ffb547 0%, transparent 50%)` + 1 个底部 `linear-gradient(to top, #d97757 0%, transparent 30%)`; 颜色取暖橙 / 桃 / 玫红 / 暗黄, **禁止冷蓝**。
...
- "胶片齿孔": 字幕框黑边内, 等距小白方块 (CSS repeating-linear-gradient)。
