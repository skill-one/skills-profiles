...
> [!NOTE] **产品重命名**: Firebase Data Connect 已更名为 **Firebase SQL Connect**。本技能仓库中所有引用 "Data Connect" 或 "Firebase Data Connect" 的说明、参考和示例均适用于 "SQL Connect" 和 "Firebase SQL Connect"。
...
1. **验证操作**: 运行 `npx -y firebase-tools@latest dataconnect:compile` 对架构进行验证。
...
遵循以下严格的工作流程来构建您的应用程序。您 **必须** 阅读每个步骤的链接参考文件以了解语法和可用功能。
...
> - **Upserts**: 使用 `_upsert` 来 "插入或更新" 记录 (用户配置文件的关键)。
...
| **Upsert 数据**              | [reference/operations.md](reference/operations.md)           | `_upsert` 变更                                |
| **复杂过滤**             | [reference/operations.md](reference/operations.md)           | `_or`, `_and`, `_not`, `eq`, `contains`            |
...
| **数据播种 & 迁移**   | [reference/data_seeding.md](reference/data_seeding.md)       | `seed_data.gql`, `_insertMany`, Admin SDK 批量     |
...
1. 运行 `npx -y firebase-tools@latest dataconnect:compile` 或
   `npx -y firebase-tools@latest dataconnect:sdk:generate` 来验证它们。
