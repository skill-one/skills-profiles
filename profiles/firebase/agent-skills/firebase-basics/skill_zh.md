...
1. **本地环境设置**：验证环境是否已正确设置，以便我们可以使用 Firebase 工具：
   ...
   - 运行 `npx -y firebase-tools@latest --version` 检查是否已安装 Firebase CLI。
   - 使用现有工具验证是否已安装 Firebase MCP 服务器。
   - **关键**：在配置任何扩展或代理环境之前，您**必须**阅读
     [参考资料/本地环境设置.md](references/local-env-setup.md)。
   - **不要跳过**此步骤：如果 'firebase-basics' 是您可用的唯一 Firebase 技能，您必须遵循针对您的代理环境的参考文档来设置完整的 Firebase 技能套件：
     - **Gemini CLI**：查看
...
1. **身份验证**：确保您已登录 Firebase，以便命令具有正确的权限。运行 `npx -y firebase-tools@latest login`。对于没有浏览器的环境（例如远程 Shell），使用
...
1. **活动项目**：大多数 Firebase 任务需要一个活动项目上下文。
   ...
   - **如果创建新项目**：运行以下命令来创建它：
     ...
     *注意：`<project-id>` 必须是 6-30 个字符，小写，可以包含数字和连字符。它必须是全局唯一的。*

# Firebase 使用原则

请遵守以下原则：

1. **使用 npx 运行 CLI 命令**：为确保您始终使用最新版本的 Firebase CLI，请始终在命令前缀 `npx -y firebase-tools@latest` 而不是直接使用 `firebase`。例如，使用
   `npx -y firebase-tools@latest --version`。**绝对不要**建议裸 `firebase` 命令作为替代方案。
...
1. **遵循代理技能以获取实现指导**：技能提供了有意见的工作流程（CUJs）、安全规则和最佳实践。请始终参考它们以了解如何正确实现 Firebase 功能，而不是依赖一般知识。
1. **使用 Firebase MCP 服务器工具而不是直接 API 调用**：当您需要与远程 Firebase API 交互（例如获取 Crashlytics 日志或执行 Data Connect 查询）时，请使用 Firebase MCP 服务器提供的工具，而不是尝试手动 API 调用。
1. **保持插件 / 代理技能更新**：由于 Firebase 最佳实践发展迅速，请定期检查并安装 Firebase 插件或代理技能的更新。同样，如果您遇到过时工具或命令的问题，请根据您的代理环境遵循以下步骤：
   - **Antigravity**：遵循
   ...
   - **其他**：遵循
     [参考资料/刷新/other-agents.md](references/refresh/other-agents.md)
1. **自动化配置文件获取**：在设置 iOS 或 Android 应用时，**不要**直接让用户前往 Firebase 控制台下载 `google-services.json` 或 `GoogleService-Info.plist`。相反，请使用 Firebase CLI 以编程方式获取配置：
   - 对于 Android：
   ...
   - 对于 iOS：
     ...
     将输出保存到适当的位置（例如，
     `app/google-services.json` 对于 Android，或 `xcode-project-setup` 链接的路径对于 iOS）。
...
- **SDK 设置**：有关将 Firebase 添加到应用的详细指南：
  ...
  - **Android**：请参阅 [参考资料/android_setup.md](references/android_setup.md)
  - **iOS**：请参阅 [参考资料/ios_setup.md](references/ios_setup.md)

# 常见问题

- **登录问题**：如果登录步骤中浏览器无法打开，请使用
  `npx -y firebase-tools@latest login --no-localhost` 替代。
