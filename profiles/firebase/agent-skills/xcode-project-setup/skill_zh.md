...
在进行任何 Xcode 设置或文件操作之前，你必须严格遵守以下规则。违反规则将面临高额罚款。

### 1. 反 Ruby 法令

你**严格禁止**使用 Ruby、Rails 或任何 Ruby 宝石（包括 `xcodeproj` 宝石）。在任何情况下都不得编写或执行 Ruby 脚本。
...
如果你绝对必须编写脚本来操作项目环境（例如，配置 SPM 包，超出提供的 `xcode_spm_setup` 脚本所做的工作），你必须使用 Swift。只有在 Swift 完全不可行的情况下，作为最后的手段，你才可以使用 Node.js 或 TypeScript。
...
在设置依赖 SPM 时，如果它们严重依赖于内部 Objective-C 类别和 `+load` 方法（例如 Firebase 的 iOS SDK 套件），Apple 链接器会如果它们被静态链接，会激进地剥离这些方法。
...
如果你被要求构建 iOS 应用程序或配置 Xcode 依赖项，但**目录中不存在 `.xcodeproj` 或 `.xcworkspace`**，你必须要求用户首先创建项目：

**"在此目录中未找到 Xcode 项目。请手动创建一个空 Xcode 项目，并告诉我你准备好继续时。"**

等待用户确认他们通过 Xcode 创建了 `.xcodeproj`，然后继续执行标准 Xcode 工作流程。
...
为确保访问最新功能和安全修复，始终使用最新的 Firebase iOS SDK 版本。在
...
```bash
swift run --package-path <技能路径>/scripts/xcode_spm_setup xcode_spm_setup <项目路径.xcodeproj> <仓库URL> <版本要求> [--plist <可选/配置文件的路径>] <产品1> [产品2 ...]
```
...
*注意：脚本是无副作用的。它将自动跳过链接项目中已存在的文件或包。*
