...
基于 Rolldown 和 Oxc 的 TypeScript/JavaScript 库的极速捆绑器。

## 运行时要求

`tsdown` 需要 **Node.js 22.18.0 或更高版本运行**（仅构建时）。然而，通过 [`target`](references/option-target.md) 选项，捆绑的输出可以针对更低版本的 Node.js，因此使用 tsdown 构建的库在 **运行时不受 Node.js 22+ 的限制**。

如果你的包需要支持 Node.js 18 / 20：

- **在 CI 中使用 Node.js 22+ 构建**（例如，设置 `target: 'node18'` 或 `target: 'node20'`）。
- **在打算支持的较低 Node.js 版本上测试构建的输出（或打包的 tarball）** — 例如，使用矩阵作业在 Node.js 18 / 20 / 22 上运行已发布包的测试。
...
| 配置文件 | 配置文件格式、多个配置文件、工作区 | [option-config-file](references/option-config-file.md) |
...
| 编程式 API | 从 Node.js 脚本构建、内存中构建（`write: false`） | [advanced-programmatic](references/advanced-programmatic.md) |
...
> 要获得完整的迁移辅助和完整的选项映射，请安装专用的 [`tsdown-migrate`](../tsdown-migrate/SKILL.md) 技能：`npx skills add rolldown/tsdown --skill tsdown-migrate`
...
| 日志级别 | `logLevel: 'silent'`, `failOnWarn: false`, `suppressWarnings: [...]` | [option-log-level](references/option-log-level.md) |
...
| 仅导入 | `deps: { onlyImport: ['cac'] }` - 在输出中白名单运行时导入 | [option-dependencies](references/option-dependencies.md) |
| 外部化所有 | `deps: { neverBundle: true }` | [option-dependencies](references/option-dependencies.md) |
| 解析子路径 | `deps: { resolveDepSubpath: true }` - 解析外部子路径导入（默认 `false`） | [option-dependencies](references/option-dependencies.md) |
...
| CSS 处理 | **[实验性]** `css: { ... }` — 使用预处理器、Lightning CSS、PostCSS、CSS 模块、代码拆分；需要 `@tsdown/css` | [option-css](references/option-css.md) |
...
| 复制文件 | `copy: 'public'` 或 `copy: { from, to, flatten, rename }` - 将静态资源复制到输出 | [option-copy](references/option-copy.md) |
...
| 可执行文件 | **[实验性]** `exe: true` - 捆绑为独立的可执行文件，通过 `@tsdown/exe` 实现跨平台 | [option-exe](references/option-exe.md) |
...
export default defineConfig({
  ...
  format: ['esm', 'cjs'],
  dts: true,
  failOnWarn: 'ci-only',  // 选择性：在 CI 中失败警告
  publint: 'ci-only',
  attw: 'ci-only',
})
```
...
tsdown --filter /^pkg-/        # 通过正则筛选
tsdown -W --concurrency 4      # 限制并行 Rolldown 构建
...
6. **自动生成 package.json 导出**：
   ```ts
   { exports: true }  // 创建正确的导出字段
   ```
