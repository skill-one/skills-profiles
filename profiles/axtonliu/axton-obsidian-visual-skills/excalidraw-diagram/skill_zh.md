...
1. **从触发词检测输出模式**（参考上方输出模式表格）
2. 分析内容 - 识别概念、关系、层级
3. 选择图表类型（参考下方图表类型选择指南）
...
tags: [excalidraw]
---
==⚠  切换到 EXCALIDRAW 视图在文档的 MORE OPTIONS 菜单中。 ⚠== 您可以使用命令面板解压缩绘图数据：'解压缩当前 Excalidraw 文件'。更多信息请查看插件设置中的 '保存'
...
每个元素需要这些字段（**不要**添加 `frameId`, `index`, `versionNonce`, `rawText` 等字段 - 它们可能导致 excalidraw.com 出现问题。`boundElements` 必须是 `null` 而不是 `[]`，`updated` 必须是 `1` 而不是时间戳）：
...
### 文本元素处理
- `## 文本元素` 部分在 Markdown 中**必须留空**，仅用 `%%` 作为分隔符
...
| 模式 | 文件名格式 | 示例 |
|------|-----------|------|
| Obsidian | `[主题].[类型].md` | `商业模式.关系.md` |
| Standard | `[主题].[类型].excalidraw` | `商业模式.关系.excalidraw` |
| Animated | `[主题].[类型].animate.excalidraw` | `商业模式.关系.animate.excalidraw` |
...
保存位置：商业模式.关系.animate.excalidraw
