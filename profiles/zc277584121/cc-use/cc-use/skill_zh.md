...
将当前 Agent 作为外层监督者，将 tmux 中的 Codex CLI 或 Claude Code 作为内层执行者。

`scripts/cc-use` 只负责创建 session、发送输入、观察屏幕、记录内层进程退出状态和清理资源。它不理解任务内容，也不判断屏幕表示完成、失败或阻塞。所有任务语义判断都由外层 Agent 完成。

## 运行模型与关键概念

cc-use 用 tmux 将执行上下文和监督上下文分开。内层 Agent 可以读取大量文件、运行长命令并产生很多终端输出；外层 Agent 保留用户目标、风险和验收标准，只在适合判断的时刻查看屏幕证据。
