...
这允许在 Studio 中设置样式和关键帧。要明智一些，如果一个组件包含许多元素，时间轴可能会变得混乱。
...
最佳方式是直接将一个普通对象传递给 `style` —— 不引用常量，不展开对象，不做数学运算。
...
将动画作为在变化属性上的内联 `interpolate()` 调用编写。
输出范围、缓动、外推和 `output` 属性应使用硬编码值。

输入范围可以额外使用从 `useVideoConfig()` 直接解构的 `durationInFrames`、`fps`、`width` 和 `height`。支持裸标识符，如 `durationInFrames`、乘以数字如 `2 * fps` 或 `fps * 2`，以及减去数字如 `durationInFrames - 1`。
...
效果数组不应进行计算。
设置关键帧与 `interpolate()` 的规则相同：所有值也应该是硬编码的：输入范围、输出范围、缓动、外推、`output` 属性。
...
如果一个 Remotion 组件主要由视频和音频片段组成，请参阅视频编辑以了解如何构建 Remotion 标记的最佳实践，以便片段在时间轴中可交互编辑。
