...
在创建视觉效果时，考虑是否可以使用 CSS 和 HTML 实现，或者是否需要着色器。
顺序或优先级：

1. 普通HTML + CSS或其他网页技术
2. 直接应用于元素（`<Video>`、`<Img>`）的效果，或通过用 [`<HtmlInCanvas>`](html-in-canvas.md) 包裹内容，它也接受 `effects`：
...
当需要使用音效时，加载 [./sfx.md](./sfx.md) 文件获取更多信息。

## 音频可视化

当需要可视化音频（频谱条、波形、低音反应效果）时，加载 [./audio-visualization.md](./audio-visualization.md) 文件获取更多信息。
...
当处理字幕或字幕时，加载 Remotion 字幕技能获取更多信息。
...
查看 [lottie.md](lottie.md) 了解如何在 Remotion 中嵌入 Lottie 动画。

## 时间

查看 [timing.md](timing.md) 了解 `interpolate()` 的更多时间技术。

## 参数化视频

查看 [parameters.md](parameters.md) 了解如何通过添加 Zod 模式使组合可参数化。

## 测量 DOM 节点

查看 [measuring-dom-nodes.md](measuring-dom-nodes.md) 了解如何在 Remotion 中测量 DOM 元素尺寸。

## 测量文本

查看 [measuring-text.md](measuring-text.md) 了解如何测量文本尺寸、将文本适配到容器以及检查溢出。

## 使用 FFmpeg

对于某些视频操作，如修剪视频或检测静音，应使用 FFmpeg。加载 [./ffmpeg.md](./ffmpeg.md) 文件获取更多信息。
...
查看 [calculate-metadata.md](calculate-metadata.md) 了解如何动态设置组合持续时间、尺寸和属性。

## 高级组合

查看 [compositions.md](compositions.md) 了解如何定义静态图像、文件夹、默认属性以及如何嵌套组合。

## 高级时间轴

查看 [sequencing.md](sequencing.md) 了解更多时间轴模式 - 延迟、修剪、限制项持续时间。
...
你可以使用 CLI 渲染单个帧以检查布局、颜色或时间。对于琐碎的编辑、纯重构或当你已经从 Studio 或先前的渲染中足够自信时，可以跳过它。
