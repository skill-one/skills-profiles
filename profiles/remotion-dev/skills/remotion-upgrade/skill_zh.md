...
3. 如果 `@remotion/cli` 不可用，手动升级：
   ...
   - 在整个项目中查找所有已安装的 `remotion` 和 `@remotion/*` 依赖项，并将它们全部升级到该确切版本。保留它们的依赖项部分和项目的工作区或目录约定。
   - 使用 `npm view @remotion/studio@<version> dependencies --json` 读取目标 Remotion 版本的 `@remotion/studio` 依赖项。将安装的辅助包（如 `zod`、`mediabunny` 和 `@huggingface/transformers`）与此处列出的版本对齐。使用 `mediabunny` 版本更新已安装的 `@mediabunny/*` 包。
   - 运行项目的包管理器以更新其锁定文件。
...
[Remotion 发布版本](https://github.com/remotion-dev/remotion/releases) 包含变更日志，在升级后总结相关更改时可能有用。
