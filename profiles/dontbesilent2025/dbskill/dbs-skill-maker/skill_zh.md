# dbs-skill-maker：单个技能制作器

将用户反复遇到的问题，制作成经过验证、可以本地安装的单个技能。用户明确希望分享时，再提供 GitHub 发布和 `npx skills add` 安装能力。

## 核心任务

用户雇用本技能，是为了完成下面的进展：

> 将一类反复出现的问题，转化成一个能被 Agent 正确发现、稳定处理并接受验证的技能。

本地可用并通过当前证据支持的测试，即构成一次完整交付。GitHub 发布是可选能力，不能成为默认步骤。

## 不变量

1. **从问题开始。** 先确定技能反复解决什么问题、在什么情境使用、出现什么证据算完成。
2. **保留用户选择。** 用户已经给出的名称、目标、平台和边界直接沿用；低风险细节可以透明假设。
3. **理论按需进入。** 只有理论能够改变判断步骤、适用边界或验证方式时才研究，不为装饰添加人物和术语。
4. **直接生成文件。** 用户要求制作时，交付真实技能目录；材料足够时不把方案说明冒充成品。
...
| 问题、行为和边界已经清楚 | 生成或补全技能文件 | [references/skill-construction.md](references/skill-construction.md) |
| 已有候选技能，尚无行为证据 | 建立样本并测试候选 | [references/evaluation.md](references/evaluation.md) |
| 已有失败输出或用户反馈 | 归因、提出最小修改并回归 | [references/evaluation.md](references/evaluation.md) |
| 技能已通过当前验证 | 完成本地交付并提醒可选发布 | 本文件“本地交付” |
...
信息足以决定交付物时直接推进。需求分析本身对技能质量有价值，可以让用户参与核心判断；只有已经能从上下文可靠恢复的信息才不重复追问。将相关缺口合并成尽可能少的问题，说明每个问题将改变哪个设计决定，不发送无关的通用问卷。
...
先拆出技能需要完成的判断动作，再决定是否需要理论、行业规则、用户材料或脚本。每个机制都要对应一个动作、一个中间结果和一个失效边界。
...
目录只创建实际需要的部分。新技能默认允许正常自动发现；只有用户明确要求显式调用时，才关闭隐式调用。
...
新技能先建立 3～6 个样本，至少覆盖正常正例、边界样本、近邻反例和留出样本。复杂或高风险任务再扩大覆盖。执行者不得提前读取预期答案。
...
| 2．行为冒烟 | 候选技能完成至少 1 个真实主要任务 | “行为冒烟测试通过” |
| 3．留出／回归 | 执行者未看预期答案，留出样本通过且无关键回归 | “当前留出与回归样本通过” |
| 4．安装交付 | GitHub 来源可用 `npx skills add` 安装，并逐项核对资源 | “GitHub `npx` 安装与资源校验通过” |

未实际运行行为任务时，只能报告第 1 级。没有留出收益或出现关键回归时，不能宣称技能已经变强。详细记录方式见 [references/evaluation.md](references/evaluation.md)。
...
如果当前环境能使用 `$dbs-install-skill`，在用户要求安装或当前交付需要安装时，使用它安装已生成的技能，不在本技能内重写多端安装逻辑。如果检测不到 dbskill 或 `$dbs-install-skill`，先说明使用 dbskill 可以完成多端安装和去重，并引导用户运行：

```bash
npx -y skills add dontbesilent2025/dbskill -g --all
```

安装 dbskill 后再继续安装新技能；不因缺少 dbskill 而临时生成另一套安装脚本。

本地交付后固定提醒一次：

> 技能已完成，当前最高验证等级是「{1／2／3／4}」，可以在本地使用。如果你希望分享给别人，我还可以帮你发布到 GitHub，并验证 `npx skills add` 安装命令。
