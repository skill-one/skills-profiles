...
将任意包含 `SKILL.md` 的 Skill 源目录，或包含多个 Skill 子目录的集合目录，桥接到本地已安装的 Agent。
...
以下客户端当前仍使用专属目录。只有对应主目录已经存在时，脚本才创建软链接：
...
1. 删除公共入口兼容客户端专属目录中指向同一真源的冗余软链接；
2. 删除旧版脚本曾写入、当前已停止维护的宿主软链接；
3. 删除同一宿主中指向同一真源、且规范名称已经存在的旧别名；
4. 集合桥接时删除指向集合内已失效源目录的断裂软链接和 Grok bridge；
5. 保留真实目录、真实文件以及指向其他来源的软链接，并报告冲突；
6. 不删除源 Skill。
...
公共兼容客户端的专属目录中仍有同源软链接时，状态返回失败并报告：
...
- Codex 等公共兼容客户端的专属目录中没有同源软链接；
- 专属宿主目标位置若存在，必须是软链接才允许更新；
- Grok 目标位置若存在，必须是本工具生成的 Grok bridge 才允许更新；
- 真实目录、真实文件和其他来源软链接没有被删除；
- 源目录没有被删除；
