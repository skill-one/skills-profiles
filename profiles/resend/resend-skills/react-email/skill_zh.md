...
使用 React 组件构建和发送 HTML 邮件。一种现代的、基于组件的电子邮件开发方法，可在所有主要电子邮件客户端上运行。
...
此方法适用于任何包管理器（npm、yarn、pnpm、bun）——相应地替换。

开发服务器在 localhost:3000 上运行，并提供 `emails` 文件夹中模板的预览界面。

### 添加到现有项目

安装包并在您的 `package.json` 中添加一个脚本：
...
确保电子邮件文件夹的路径相对于基本项目目录是相对的。确保 `tsconfig.json` 包含对 JSX 的正确支持。

## 基本电子邮件模板

使用 Tailwind 组件进行样式设置，创建一个结构正确的电子邮件组件：
...
- 如果用户要求使用媒体查询，请告知他们大多数电子邮件客户端不支持它们，并建议其他方法。
- 不要在 TypeScript 代码中直接使用模板变量（如 `{{name}}`）。相反，直接引用底层属性。如果用户明确要求 `{{variableName}}`，请仅在 PreviewProps 中放置花括号字符串，绝不在组件 JSX 中：
...
- 不要在组件结构中直接编写 `{{variableName}}` 模式。如果用户坚持，请解释这将使模板无效。
...
- `Head` - 元素、样式、字体
- `Body` - 主要内容包装器
- `Container` - 最外层的居中包装器（具有内置的 `max-width: 37.5em`）。每个电子邮件仅使用一次。
...
当用户请求电子邮件模板时，如果他们没有提供，请首先询问以下问题：

1. **品牌颜色** - 询问主要品牌颜色（十六进制代码，如 #007bff）
2. **标志** - 询问他们是否有标志文件及其格式（仅限 PNG/JPG - 如果是 SVG/WEBP 则警告）
3. **样式偏好** - 专业、休闲或极简风格
4. **生产 URL** - 静态资源在生产中将在何处托管？

## 静态文件和图片

### 目录结构

本地图片必须放置在电子邮件目录内的 `static` 文件夹中：
...
- 使用 `Tailwind` 与 `pixelBasedPreset`（电子邮件客户端不支持 `rem`）。从 `react-email` 中导入 `pixelBasedPreset`。
...
- 始终指定边框类型（`border-solid`, `border-dashed` 等）—— 电子邮件客户端不继承它。
...
| `Hr` / 任何边框 | `border-solid` (或 `border-dashed`, 等) | 电子邮件客户端不继承边框类型 |
| 单边边框 | `border-none` + 侧面 | 首先重置其他侧面的默认边框 |

### 结构说明
- 当使用 Tailwind CSS 时，始终在 `<Tailwind>` 内部定义 `<Head />`
- `<Preview>` 应始终是 `<Body>` 内部的第一个元素
- 仅在 `PreviewProps` 中包含组件实际使用的属性
- 对于已知尺寸的元素（标志、图标），使用固定宽高；对于内容图片，使用响应式尺寸（`w-full`, `h-auto`）
...
React Email 支持使用任何电子邮件服务提供商发送。查看 [references/SENDING.md](references/SENDING.md) 获取完整的发送文档，包括 Resend、Nodemailer 和 SendGrid 示例。
...
查看 [references/I18N.md](references/I18N.md) 获取完整的多语言文档。React Email 支持三个库：next-intl、react-i18next 和 react-intl。

## 电子邮件编辑器

React Email 包括一个可视化编辑器（`@react-email/editor`），可以嵌入到您的应用中。它是基于 TipTap/ProseMirror 构建的，并生成电子邮件就绪的 HTML。

查看 [references/EDITOR.md](references/EDITOR.md) 获取完整文档，包括：
- `EmailEditor` — 带有气泡菜单、斜杠命令和主题的完整功能组件
- `StarterKit` — 35+ 意识到电子邮件的扩展（标题、列表、表格、列、按钮等）
- `Inspector` — 用于编辑样式的上下文侧边栏
- `EmailTheming` — 内置主题（`basic`、`minimal`）以及可定制的 CSS 属性
- `composeReactEmail` — 导出编辑器内容为电子邮件就绪的 HTML 和纯文本
...
1. **跨电子邮件客户端测试** - Gmail、Outlook、Apple Mail、Yahoo Mail
2. **保持响应式** - 最大宽度约为 600px，在手机上测试
3. **使用绝对图片 URL** - 在可靠的 CDN 上托管
4. **编写有意义的 alt 文本** - 描述内容图片的目的和详细信息；对于装饰性图片使用 `alt=""`（间隔、分隔符、背景装饰）。React Email 的 `<Img>` 默认为 `alt=""`。
5. **提供纯文本版本** - 可访问性要求
6. **保持文件大小小于 102KB** - Gmail 会剪切较大的电子邮件
7. **添加正确的 TypeScript 类型** - 为所有电子邮件属性定义接口
8. **包含预览属性** - 添加 `.PreviewProps` 以进行开发测试
9. **使用验证过的域名** - 对于生产 `from` 地址
...
- `<Preview>` 还会发出 `<title>` 标签

升级 `npm install react-email@latest` 获取这些默认值。

**您仍然需要做（内容选择）：**
- 以 `<Heading as="h1">` 开头，按顺序嵌套副标题，不要跳过级别（非常短的短信式电子邮件可能完全跳过标题）
- 设置有意义的图片的描述性 `alt`；在装饰性图片上传递显式的 `alt=""` —— 永远不要省略属性
...
- 对于非英文电子邮件，传递区域设置：`<Html lang={locale} dir={isRTL ? 'rtl' : 'ltr'}>`（见 [I18N.md](references/I18N.md)）

对于完整规则集、严重性排名和编写清单，请参阅 `email-best-practices` 技能中的 [accessibility reference](https://github.com/resend/email-best-practices/blob/main/references/accessibility.md)。
