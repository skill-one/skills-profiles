...
你要为某个站点编写 adapter 的 agent。这份 skill 的目标是：对于简单站点，争取在 **30 分钟内从零到通过 `opencli browser verify`**；对于复杂、私有协议或涉及写操作的站点，以证据完整和安全为优先，不为了时限而猜测接口。

全程使用现有工具：`opencli browser *` / `opencli doctor` / `opencli browser init` / `opencli browser verify`。没有新命令。

调试浏览器型 adapter 时，优先直接带上 `--trace on --keep-tab true --window foreground`。`--trace on` 每轮都会生成 trace artifact，`summary.md` 是失败/成功复盘入口；`--keep-tab true --window foreground` 让 tab lease 保留且浏览器窗口在前台，方便核对最终页面状态。
