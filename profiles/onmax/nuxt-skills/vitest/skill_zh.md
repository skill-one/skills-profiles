...
- Vite 原生：使用 Vite 的转换管道实现快速 HMR 类似测试更新
- Jest 兼容：可替换大多数 Jest 测试套件
- 智能监听模式：仅根据模块依赖关系重新运行受影响的测试
- 无需配置的原生 ESM、TypeScript、JSX 支持
- 多线程工作线程实现并行测试执行
- 通过 V8 或 Istanbul 内置代码覆盖率
- 快照测试、模拟和监视工具

> 本技能基于 Vitest 5.x（测试版），生成于 2026-06-22。
...
| 覆盖率      | 通过 V8 或 Istanbul 提供商的代码覆盖率                      | [features-coverage](references/features-coverage.md)     |
...
| 测试标签    | 使用标签标记测试以过滤运行并应用共享选项                     | [features-test-tags](references/features-test-tags.md)   |
| 报告器      | 内置报告器，默认选择，CI/输出配置                           | [features-reporters](references/features-reporters.md)   |
| 基准测试    | 使用 bench 固定装置（Tinybench）编写基准测试                 | [features-benchmarking](references/features-benchmarking.md) |
