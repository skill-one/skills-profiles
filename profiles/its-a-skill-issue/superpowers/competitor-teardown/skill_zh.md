...
通过 [inference.sh](https://inference.sh) CLI 进行结构化的竞品分析，结合研究和截图。
...
| 4. **增长** | 用户量、收入、增长 | 网络搜索、新闻、融资 |
| 5. **评价** | 用户的优势和劣势 | G2、Capterra、App Store |
...
# 一般智能
belt app run tavily/search-assistant --input '{
  "query": "CompetitorX 公司概况 融资 团队规模 2024"
}'
...
# 功能比较
belt app run exa/search --input '{
  "query": "CompetitorX vs 替代品 功能比较 评价"
}'
...
| Product Hunt | 发布 | producthunt.com/posts/[product] |
...
# Reddit情绪
belt app run exa/search --input '{
  "query": "reddit CompetitorX 替代品 挫败感 转换"
}'
```
...
| 错误 | 问题 | 解决方法 |
|------|------|------|
| 仅看功能 | 错过定位、定价、增长 | 使用7层框架 |
| 偏见分析 | 失去信誉 | 诚实地说明竞品优势 |
| 数据过时 | 错误结论 | 标注所有研究，每季度更新 |
| 竞品过多 | 分析瘫痪 | 专注于前3-5个直接竞争对手 |
| 没有"所以呢" | 数据无洞察 | 每个部分都以对您的影响结束 |
| 仅功能比较 | 不显示定位 | 包括定价、评价、定位图 |
