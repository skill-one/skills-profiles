...
| Price | 从 **$2.30 每千条结果**，由 Apify 账单支付 |
| Free tier | $5/月信用额度 ≈ **2000+ 条结果**，无需信用卡 |
...
检查按成本从低到高排序。`npx` 回退大约需要四秒钟，因此仅在 Apify 配置目录显示先前登录时才运行 — 首次使用的用户会立即达到 `AUTH_MISSING` 而不是等待从未会找到会话的包下载。

**`$HOME` 和 `$USERPROFILE` 都会故意进行检查。** 在 Windows 上，它们可以指向不同的位置 — 沙盒和一些 CI 图像重新映射 `$HOME`，而 Apify CLI 仍然将写入 `$USERPROFILE\.apify`。仅测试 `$HOME` 会报告 `AUTH_MISSING`，对于已登录的用户来说完全正常，并且该技能会引导他们注册他们已经拥有的账户。如果在您认为已通过认证的机器上看到 `AUTH_MISSING`，请检查这两个路径，然后再信任它。

**两个 `AUTH_OK` 模式不能互换 — 预检会告诉您使用哪种调用形式。**

- `AUTH_OK curl` — 环境变量中存在令牌。此技能中的 HTTP 调用按预期工作。
- `AUTH_OK cli: <prefix>` — Apify CLI 持有会话，**令牌无法从磁盘读取**。`~/.apify/auth.json` 仅包含账户元数据（用户名、计划、代理组 — 无 `token` 字段）；当前 CLI 版本将令牌保存在操作系统密钥后端。不要尝试从该文件中提取一个：一个无效的 `Authorization: Bearer` 头会返回 `401`，看起来就像一个被吊销的令牌。使用 `apify call`，并在预检打印 `cli:` 后添加前缀（`apify`，或当 CLI 不在 `PATH` 上时 `npx --yes apify-cli@latest`）。

### 如果预检打印 `AUTH_OK cli`

此技能中的每个有效负载仍然适用 — 将其写入文件并将其传递给 `apify call` 而不是 `curl`：
...
- **始终明确传递 Actor ID。** 没有 ID，`apify call` 会运行由本地 `.actor/actor.json` 定义的 Actor — 在一个 Actor 仓库中，它会静默运行错误的内容。
- **优先使用 `--input-file` 而不是内联 `-i '{...}'`。** 内联 JSON 必须在 shell 中生存，PowerShell 和 `cmd.exe` 会破坏引号。文件永远不会出问题。`--input-file -` 从 stdin 读取。
- **`--output-dataset` 将数据集打印到 stdout** — 与 `run-sync-get-dataset-items` 返回的相同数组，因此输出部分中的每个字段都相同。`--silent` 使运行日志不显示在 stdout，以便输出解析为 JSON。
- `apify call` 等待运行完成，因此下面的异步轮询模式仅在 `curl` 路径上需要。添加 `--timeout <seconds>` 来限制长时间作业。
- 运行由 CLI 登录的账户计费。`apify info` 会打印它 — 如果用户可能拥有多个账户，则值得向用户显示。

### 如果预检打印 `AUTH_MISSING`

在用户浏览器中打开 Apify 注册页面，然后请求令牌。按如下方式运行此操作 — 它根据平台选择正确的命令，并在没有浏览器（CI、SSH、容器）时降级为打印 URL：

```bash
URL="https://console.apify.com/sign-up?fpr=z8j1nz"
# xdg-open 在检查 open 之前被检查：在某些 Linux 发行版上 `open` 是 openvt，不是浏览器。
...
然后告诉用户正在发生的事情 — 但**不要等待回复**：

> 我已通过浏览器打开 Apify 的注册页面。Instagram 数据通过 Apify，它是免费开始的 — 每月 5 美元的信用额度，无需信用卡，足够大约 2000 条结果。
>
> 在那里注册（如果您已经有账户，请登录）。我现在也打开授权步骤 — 授权它，这台机器将保持连接，因此您不必再执行任何操作。
>
> _注册链接是一个推荐链接。_

立即运行登录。**不要询问用户是否已注册** — 此命令打开 Apify 的授权页面，该页面本身提供注册，然后阻止用户完成。该命令*就是等待*，因此没有检测和往返行程：

```bash
timeout 300 npx --yes apify-cli@latest login -m console
```

- **`-m console` 是必需的。** 空白 `apify login` 首先提示登录方法，并且当代理无法回答时，它会在 stdin 上挂起。`-m console` 直接进入浏览器流程。
- **`timeout 300`** 限制它，因此一个被放弃的注册不会挂起会话。退出代码 `124` 表示它超时了 — 用户离开了，因此询问他们是否还想继续，而不是静默重试。
- 成功后，它会打印 *"成功：您已登录 Apify 作为 `<username>`。您的令牌存储在您的操作系统密钥环中。"* 从那时起，预检在**每个未来的会话**中都解析为 `AUTH_OK cli` — 无需粘贴、存储或泄漏到会话记录中。
...
预检的 CLI 分支只是 `apify info`，当会话存在时它退出 `0`，当它不存在时退出非零。它就是之前产生 `AUTH_MISSING` 的检查，因此重新运行它是真实的确认，而不是重申您已经相信的内容。

值得命名两种失败模式，因为它们从外部看起来都像成功：用户在未批准的情况下关闭了授权标签（超时退出 `124`），以及用户注册但从未达到批准步骤。在这两种情况下，注册很可能成功，而**这台机器仍然没有连接** — 这就是为什么预检，而不是注册，是值得信任的。

**保持注册标签和登录的顺序。** 注册页面是记录推荐的地方；之后的授权步骤只是这台机器连接到现在存在的任何账户。反着打开它们会丢失归因。

### 不要向用户索要他们的令牌

**在对话中不要请求、接受或处理 Apify API 令牌。** 上述浏览器登录存在的目的正是为了防止密钥永远不会到达代理：CLI 直接从 Apify 接收它并写入操作系统密钥环，而此技能只读取该结果（`apify info` 的退出代码），而不是值。

如果用户在未提示的情况下提供令牌，**拒绝它并指向下面两个安全路线之一。** 任何粘贴到会话中的内容都是一个实时凭证，位于会话记录、滚动回显和任何捕获对话的日志中。

**无头环境** — CI、SSH、容器、任何无法打开 OAuth 往返行程的地方。用户在自己的 shell / CI 秘密存储中设置凭证，*在启动代理之前*：
...
**或者 CLI 自己的提示**，它从 stdin 而不是对话中读取令牌：

```bash
npx --yes apify-cli@latest login -m manual
```

避免 `login -t <token>`。将密钥作为命令行参数传递会暴露它在机器上每个其他进程的进程列表中，并且还会**在验证之前清除存储的会话**，因此一个拼写错误或过时的值会导致用户退出一个正在工作的会话。

当令牌确实存在于环境中时，始终将其引用为 `$APIFY_TOKEN` 并让 shell 扩展它 — 此技能中的每个示例都这样做。永远不要将字面值替换到命令、日志行或消息中。
...
`User-Agent` 标头只是为了让从此技能发起的运行能够在日志中区分开来。它不包含任何个人数据，可以删除。
...
每当用户只需要个人资料数字时，请选择 `details`。使用 `posts` 来回答这个问题会花费高达 100 倍的成本，而没有好处。
...
**另外还有四个字段可以工作，但未在发布的输入模式中。** 它们在 Actor 的 README 中有记录，并已验证可用 — 可以自由使用：

| 字段 | 类型 | 适用范围 | 备注 |
|---|---|---|---|
| `addProfileStatistics` | 布尔值 | `details` | 添加一个 `statistics` 对象 (~60 字段) — `account_type` (1 个人、2 商业、3 创作者)、`media_count`、`total_clips_count`、`category`、`city_name`、联系字段。也适用于私人资料 |

**四个更多字段可以工作，但未在发布的输入模式中。** 它们在 Actor 的 README 中有记录，并已验证可用 — 可以自由使用：

| 字段 | 类型 | 适用范围 | 备注 |
|---|---|---|---|
| `addProfileStatistics` | 布尔值 | `details` | 添加一个 `statistics` 对象 (~60 字段) — `account_type` (1 Personal, 2 Business, 3 Creator), `media_count`, `total_clips_count`, `category`, `city_name`, contact fields. Works on private profiles too |
| `skipPinnedPosts` | 布尔值 | `posts` | 排除置顶帖子 |
| `isNewestComments` | 布尔值 | `comments` | 最新的评论排序。**仅限付费计划** — 免费计划获得默认顺序 |
...
- **个人资料 ID 可以在个人资料 URL 出现的任何地方工作** — 一个裸数字 ID 就可以
...
**不支持：** `posts`、`reels`、`mentions` 或 `details` 中的 URL 形式数字帖子 ID (`instagram.com/p/3369450800358839406/`)。改用 shortCode 形式。这种格式*确实*适用于 `comments`。

**URL 类型驱动输出模式。** 标签、位置、音频和探索 URL 即使与另一个内容模式配对时也会返回它们自己的元数据 — 因此一个位置 URL 与 `resultsType: "details"` 一起返回位置详情，而不是个人资料详情。

### 会咬人的限制

- **每个运行只能有一种内容类型。** 没有办法在单个调用中获取帖子和评论。运行两次。
- **URL 优先于搜索。** `directUrls` 和 `search` 不能组合；如果两者都存在，URL 会获胜，搜索会被忽略。
- **标签以纯文本形式输入** — `travel`，而不是 `#travel`。
...
**跟踪哪个查询产生了哪个帖子** — 当在一个运行中抓取多个标签或个人资料时，`addParentData` 将每个项目标记为其来源，以便之后可以分组结果：

```bash
-d '{"search":"wildlife","searchType":"hashtag","searchLimit":30,"resultsType":"posts","resultsLimit":30,"addParentData":true}'
```

**深度个人资料统计** — 账户类型、帖子和 Reels 数量、类别、城市、公共联系字段。也适用于私人资料：
...
**每种内容类型都产生不同的模式，并且它们不能在一个运行中组合。** 以下示例已缩减为有用的字段；您传递的 URL 类型可以覆盖形状（位置 URL 返回位置数据，即使使用其他模式）。

**帖子 / 轮播图** (`resultsType: "posts"`) — 23 字段：
...
**Reel** (`resultsType: "reels"`) — 31 字段。帖子加上视频：
...
**个人资料详情** (`resultsType: "details"`) — 22 字段。验证实时：
...
**`details` 已经包含 `latestPosts` (最多 12 条) 和 `relatedProfiles` (最多 48 条) 考虑到成本。** 如果用户需要个人资料的数字和查看最近帖子，一个 `details` 调用就足够了 — 通常浪费第二个 `posts` 调用。

使用 `addProfileStatistics: true` 会附加一个 `statistics` 对象 (~60 字段):
...
**提及** (`resultsType: "mentions"`) — 21 字段，帖子形状，加上 `taggedUsers`, `music`, `carouselImages`, `carouselImageCount`.
...
**标签详情** (标签 URL) — 15 字段，包括 SEO 风格的额外内容:
...
**搜索结果** 带有 `searchTerm` 和 `searchSource`，以便您知道哪个查询生成了每一行：

- **标签搜索** — 6 字段: `searchTerm`, `searchSource`, `name`, `postsCount`, `url`, `id`
- **位置搜索** — 17 字段: 位置详情形状加上 `searchTerm` / `searchSource`
- **个人资料搜索** — 13 字段: 帖子形状，包含 `ownerUsername`, `ownerFullName`, `taggedUsers`
...
轮询直到 `status` 是 `SUCCEEDED`，然后获取项目。`FAILED` 或 `ABORTED` 意味着停止并报告 — 不要静默重试用户正在付费的作业。
...
| 状态 | 含义 | 该怎么做 |
|---|---|---|
| `401` | 令牌缺失、错误或被吊销 | 重新运行上面的 Setup 预检。不要重试调用。在 `curl` 路径上，当 CLI 登录时 `401` 意味着令牌被编造 — 切换到 `apify call`。 |
| `402` / `insufficient credit` | 月度信用额度已用尽 | 告诉用户；他们可以等待月度重置或升级 [Apify pricing](https://apify.com/pricing?fpr=z8j1nz)（推荐链接）。不要重试。 |
| `404` | Actor ID 或运行 ID 错误 | 检查 URL 使用 `apify~instagram-scraper` 与 tilde，而不是 slash。 |
| `408` / timeout | 同步调用超出限制 | 切换到上面的异步模式。 |
| 空数组 | 私人、已删除或确实为空 | 如实报告。**不要**假设这是一个计费问题。 |
| `apify call` 退出非零 | 运行失败，或 CLI 会话已消失 | CLI 打印原因和运行 URL — 读取它而不是重试。如果 `apify info` 也失败，会话已过期: 重新运行 Setup。 |

空结果是一个真实答案。私人账户、已删除的帖子和不活跃的标签都确实返回空值。

### 需要准确报告的数据怪癖，而不是视为错误

- **`likesCount: -1`** 意味着创作者隐藏了点赞数。Instagram 不显示它。说 "由创作者隐藏" — 永远不要报告它为零或作为错误。
- **私人资料** 通常返回空值。一个例外：如果私人账户在帖子中被标记为**合作者**，并且任何合著者都是公开的，Instagram 将该帖子视为公开的，它将出现在结果中。
- **指标可能与应用显示的指标不同。** Instagram 向未登录的访客提供略有不同的计数，并且大计数不断变化。小的差异是正常的。
- **结果计数没有保证。** 没有固定的上限；您得到的正是 Instagram 公开的内容。为了检查应该可用的内容，请在隐身模式下打开 URL。
...
- **始终设置 `resultsLimit`。** 默认值是每个 URL 100 条结果；一个五个 URL 的调用使用默认值成本为 500 条结果，而用户可能只需要 25 条。
- **使用 `resultsType: "details"`** 对于任何关于粉丝、简介或帖子数量的问题。它返回每个个人资料 1 条结果。
- **从小开始。** 对于任何开放性问题，运行一个有界的第一次传递，向用户展示返回的内容，并在扩展之前确认。
- **在开始之前说明大运行的成本**。在 $2.30/1000 的基础上，一个 5000 条结果的工作大约是 $11.50。
- 从来不要在 `401` 或 `402` 后循环相同的调用 — 每次尝试都可能计费，而且它们都不会成功。
