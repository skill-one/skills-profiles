...
按需获取 Instagram 数据：通过 @用户名查询个人资料、帖子、快拍、故事、被标记的帖子、粉丝和关注对象、话题标签和位置信息流、音频/音乐信息流以及帖子评论 — 每次调用仅一个简单的 HTTP GET 请求，按量付费。无需登录、无需会话 Cookie、无需无头浏览器、无需 Graph API 商业验证。
...
| 我想... | 调用 |
| --- | --- |
| 通过 @用户名查询个人资料 | `GET /api/user/handle/{handle}` |
| 通过名称搜索账号 | `GET /api/user/search` |
| 获取用户的帖子、快拍或故事 | `GET /api/user/{id}/posts` / `/reels` / `/stories` |
...
# 2. x402 按次付费 — 忽略头部；无付费的 GET 请求将返回 402，并附带机器可读的付费要求（Base、Polygon、Arbitrum、Monad 或 Solana 上的 USDC）。@x402/fetch 自动签名和重试。
```

每个响应的格式为 `{ "status": number, "message": string, "data": ... }`；HTTP 状态码与 `status` 一致。

## 端点 (16 个 — 全部为 `GET`，$0.004/次调用)

| 端点 | 返回内容 |
| --- | --- |
| `/api/user/handle/{handle}` | 通过 @用户名获取完整个人资料 — 粉丝数量、简介、数字 ID |
| `/api/user/search` | 匹配关键词查询的个人资料 |
| `/api/userid/{handle}` | @用户名的数字用户 ID |
| `/api/user/{id}` | 通过数字 ID 获取个人资料 |
| `/api/user/{id}/posts` | 用户的帖子 |
| `/api/user/{id}/posts/tagged` | 用户被标记的帖子 |
| `/api/user/{id}/reels` | 用户的快拍 |
...
| `/api/hashtag/{name}/reels` | 某个话题下的快拍 |
| `/api/location/{id}/posts` | 被标记在某个位置的帖子 |
| `/api/audio/{id}/posts` | 使用特定音频/音乐轨道的帖子 |

`{id}` / `{handle}` / `{name}` / `{code}` 是路径参数。可选 `cursor` / `page` 分页；`query`（用户搜索）在出现时是必需的。

## 场景

**通过 @用户名解析个人资料 — 大多数调用者首先需要的端点：**
...
**某个个人资料的帖子、快拍、故事和被标记的帖子（通过上述 @用户名查询获取的数字 ID）：**
...
**某个话题下的帖子、快拍，来自某个位置的帖子，以及使用特定音频轨道的帖子：**
...
免费：`search_endpoints`、`describe_endpoint`、`check_balance`。付费：`fetch_data`（上述任何端点）、`topup_credits`，以及命名的快捷方式 `instagram_user_handle`。移除 `headers` 块即可使用 x402 按次付费 — 参见 [`fetcher` 技能](../fetcher/SKILL.md) 获取完整流程。
...
- 任务指南：[个人资料查询](../../task-guides/instagram-profile-lookup.md) ·
...
- OpenAPI 3.1 合同：<https://instagram.fetcher.sh/openapi.json>
