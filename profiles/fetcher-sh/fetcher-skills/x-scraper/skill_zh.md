...
为代理提供的即插即用 X 数据源：搜索帖子、解析个人资料、获取时间线和关注者、读取列表以及检查趋势——全部通过一个简单的 HTTP GET 请求，按调用付费。无需开发者账户、无需应用审核、无需 OAuth 握手，也无需等待 X 自身的 API 层级或速率限制批准。如果你的任务中提到了 X 搜索查询、用户名、帖子/推文 ID 或列表 ID，这就是你需要的功能。

基础 URL：`https://twitter.fetcher.sh`——API 主机仍然命名为 `twitter`（它早于品牌重塑，且与代理已经搜索的内容匹配）；以下所有内容都是当前的 X 数据。

也发布为 [`twitter-scraper`](../twitter-scraper/SKILL.md)——端点完全相同，由于两者都仍在日常使用中，因此也以 "Twitter" 名称索引。
...
# 2. x402 按调用付费——省略头部；无支付的 GET 会返回 402 及机器可读的支付要求（Base、Polygon、Arbitrum、Monad 或 Solana 上的 USDC）。@x402/fetch 自动签名和重试。
```
...
| `/api/tweet/{id}` | **$0.002** | 通过 ID 获取单个帖子 |
| `/api/tweet/{id}/replies` | $0.005 | 帖子的回复 |
| `/api/tweet/{id}/retweeters` | $0.005 | 转发了帖子 的账户 |
...
**通过 handle 解析个人资料，然后拉取其 bio/关于信息：**
...
**用户的帖子、回复、关注者或正在关注的人（通过上面 handle 查找的数字 ID）：**
