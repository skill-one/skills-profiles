...
> 最新的 ACE Step 生成。**50+ 语言人声支持**，精细的结构化歌词处理，$0.0003/s。开放权重（Apache 2.0）。
...
> 选择原因：将 30 s 的钩子扩展为 2 min 的剪辑，添加淡出，围绕现有钩子构建更长的编排。
...
单向调用即可双向扩展——设置 `extend_before_duration` 和 `extend_after_duration` 同时添加前奏和尾声。总时长上限为 4 min。
...
### “我生成了一个 30 s 的钩子，但我需要一个 2 min 的曲目”
- **路径 4（ACE Step 音频扩展）** 将钩子作为 `audio`，一次调用添加 30 s 前奏 + 60 s 尾声。

### “我的第二个副歌出错了”
- **路径 3（ACE Step 音频修复）** `start_time` / `end_time` 围绕糟糕的副歌，标签匹配原始歌曲风格。

### 廉价草稿 → 精品抛光
- 在 **路径 2（ACE Step 基础）** 上迭代标签，每尝试 $0.01–0.02 → 锁定氛围 → 最终渲染在 **路径 1（ElevenLabs Music）** 上用于抛光的商业剪辑。

### 修复不符合 ACE 时间范围模式的片段
- 目前 CLI 没有提供基于掩码的音频修复端点。要么重新表述为时间范围编辑，要么使用 **路径 2** 重新生成完整音轨并调整标签。
...
4. **成本敏感批量还是单条音轨？**
   ...
   - 单条高质量音轨 → **路径 1（ElevenLabs Music）** 或 **路径 2（ACE Step 1.5）** — 根据预算选择
5. **编辑类型？**
...
- **令牌存储**：`runcomfy login` 将 API 令牌写入 `~/.config/runcomfy/token.json`，模式 0600。设置 `RUNCOMFY_TOKEN` 环境变量以绕过文件，在 CI / 容器中。永远不要将令牌回显到提示、日志或提交中。
- **输入边界（shell 注入）**：提示、标签、歌词和音频 URL 作为 JSON 字符串通过 `--input` 传递。CLI 不 shell 扩展提示内容；它直接通过 HTTPS 将 JSON 正文传输到模型 API。**提示内容无 shell 注入表面**。
...
- [`ai-video-generation`](https://www.skills.sh/agentspace-so/runcomfy-agent-skills/ai-video-generation) — 将生成的音轨与生成的视频配对
