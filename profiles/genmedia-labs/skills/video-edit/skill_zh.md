...
| 用户意图 | 模型 | 原因 |
|---|---|---|
| 重制口播视频——保留面部/姿势/唇部运动 | **Wan 2.7 Edit-Video** | 强大的身份+动作保留；支持最高1080p |
| 交换产品背景，保持摄像机运动 | **Wan 2.7 Edit-Video** | 保留摄像机运动；单向编辑得到尊重 |
...
| 目标角色的唇部运动与源视频的唇部运动同步 | **Kling 2.6 Pro Motion Control** | 建立在紧密的时间一致性基础上 |
...
runcomfy run wan-ai/wan-2-7/edit-video \
  --input '{
    "prompt": "保留说话者的面部、姿势和唇部运动；将背景更改为现代化的办公室，使用中性光照。",
...
runcomfy run wan-ai/wan-2-7/edit-video \
  --input '{
    "prompt": "保持原始构图和手部运动；使用参考图像替换包装设计。",
...
在编辑是**局部风格修改**时使用——服装替换、场景重光、大气重制——并且身份保留至关重要。比Wan 2.7 Edit轻量；最高720p。
