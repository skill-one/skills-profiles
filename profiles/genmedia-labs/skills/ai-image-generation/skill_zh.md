...
> Step-distilled, 4–25 步骤, 原生多参考条件, 强大的照片真实 + 插画全能。
...
> FLUX 2 基础的更高保真度层级。电影 + 品牌工作, 英雄镜头。
> 选择原因：生产光泽, 品牌活动。
...
> 最高保真度的 Nano Banana 层级。基于 Gemini, 可选网络搜索用于现实世界参考（产品, 地标）。
...
> 最新字节跳动 Seedream 层级。照片真实肤色, 自然光线, 强大的东亚美学。
...
> 阿里巴巴 Qwen 最新, 开放权重, LoRA 兼容 (`/lora` 变体)。
...
> 最高保真度的 Nano Banana 编辑层级。身份保留, 多参考。
...
> 避免原因：精确遮罩区域 — 使用 [`image-edit`](https://www.skills.sh/agentspace-so/runcomfy-agent-skills/image-edit) 技能 (Z-Image Inpaint)。

**GPT Image 2 Edit** — `openai/gpt-image-2/edit`
> 最多 10 个参考图像, 多语言图像中文本重写, 布局精确的重新定位。
> 选择原因：多语言标题替换, 多参考构图, 布局重新定位, 品牌锁定身份跨翻译。
> 避免原因：遮罩驱动修复 — 使用 [`image-edit`](https://www.skills.sh/agentspace-so/runcomfy-agent-skills/image-edit) 技能。
...
> 单参考单指令, 最高保留保真度 ("保留所有内容除 X")。
> 选择原因：单图像精确局部编辑 ("仅将她的雨伞改为橙色")。
> 避免原因：批量工作, 多参考构图, 遮罩驱动修复。

> **需要遮罩驱动修复, 控制性外扩, 或完整编辑处理？** → 使用 [`image-edit`](https://www.skills.sh/agentspace-so/runcomfy-agent-skills/image-edit) 技能。
...
| 字段 | 类型 | 必填 | 默认 | 备注 |
|---|---|---|---|---|
| `prompt` | string | 是 | — | 约 512 个 token; 更长会退化。主语优先, 陈述性 |
| `steps` | int | 否 | 25 (9B) / 4 (4B) | Step-distilled; 4–8 足够用于构思, ~25 用于光泽, >25 购买收益不大 |
| `width` | int | 否 | 1024 | 512–1536 典型, 最大 ~2K 总计。长宽比上限 16:9 |
| `height` | int | 否 | 1024 | 与 width 的长宽比意图匹配 |

在同一个端点支持最多 **4 个参考图像** 用于风格迁移 / 指导构图。字段名在 [模型页面](https://www.runcomfy.com/models/blackforestlabs/flux-2-klein/9b/text-to-image?utm_source=skills.sh&utm_medium=skill&utm_campaign=ai-image-generation) 上有文档。
...
runcomfy run blackforestlabs/flux-2-klein/9b/text-to-image \
  --input '{
    "prompt": "一只紫色的小猫坐在覆盖着苔藓的石头上，金色黄昏的边缘光，浅景深，照片真实",
    "steps": 25,
    "width": 1536,
    "height": 864
  }' \
...
- **主语优先, 场景其次, 修饰语最后。** "一只紫色的小猫 … 坐在苔藓石头上 … 金色黄昏, 浅景深."
- **步骤策略**: 4–8 用于构思, ~25 用于光泽。不要超过 28 — 收益递减。
- **9B vs 4B**: 默认 9B; 仅当您需要亚秒级批量构思时才降至 4B。
...
runcomfy run openai/gpt-image-2/text-to-image \
  --input '{
    "prompt": "极简产品海报。居中粗体标题精确地读取 \"AURORA — Spring 2026\"，使用干净的白色无衬线字体，在深蓝色背景下。标题下方有一行小字，使用等宽字体读取 \"runs on water\"。3:2 布局。",
    "size": "1536_1024"
  }' \
  --output-dir ./out
```
...
runcomfy run openai/gpt-image-2/text-to-image \
  --input '{
    "prompt": "日本杂志封面。垂直标题精确地读取 \"今日のおすすめ\"，使用粗体日语假名, 右边缘对齐, 照片真实女性肖像。",
    "size": "1024_1536"
  }' \
  --output-dir ./out
```
...
- **精确引用图像中文本。** `"标志读取精确地 'CLOSED'"` — 不加引号，模型会意译。
- **为非拉丁文命名脚本**: `"Japanese kana"`, `"Cyrillic"`, `"Arabic right-to-left"`. 不加此内容，它会回退到罗马化。
- **布局语言得到尊重**: `"top-left"`, `"centered"`, `"two-line stacked"`, `"baseline aligned"`.
- **仅 3 个尺寸。** 不要传递任意宽度。
...
- **主语优先, 陈述性。** "一个咖啡杯放在大理石上" 比 "生成一个创意咖啡杯镜头" 更好。
- **`enable_web_search: true`** 当提示中命名真实产品, 地点或人物，其外观必须与现实匹配（标志, 地标）。
- **降至 `0.5K` 用于构思, 仅当最终时跳至 `2K`+ — `4K` ~16 倍于 `0.5K` 的成本。
