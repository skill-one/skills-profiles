...
**图像转视频，意图路由。** 这个技能不会将你锁定在一个模型上——它会根据用户实际想要的效果，在 RunComfy 目录中挑选合适的 i2v 模型：肖像动画、自定义配音口型同步或多模态合成。
...
| 用户意图 | 模型 | 原因 |
|---|---|---|
| 动画化肖像——保持身份稳定 | **HappyHorse 1.0 I2V** | 在 Artificial Analysis Arena 上排名第一（Elo 1392）；强大的面部保真度 |
| 产品发布 / 360度 / 微距运动 | **HappyHorse 1.0 I2V** | 几何保真度 + 平滑的摄像机移动 |
| 单次处理中同步原生环境音 | **HappyHorse 1.0 I2V** | 处理中音频合成 |
| 动画化 **并** 口型同步到 **自定义配音轨道** | **Wan 2.7 + `audio_url`** | 接受你自己的 MP3/WAV（3–30秒，≤15MB）并驱动口型同步到它 |
...
| 品牌一致的故事——角色参考 + 场景参考 + 音频参考 | **Seedance 2.0 专业版** | 图像保持身份，视频保持场景，音频保持声音 |
| 未指定时的默认值 | **HappyHorse 1.0 I2V** | 最佳综合质量 + 原生音频 |
...
| 字段 | 类型 | 必填 | 默认 | 备注 |
|---|---|---|---|---|
| `image_url` | string | 是 | — | JPEG/JPG/PNG/WEBP。最小 300px。长宽比 1:2.5–2.5:1。≤10MB。 |
| `prompt` | string | 是 | — | ≤5000 个非中文字符或 2500 个中文字符。**动作 / 摄像机 / 照明** 描述。 |
| `resolution` | enum | 否 | `1080P` | `720P` 或 `1080P`。 |
| `duration` | int | 否 | 5 | 3–15 秒。 |
| `seed` | int | 否 | 0 | 用于变体比较时重复。 |
| `watermark` | bool | 否 | true | 提供水印切换。 |
...
runcomfy run happyhorse/happyhorse-1-0/image-to-video \
  --input '{
    ...
    "prompt": "缓慢地围绕人物的脸上漂移摄像机，轻微的呼吸动作，身份稳定的特征，柔和的自然光。"
  }' \
  --output-dir <绝对路径>
```
...
- **以动作动词开头**: "漂移", "推近", "轨道", "仰角", "揭示", "眨眼", "呼吸"。优先加载正在移动的内容。
- **不要重述图像** — 模型已经看到了它。专注于变化的标记。
- **明确保存目标**: "身份稳定的特征", "包装不变", "背景几何稳定"。
- **照明演变**: "轮廓光增强", "随着摄像机上升，阴影变短"。
- **每个剪辑一拍** — 单一主要动作（轨道 OR 推近 OR 仰角 OR 角色动作）。

---

## 路径 2：Wan 2.7 + `audio_url` — 当用户有自定义配音时

**模型**: `wan-ai/wan-2-7/text-to-video`（不是 `/image-to-video` — Wan 2.7 的 t2v 端点接受一个 `audio_url` 驱动口型同步）

**关于 Wan 2.7 的 i2v 注意**: Wan 2.7 的主要 i2v 动画不在此处专门的端点上。对于纯 i2v（仅通过动作提示动画的图像），请优先选择 **HappyHorse i2v**。当用户有自定义音频轨道并希望将其口型同步到生成的说话头片段时，专门使用 Wan 2.7。

### Schema (Wan 2.7 t2v 带音频)

| 字段 | 类型 | 必填 | 默认 | 备注 |
|---|---|---|---|---|
| `prompt` | string | 是 | — | 约 5000 个字符。描述说话头镜头：构图、照明、动作。 |
| `audio_url` | string | 是（用于口型同步） | — | WAV/MP3，3–30秒，≤15MB。**驱动口型同步。** |
| `aspect_ratio` | enum | 否 | `16:9` | `16:9`, `9:16`, `1:1`, `4:3`, `3:4`。 |
| `resolution` | enum | 否 | `1080p` | `720p` 或 `1080p`。 |
| `duration` | enum | 否 | `5` | 2–15（整秒）。匹配音频长度。 |
| `negative_prompt` | string | 否 | — | 要避免的具体问题（例如 "无字幕，无闪烁"）。 |
| `seed` | int | 否 | — | 可重复性。 |
...
runcomfy run wan-ai/wan-2-7/text-to-video \
  --input '{
    "prompt": "自信的发言人中等特写，在柔和照明的录音室中，略微向摄像机倾斜，三脚架固定，浅景深，来自摄像机左侧的温暖主光。",
...
- **匹配 `duration` 到音频长度** — 如果太长，片段在音频结束后将保持沉默。
- **使用 `negative_prompt` 指出问题**: `"无字幕，无闪烁，无变形的手"`.
...
当用户想要单个片段结合：**主题图像** + **参考视频中的场景** + **参考音频中的声音语调** 时使用。

### Schema (Seedance 2.0 专业版，与 i2v 相关的字段)

| 字段 | 类型 | 必填 | 默认 | 备注 |
|---|---|---|---|---|
| `prompt` | string | 是 | — | CN ≤500 字符 OR EN ≤1000 字。 |
| `image_url` | array | 是（用于 i2v） | `[]` | 0–9 张图像。**第一个是主要主题。** |
| `video_url` | array | 否 | `[]` | 0–3 个参考片段（MP4/MOV），每个 2–15 秒。 |
| `audio_url` | array | 否 | `[]` | 0–3 个参考音频（WAV/MP3），每个 2–15 秒，< 15MB。 |
...
| `generate_audio` | bool | 否 | true | 处理中同步语音 / 音效 / 音乐。 |
| `seed` | int | 否 | — | 可重复性。 |
...
runcomfy run bytedance/seedance-v2/pro \
  --input '{
    "prompt": "主题从图像 1 走过咖啡馆，在视频 1 中，声音语调匹配音频 1。中等特写，缓慢推近，温暖的光线，轻柔的氛围。",
...
- **提示中编号参考**: `"主题从图像 1，照明从视频 1，声音从音频 1"`。Seedance 正确路由提示。
...
- **无多路径混合。** 此技能每次调用选择一个模型。如果用户想要 HappyHorse 动画 + Wan 风格口型同步在同一片段中，那是两个调用 + 一个拼接（在此范围之外）。
- **品牌特定覆盖** — 如果用户命名了未列出的特定模型变体（例如 Wan 2.6，Seedance 1.5），路由到相应的品牌技能（`wan-2-7`，`seedance-v2`）而不是强制通过这里。
...
该技能根据用户意图选择 HappyHorse 1.0 I2V / Wan 2.7 t2v+音频 / Seedance 2.0 专业版之一，并调用 `runcomfy run <model_id>` 使用匹配的 JSON 正文。CLI 向模型 API POST，轮询请求，获取结果，并将任何 `.runcomfy.net`/`.runcomfy.com` URL 下载到 `--output-dir`。`Ctrl-C` 在退出前取消远程请求。

## 安全与隐私

- **令牌存储**: `runcomfy login` 将 API 令牌写入 `~/.config/runcomfy/token.json`，模式为 0600（仅所有者可读写）。设置 `RUNCOMFY_TOKEN` 环境变量以在 CI / 容器中完全绕过文件。
