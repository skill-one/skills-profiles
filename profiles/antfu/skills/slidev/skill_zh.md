...
- `---` 分隔幻灯片
- 第一行 frontmatter = headmatter（演示文稿配置）
- HTML 注释 = 演讲者笔记
...
| frontmatter | 单张幻灯片配置选项 | [core-frontmatter](references/core-frontmatter.md) |
...
| 功能 | 使用方式 | 参考 |
|------|----------|------|
| 录制 | 按 `G` 摄像 | [presenter-recording](references/presenter-recording.md) |
...
| 带 PDF 构建 | `download: true` | [build-pdf](references/build-pdf.md) |
| 缓存图像 | 自动针对远程 URL | [build-remote-assets](references/build-remote-assets.md) |
| OG 图像 | `seoMeta.ogImage` 或 `og-image.png` | [build-og-image](references/build-og-image.md) |
