...
JavaScript/TypeScript 单一仓库的构建系统。Turborepo 缓存任务输出，并根据依赖关系图并行运行任务。

## 重要提示：使用包任务，而非根任务

**优先使用包任务，而非根任务。**

在创建任务/脚本/管道时，你必须默认使用包任务：

1. 将脚本添加到每个相关包的 `package.json` 中
2. 在根 `turbo.json` 中注册任务
3. 根 `package.json` 仅通过 `turbo run <task>` 进行委托

**不要**在可以存在于包中的情况下将任务逻辑放在根 `package.json` 中。这会破坏 Turborepo 的并行化。
...
根任务（`//#taskname`）仅适用于确实无法存在于包中的任务，例如 Vitest 项目的 `//#test`、全仓库发布脚本或不会调用 `turbo` 的工具。
...
**简写 `turbo <tasks>` 仅适用于一次性终端命令**，由人类或代理直接输入。永远不要将 `turbo build` 写入 package.json、CI 或脚本中。
...
**`--affected` 是运行仅更改包的主要方式。** 它会自动与默认分支进行比较并包含依赖项。
...
1. **如果依赖项已声明**（例如，package.json 中的 `"@repo/types": "workspace:*"`），则删除 `prebuild` 脚本。Turbo 的 `dependsOn: ["^build"]` 会自动处理此情况。
...
**关键洞察：** `^build` 仅在列出的依赖项中运行构建。没有依赖项声明 = 没有自动构建排序。
...
使用 `futureFlags.globalConfiguration` 可以减少这个问题，因为 `global.inputs` 文件会折叠到每个任务的输入中（而不是全局哈希）。任务可以排除特定文件：

```json
// 最佳 - global.inputs 与任务级排除
{
...
一个大型 `env` 数组（即使有 50+ 个变量）**不是**问题。这通常意味着用户在构建的环境依赖项上很彻底。不要将此视为问题。

### 使用 `--parallel` 标志

`--parallel` 标志绕过 Turborepo 的依赖关系图。如果任务需要并行执行，请正确配置 `dependsOn` 而不是使用 `--parallel`。
...
不要使用 `../` 等相对路径来引用包外的文件。使用 `$TURBO_ROOT$` 代替。
...
当 `tsconfig.json` 中的 `incremental: true`，`tsc --noEmit` 会写入 `.tsbuildinfo` 文件，即使不生成 JS。在假设没有输出之前检查 tsconfig：
...
Turborepo 不会加载 `.env` 文件——你的框架会加载。但 Turborepo 需要知道有关更改的信息：
...
单一仓库根目录中的 `.env` 文件是一种反模式——即使是小型单一仓库或启动模板。它创建了包之间的隐式耦合，并使它不明确哪些包依赖于哪些变量。
...
- 不清楚哪些包使用哪些变量
- 所有包都会收到所有变量（即使它们不需要）
- 缓存失效粒度粗糙（根 .env 变化使所有内容失效）
- 安全风险：包可能会意外访问其他包打算使用的敏感变量
- 不良习惯从小处开始——启动模板应建模正确的模式

**如果你必须共享变量**，请使用 `globalEnv` 来明确共享的内容，并记录原因。

### 严格模式过滤 CI 变量

默认情况下，Turborepo 仅过滤环境变量为 `env`/`globalEnv` 中的那些。CI 变量可能缺失：
...
根 `turbo.json` 中的 `dev` 任务具有 `dependsOn: ["^dev"]` 和 `persistent: false` 可能看起来不寻常，但**正确**用于 `turbo watch` 工作流：
...
// 包 turbo.json (apps/web/turbo.json)
{
  ...
  "tasks": {
    "dev": {
      "persistent": true  // 应用运行长时间运行的开发服务器
    }
  }
}
```
...
- **包**（例如，`@acme/db`, `@acme/validators`）具有 `"dev": "tsc"` — 一个 Shot 类型生成，完成速度快
- **应用**使用 `persistent: true` 运行实际的开发服务器（Next.js 等）
- **`turbo watch`** 重新运行包的 `dev` 脚本，在源文件更改时保持类型同步

**预期用法：** 运行 `turbo watch dev`（不要 `turbo run dev`）。监视模式在文件更改时重新执行一个 Shot 任务，同时保持持久化任务运行。

**替代模式：** 使用不同的任务名称，如 `prepare` 或 `generate`，用于一次性依赖项构建，以使意图更清晰：
...
`transit` 任务创建依赖关系，但与任何实际脚本不匹配，因此任务可以并行运行并具有正确的缓存失效。
...
使用 `futureFlags.globalConfiguration`，相同的配置会移动到 `global` 下——`.env` 成为每个任务的输入，而不是全局哈希输入：
...
| [caching/remote-cache.md](./references/caching/remote-cache.md) | Vercel 远程缓存，自我托管，登录/链接 |
...
| 文件                                                                          | 目的                                                         |
| ----------------------------------------------------------------------------- | --------------------------------------------------------------- |
| [best-practices/RULE.md](./references/best-practices/RULE.md)                 | 单一仓库最佳实践概述                                |
...
| 文件                                        | 目的                                         |
| ------------------------------------------- | ----------------------------------------------- |
| [watch/RULE.md](./references/watch/RULE.md) | turbo watch，中断任务，开发工作流 |
...
- 源：Turborepo 存储库中的 `apps/docs/content/docs/`
