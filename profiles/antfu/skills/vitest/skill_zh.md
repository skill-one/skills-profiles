Vitest 是一款由 Vite 驱动的下一代测试框架。它提供了与 Jest 兼容的 API，并内置原生 ESM、TypeScript 和 JSX 支持。Vitest 与您的 Vite 应用共享相同的配置、转换器、解析器和插件。
...
> 本指南基于 Vitest 5.x（测试版），生成于 2026-06-22。

## 核心

| 主题 | 描述 | 参考 |
|------|------|------|
| 配置 | Vitest 与 Vite 配置集成，defineConfig 的使用 | [core-config](references/core-config.md) |
...
| 覆盖率 | 通过 V8 或 Istanbul 提供商实现代码覆盖率 | [features-coverage](references/features-coverage.md) |
| 测试上下文 | 测试固定件，context.expect，test.extend 用于自定义固定件 | [features-context](references/features-context.md) |
| 并发 | 并行测试，并行执行，分片 | [features-concurrency](references/features-concurrency.md) |
| 过滤 | 按名称、文件模式、标签过滤测试 | [features-filtering](references/features-filtering.md) |
| 测试标签 | 使用标签标记测试，以过滤运行并应用共享选项 | [features-test-tags](references/features-test-tags.md) |
| 报告器 | 内置报告器，默认选择，CI/输出配置 | [features-reporters](references/features-reporters.md) |
| 基准测试 | 使用 bench 固定件（Tinybench）编写基准测试 | [features-benchmarking](references/features-benchmarking.md) |
