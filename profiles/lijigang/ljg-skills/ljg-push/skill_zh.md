...
一键同步本地 `~/.agents/skills/ljg-*` 中修改过的技能到 GitHub 仓库，覆盖 master 和 md 两个分支。

## 仓库路径（硬编码）

```
SKILLS_REPO="$HOME/code/ljg-skills"     # 本地工作仓库
SKILLS_LOCAL="$HOME/.agents/skills"      # 本地技能源
REPO_URL="git@github.com:lijigang/ljg-skills.git"
```

如果 `$SKILLS_REPO` 不存在，脚本会自动 clone。如果它存在但不是 ljg-skills 的 git 仓库，脚本会报错退出（不破坏现有目录）。
...
`~/.agents/skills/` 里的技能是 *master 风格*（源版本）。md 分支的差异由脚本自动转换 + 必要时手工补。
...
1. *新增技能了吗*？README 的技能清单 / 安装命令需要加一行
2. *删了技能了吗*？README 对应行要删
3. *某个技能的描述大改了吗*？README 的简介可能要同步
...
- *README 漂移是最容易被忽略的*——加完新技能直接推，README 还停在老清单。脚本现在有硬 gate 拦这一刀；拦下来时不要无脑加 `--skip-readme-check`，先去看一下 README
