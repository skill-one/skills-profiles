...
1. 先读取 `../byted-mediakit-shared/SKILL.md`，执行统一前置检查；该技能缺失时停止并提示安装。
...
5. 执行时设置 `MEDIAKIT_SURFACE=skill`，指定调用来源是技能。
...
| adjust-video-speed | 用于视频调速，通过调整播放倍速产生快放或慢放效果。 | Cloud / Local | `mediakit-cli editing adjust-video-speed` | [reference/adjust-audio-speed.md](reference/adjust-audio-speed.md) |
