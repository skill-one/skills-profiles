...
**QianWen-AI/qianwen-ai** 技能包的自动版本检查器。将本地安装的版本与 GitHub 上的最新发布版本进行比较，并在有更新可用时通知用户。
...
4. 在 `<repo_root>/.agents/state.json` 中记录检查时间戳（`last_interaction`），以限制网络请求每 24 小时最多一次。
