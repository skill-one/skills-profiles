...
> 一个命令行工具（CLI）：将用户问题 + 指定的 **Alice 技能（中文名或英文名均可）** 发送到万得 Alice Agent 接口，通过 SSE 流式拉取并打印 `agentResult.value`。

---

## 关键机制（必读）

实测：Alice 服务端**不是**通过 `selectedSkillIds` / `activatedSkills` 来选择技能的，而是通过 **prompt 文本前缀**（按问句语言二选一）：
